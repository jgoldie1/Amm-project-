...
const PROVIDERS = {
  ...
  flutterwave: {
    id: 'flutterwave',
    currency: 'NGN',
    methods: ['card','bank-transfer','virtual-account','ussd'],
...
module.exports = function registerNigeriaPayments({ app, auth, admin, clean, id, getStore, saveStore }) {
  ...
  function appendLedger(reference, entries) {
    ...
    for (const entry of entries) store.ledgerEntries.push({ id: id('led'), reference, createdAt: new Date().toISOString(), ...entry });
  }
  ...
  app.post('/api/payments/nigeria/intents', auth, async (req, res) => {
    ...
    if (!['holocredits','subscription','marketplace','ticket','advertising'].includes(purpose)) return res.status(400).json({ error: 'Unsupported payment purpose' });
    ...
    const intent = {
      ...
      providerReference: null, production: false, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
    };
    ...
    await saveStore();
    res.status(201).json({ intent, next: provider.configured() ? 'initialize-with-provider-sandbox' : 'configure-provider-secret' });
  });
  ...
  app.post('/api/payments/nigeria/intents/:intentId/simulate-success', auth, async (req, res) => {
    ...
    intent.status = 'succeeded'; intent.providerReference = `sandbox_${intent.provider}_${crypto.randomUUID()}`; intent.updatedAt = new Date().toISOString();
    appendLedger(intent.id, [
    ...
    await saveStore();
    res.json({ intent, ledgerPosted: true, simulation: true });
  });
  ...
  app.post('/api/webhooks/paystack', async (req, res) => {
    ...
    store.webhookEvents.push({ id: id('wh'), provider: 'paystack', signatureValid: valid, payload: req.body, createdAt: new Date().toISOString() });
    await saveStore();
  ...
  app.post('/api/webhooks/flutterwave', async (req, res) => {
    ...
    store.webhookEvents.push({ id: id('wh'), provider: 'flutterwave', signatureValid: valid, payload: req.body, createdAt: new Date().toISOString() });
    await saveStore();
    if (!valid) return res.status(401).json({ error: 'Invalid webhook signature' });
    res.json({ received: true });
  });
  ...
  app.post('/api/payouts/nigeria', auth, async (req, res) => {
    ...
    const provider = selectProvider(clean(req.body.provider, 30).toLowerCase());
    const payout = { id: id('ngout'), userId: req.user.id, provider: provider.id, amountMinor, currency: 'NGN', status: 'eligibility_review', idempotencyKey, production: false, createdAt: new Date().toISOString() };
    store.payouts.push(payout);
    await saveStore();
    res.status(201).json({ payout, next: 'recipient-validation-and-provider-submission' });
  });
  ...
  app.get('/api/admin/payments/nigeria/reconciliation', auth, admin, (_req, res) => {
    ...
    res.json({
      ...
      paymentIntents: store.paymentIntents.length, payouts: store.payouts.length,
      webhookEvents: store.webhookEvents.length, ledgerEntries: store.ledgerEntries.length,
      missingLedger: missingLedger.map(item => item.id), status: missingLedger.length ? 'review-required' : 'balanced-for-recorded-sandbox-events'
    });
  });
};
