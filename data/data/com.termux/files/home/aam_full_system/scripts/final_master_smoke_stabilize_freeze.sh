...
echo "=== FINAL MASTER SMOKE + STABILIZE + FREEZE ==="
...
curl -s -X POST http://127.0.0.1:4900/join-api \
  ...
  -d '{"username":"final_master_user","referrer":"all_american_creator"}'
echo
...
find studio_os -maxdepth 4 -type f | sort | tail -n 30
...
tar -czf snapshots/final/final_master_bundle_${STAMP}.tar.gz studio_os public/streaming scripts

cat > snapshots/final/final_master_status_${STAMP}.txt <<EOF
checkpoint=$STAMP
