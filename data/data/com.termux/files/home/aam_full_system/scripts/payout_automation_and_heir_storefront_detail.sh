...
echo "=== PAYOUT AUTOMATION + HEIR STOREFRONT DETAIL START ==="
...
for rule_name, cycle_type, target_type, target_id in rules:
    exists = cur.execute("""
...
conn.commit()
conn.close()
print("[OK] payout automation DB ready")
PYEOF
...
helper = r'''
function renderHeirStorefrontDetailPage(heirId, user = null) {
...
if "function renderHeirStorefrontDetailPage(heirId, user = null)" not in text and marker in text:
...
if "pathname.startsWith('/heir-storefronts/')" not in text and anchor in text:
    route = """    if (req.method === 'GET' && pathname.startsWith('/heir-storefronts/')) {
      const session = hardenPublicSession(req);
      const heirId = Number(pathname.split('/')[2]);
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      return res.end(renderHeirStorefrontDetailPage(heirId, session));
    }
    ...
    if (req.method === 'POST' && pathname.startsWith('/payout-automation/run/')) {
      ...
      dbRun(`INSERT INTO payout_automation_runs (
cd ~/aam_full_system

cat > scripts/payout_automation_and_heir_storefront_detail.sh << 'EOF'
#!/data/data/com.termux/files/usr/bin/bash
set -e
...
# Add detail links to ownership list
text = text.replace(
    "<td>${r.name || ''}</td>",
    "<td><a href=\"/heir-storefronts/${r.heir_id || ''}\">${r.name || ''}</a></td>"
)

p.write_text(text)
print("[OK] payout automation + storefront detail UI added")
PYEOF
...
cp apps/dashboard.js "backups/dashboard_payout_automation_${STAMP}.js"
cp apps/world_socket.js "backups/world_socket_payout_automation_${STAMP}.js"
cp db/aam.db "backups/aam_payout_automation_${STAMP}.db"
...
cat > "reports/payout_automation_storefront_detail_${STAMP}.txt" <<REPORT
PAYOUT AUTOMATION + HEIR STOREFRONT DETAIL REPORT
Timestamp: ${STAMP}

Added:
- payout_automation_rules
- payout_automation_runs
- per-heir storefront detail route
- manual heir payout action
- automation payout run action
...
echo "PAYOUT AUTOMATION + HEIR STOREFRONT DETAIL COMPLETE: $STAMP"
echo "Test:"
