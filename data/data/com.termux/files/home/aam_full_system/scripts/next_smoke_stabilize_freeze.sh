...
echo "=== NEXT SMOKE + STABILIZE + FREEZE ==="
...
curl -s -X POST http://127.0.0.1:4900/join-api \
  ...
  -d '{"username":"next_check_user","referrer":"all_american_creator"}'
echo
...
tar -czf snapshots/final/next_bundle_${STAMP}.tar.gz studio_os public/streaming scripts

cat > snapshots/final/next_status_${STAMP}.txt <<EOF
checkpoint=$STAMP
