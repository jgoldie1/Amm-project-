...
cd ~/aam_full_system
echo "=== FIX WEB3D ROUTE PATCH ROBUST START ==="
...
# 1) BACKUPS
########################################
cp apps/dashboard.js "backups/dashboard_web3d_route_robust_${STAMP}.js"
cp db/aam.db "backups/aam_web3d_route_robust_${STAMP}.db"
...
<body>
  <div class="card">
    ...
    <p>Recovery page created because the full world client HTML was missing.</p>
    <p><a href="/web3d-client">Open Web 3D Client</a></p>
  </div>
</body>
</html>
HTML
...
if "function renderWeb3DClientPage(req, user = null, message = '')" not in text:
    ...
    if marker not in text:
        raise SystemExit("Could not find server creation marker")
...
if "pathname === '/world3d'" not in text or "pathname === '/web3d-client'" not in text:
    # insert immediately after server start line
    marker = "const server = http.createServer(async (req, res) => {"
    if marker not in text:
        raise SystemExit("Could not find insertion marker for routes")
    text = text.replace(marker, marker + route_block, 1)

p.write_text(text)
print("[OK] robust web3d route patch applied")
PYEOF
...
# 8) REPORT
########################################
cat > "reports/fix_web3d_route_patch_robust_${STAMP}.txt" <<REPORT
FIX WEB3D ROUTE PATCH ROBUST REPORT
Timestamp: ${STAMP}
...
echo "FIX WEB3D ROUTE PATCH ROBUST COMPLETE: $STAMP"
echo "Check:"
