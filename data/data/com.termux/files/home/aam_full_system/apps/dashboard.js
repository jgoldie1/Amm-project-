...
<head>
  ...
  <title>${title}</title>
  <style>
    body { font-family: Arial, sans-serif; background:radial-gradient(circle at top, #13203f 0%, #081120 45%, #030712 100%); color:#e2e8f0; margin:0; padding:20px; }
    .topbar,.card { background:linear-gradient(180deg, rgba(17,24,39,.94), rgba(15,23,42,.90)); border:1px solid rgba(96,165,250,.30); border-radius:18px; padding:16px; box-shadow:0 0 24px rgba(37,99,235,.14), inset 0 0 18px rgba(255,255,255,.02); backdrop-filter:blur(10px); }
    .topbar { margin-bottom:20px; }
    .grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:16px; }
    .section { margin-top:24px; }
    .pill { display:inline-block; padding:6px 10px; background:linear-gradient(90deg,#2563eb,#7c3aed); border-radius:999px; margin-right:6px; margin-top:6px; font-size:13px; box-shadow:0 0 12px rgba(59,130,246,.35); }
    .hero { padding:24px; border-radius:22px; background:linear-gradient(135deg, rgba(37,99,235,.20), rgba(124,58,237,.18), rgba(6,182,212,.14)); border:1px solid rgba(96,165,250,.34); box-shadow:0 0 30px rgba(59,130,246,.18); }
    .hero h2 { margin-top:0; font-size:30px; }
    .hero p { color:#cbd5e1; }
    .launch-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(190px,1fr)); gap:14px; margin-top:16px; }
    .launch-tile { display:block; padding:18px; border-radius:18px; text-decoration:none; color:#fff; background:linear-gradient(135deg, rgba(30,41,59,.96), rgba(37,99,235,.28)); border:1px solid rgba(96,165,250,.28); box-shadow:0 0 18px rgba(37,99,235,.16); }
    .launch-tile strong { display:block; font-size:16px; margin-bottom:6px; }
    a,button { color:white; background:linear-gradient(90deg,#2563eb,#4f46e5); border:none; border-radius:10px; padding:10px 14px; text-decoration:none; display:inline-block; margin-right:8px; cursor:pointer; box-shadow:0 0 10px rgba(37,99,235,.25); }
...
function renderDashboard() {
  ...
  const holoGrid = `
    <div class="grid">
      <div class="card"><h3>Dashboard Core</h3><p class="ok">ONLINE · 4900</p></div>
      <div class="card"><h3>Jarvis Core</h3><p class="ok">ONLINE · 5000</p></div>
      <div class="card"><h3>Life World</h3><p class="ok">LIVE · 4902</p></div>
      <div class="card"><h3>Holo GPT</h3><p class="ok">ENABLED</p></div>
      <div class="card"><h3>Holo Search</h3><p class="ok">ENABLED</p></div>
  ...
  const launchTiles = `
    <div class="launch-grid">
      <a class="launch-tile" href="/modules"><strong>Modules Registry</strong><span>Open AI, commerce, and system modules.</span></a>
      <a class="launch-tile" href="http://127.0.0.1:4902/"><strong>Launch Life World</strong><span>Open the live playable world.</span></a>
      <a class="launch-tile" href="http://127.0.0.1:5000/"><strong>Jarvis Core</strong><span>Open AI command routing.</span></a>
      <a class="launch-tile" href="/university"><strong>University</strong><span>Open education systems.</span></a>
      <a class="launch-tile" href="/marketplace"><strong>Marketplace</strong><span>Open creator and commerce lanes.</span></a>
    </div>
  `;
  ...
  return htmlPage('AAM Dashboard', `
    <div class="section">
      <div class="hero">
        <h2>Holo Command Center</h2>
        <p>Dynamic control surface for AI, commerce, world systems, creators, and platform operations.</p>
        ${launchTiles}
      </div>
    </div>
    <div class="section">
      <div class="card">
        <h2>Live Holographic System Grid</h2>
        <p class="muted">Real-time visual overview of the AAM beta ecosystem.</p>
      </div>
      ${holoGrid}
    </div>
    <div class="section">
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if ((req.method === 'GET' || req.method === 'HEAD') && pathname === '/nft-launchpad') {
      const authUser = requireAuth(req, res, ['root', 'admin']);
      if (!authUser) return;
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    ...
    if ((req.method === 'GET' || req.method === 'HEAD') && pathname === '/creator-payouts') {
      const authUser = requireAuth(req, res, ['root', 'admin']);
      if (!authUser) return;
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
    ...
    if ((req.method === 'GET' || req.method === 'HEAD') && pathname === '/ai-control') {
      const authUser = requireAuth(req, res, ['root', 'admin']);
      if (!authUser) return;
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
