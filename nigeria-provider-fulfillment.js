...
module.exports = function registerNigeriaProviderFulfillment({ app, auth, admin, clean, getStore, saveStore, persistence, appendLedger }) {
  ...
  app.post('/api/payments/nigeria/intents/:intentId/initialize', auth, async (req, res, next) => {
    try {
      ...
      if (!['created','initialization_failed'].includes(intent.status)) {
        return res.status(409).json({ error: `Intent cannot initialize from ${intent.status}` });
      }
      const email = clean(req.user.email, 200);
      if (!email) return res.status(400).json({ error: 'Account email is required' });
      if (intent.provider === 'paystack' && !providers.paystackConfigured()) {
        return res.status(503).json({ error: 'Paystack sandbox is not configured' });
      }
      if (intent.provider === 'flutterwave' && !providers.flutterwaveConfigured()) {
        return res.status(503).json({ error: 'Flutterwave sandbox is not configured' });
      }
  ...
  app.post('/api/payments/nigeria/intents/:intentId/verify-provider', auth, async (req, res, next) => {
    try {
      const intent = findIntent(req.params.intentId, req.user);
      if (!intent) return res.status(404).json({ error: 'Payment intent not found' });
      if (intent.status === 'succeeded') return res.json({ intent, duplicate: true, ledgerPosted: true });
      ...
      if (!sameReference || !amountMatches || !currencyMatches || !succeeded) {
        ...
        const persisted = await persistIntent(intent, req.user.id, 'payment.provider.verification_failed', {
          sameReference, amountMatches, currencyMatches, providerStatus: verified.status
        });
  ...
  app.post('/api/payments/nigeria/intents/:intentId/refund-request', auth, async (req, res) => {
    ...
    const refund = {
      id: `refund_${Date.now()}_${Math.random().toString(36).slice(2,10)}`,
      intentId: intent.id,
      userId: req.user.id,
      amountMinor: intent.amountMinor,
      currency: intent.currency,
