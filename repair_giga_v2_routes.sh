...
cp "$APP" "$BACKUP_DIR/app_pre_repair_v2_${STAMP}.py"
echo "Backup saved: $BACKUP_DIR/app_pre_repair_v2_${STAMP}.py"
    ...
    "district_missions_v2.json": [
        {
            ...
            "lng": -87.6243,
            "pin_type": "mission",
            "media": [{"type": "image", "url": "/static/img/mission-default.jpg", "label": "Mission visual"}]
        }
    ],
...
def giga_data_dir_repair_20260318():
    return giga_Path_repair_20260318.home() / "marketplace" / "data"
...
def giga_api_v2_district_missions_repair_20260318():
    ...
    missions = sorted(
        missions,
        key=lambda m: (
            giga_priority_sort_repair_20260318(m.get("priority")),
            str(m.get("district", "")),
            str(m.get("title", ""))
        )
    )
...
'''
    ...
    print("Routes appended to app.py")
else:
    print("Repair block already present")
PY
...
grep -n "GIGA_V2_ROUTE_REPAIR_20260318\|/api/v2/district-missions\|/api/v2/omni-cinema/episodes\|/api/v2/attached-media" "$APP" || true

echo
echo "Testing new endpoints..."
curl -s http://127.0.0.1:8080/api/v2/giga-phase1-health || true
echo
curl -s http://127.0.0.1:8080/api/v2/district-missions || true
echo
curl -s http://127.0.0.1:8080/api/v2/district-pins || true
echo
curl -s http://127.0.0.1:8080/api/v2/omni-cinema/rows || true
echo
curl -s http://127.0.0.1:8080/api/v2/omni-cinema/episodes || true
echo
curl -s http://127.0.0.1:8080/api/v2/attached-media || true
echo
