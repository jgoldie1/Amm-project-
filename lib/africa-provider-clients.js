...
async function initializePaystack({ email, amountMinor, reference, callbackUrl, metadata }) {
  ...
  const data = await jsonRequest('https://api.paystack.co/transaction/initialize', {
    ...
    body: JSON.stringify({ email, amount: amountMinor, currency: 'NGN', reference, callback_url: callbackUrl, metadata })
  });
...
async function initializeFlutterwave({ email, amountMinor, reference, callbackUrl, metadata }) {
  ...
  const data = await jsonRequest('https://api.flutterwave.com/v3/payments', {
    ...
    body: JSON.stringify({
      ...
      customer: { email },
      meta: metadata,
      customizations: { title: 'TryAMM Nigeria', description: metadata?.purpose || 'TryAMM payment' }
    })
  });
