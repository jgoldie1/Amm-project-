...
function createAssetMarketplaceManager({ manifest, io }) {
  ...
  function createAsset(input = {}) {
    ...
    const asset = { id: crypto.randomUUID(), name, type, ownerId, description: text(input.description, 3000), tags: Array.isArray(input.tags) ? input.tags.slice(0, 50).map(v => text(v, 80)).filter(Boolean) : [], version: '1.0.0', parentAssetId: input.parentAssetId || null, contentHash: text(input.contentHash, 256) || null, sourceRef: text(input.sourceRef, 1000) || null, previewRef: text(input.previewRef, 1000) || null, aiDisclosure: text(input.aiDisclosure, 1000) || null, rights: input.rights || {}, moderationStatus: 'pending', status: 'draft', createdAt: now(), updatedAt: now() };
  ...
  function updateAsset(id, patch = {}) { const asset = getAsset(id); if (!asset) return null; const allowed = ['description','tags','previewRef','sourceRef','contentHash','aiDisclosure','rights','moderationStatus','status','version']; for (const key of allowed) if (patch[key] !== undefined) asset[key] = patch[key]; asset.updatedAt = now(); emit('asset-marketplace:asset', asset); return asset; }
  ...
  function createListing(assetId, input = {}) {
    ...
    const listing = { id: crypto.randomUUID(), assetId, sellerId: asset.ownerId, title: text(input.title, 180) || asset.name, licenseId, priceUsd: Math.round(priceUsd * 100) / 100, currency: 'USD', status: 'active', createdAt: now(), updatedAt: now() };
  ...
  function grantEntitlement(input = {}) { const listing = listings.get(input.listingId); if (!listing) throw new Error('LISTING_NOT_FOUND'); const buyerId = text(input.buyerId,160); if (!buyerId) throw new Error('buyerId required'); const entitlement = { id: crypto.randomUUID(), listingId: listing.id, assetId: listing.assetId, buyerId, licenseId: listing.licenseId, paymentReference: text(input.paymentReference,300), status: 'active', grantedAt: now() }; entitlements.set(entitlement.id, entitlement); emit('asset-marketplace:entitlement', entitlement); return entitlement; }
  function recordUsage(input = {}) { const entitlement = entitlements.get(input.entitlementId); if (!entitlement || entitlement.status !== 'active') throw new Error('ACTIVE_ENTITLEMENT_REQUIRED'); const event = { id: crypto.randomUUID(), entitlementId: entitlement.id, assetId: entitlement.assetId, projectId: text(input.projectId,160), surface: text(input.surface,120), version: text(input.version,80), at: now() }; usage.unshift(event); if (usage.length > 10000) usage.length = 10000; return event; }
  function report() { return { assets: assets.size, publishedAssets: listAssets().filter(x => x.status === 'published').length, activeListings: listListings().length, activeEntitlements: Array.from(entitlements.values()).filter(x => x.status === 'active').length, usageEvents: usage.length }; }
