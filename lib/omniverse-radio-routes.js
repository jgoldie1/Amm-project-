...
module.exports = function registerOmniverseRadioRoutes({ app, auth, clean, id, getStore, saveStore, io }) {
  ...
  app.get('/api/radio/stations', (_req, res) => {
    res.json({ stations: store.radioStations.filter((s) => s.status === 'live' || s.status === 'scheduled') });
  });
  ...
  app.post('/api/music/tracks/:trackId/license', auth, async (req, res) => {
    ...
    const permissions = ['radio','streetverse','vehicle-radio','business-radio','live-dj','reels','games','movies','ads','podcasts'].filter((p) => Array.isArray(req.body.permissions) && req.body.permissions.includes(p));
  ...
  app.post('/api/radio/stations/:stationId/play', auth, async (req, res) => {
    ...
    const event = { id: id('rplay'), stationId: station.id, trackId: track.id, artistName: track.artistName, title: track.title, isrc: clean(track.isrc, 30), licenseId: directLicense?.id || null, territory: clean(req.body.territory, 10) || 'US', listeners: Math.max(0, Number(req.body.listeners || 0)), startedAt: new Date().toISOString() };
  ...
  app.post('/api/radio/ads', auth, async (req, res) => {
    ...
    const ad = { id: id('radad'), buyerId: req.user.id, businessName: clean(req.body.businessName, 120), creativeUrl: clean(req.body.creativeUrl, 500), durationSeconds: [15,30,60].includes(Number(req.body.durationSeconds)) ? Number(req.body.durationSeconds) : 30, amountCents, status: 'pending-payment-and-review', impressions: 0, createdAt: new Date().toISOString() };
