...
module.exports = function registerFamilyEnterpriseRoutes({ app, auth, admin, clean, id, getStore, saveStore }) {
  ...
  app.get('/api/family-enterprise/overview', adminChain, (_req, res) => {
    const data = store();
    res.json({
      members: data.members,
      enterprises: data.enterprises,
      pathways: data.pathways,
      opportunityFunds: data.opportunityFunds,
      ...
      guardrails: {
        ...
        managementAuthorityIsSeparate: true,
        educationAccessIsSeparate: true,
        ipRoyaltyRightsAreSeparate: true,
        regulatedCareersRequireExternalCredentials: true,
        regulatedFinancialActivityRequiresCompliantPartnersOrLicensing: true,
        successionEntriesArePlanningOnlyUntilLegallyExecuted: true
      }
    });
  });
  ...
  app.post('/api/family-enterprise/council', adminChain, async (req, res) => {
    const data = store();
    const memberId = clean(req.body.memberId, 100);
    const familyMember = data.members.find(item => item.id === memberId);
    if (!familyMember) return res.status(404).json({ error: 'Family member not found' });
    if (data.council.some(item => item.memberId === memberId && item.status === 'active')) return res.status(409).json({ error: 'Family member is already active on the council' });
    const councilSeat = {
      id: id('council'),
      memberId,
      displayName: familyMember.displayName,
      role: clean(req.body.role, 100) || 'family council member',
      advisoryOnly: req.body.advisoryOnly !== false,
      status: 'active',
      createdAt: new Date().toISOString()
    };
    data.council.push(councilSeat);
    await saveStore();
    res.status(201).json({ councilSeat });
  });
};
