...
function createHolo5dxManager({ io } = {}) {
  ...
  function createCalibration(input = {}) {
    return calibrateQuantumCone(input);
  }

  function createRenderPlan(input = {}) {
    const mode = input.mode || 'standard-3d';
    const calibration = input.calibration || input;
    return {
      opticalProfile: opticalProfile.product || opticalProfile.name || 'Holo5DX Quantum Cone Lens',
      ...planOpticalOutput({ mode, calibration })
    };
  }
