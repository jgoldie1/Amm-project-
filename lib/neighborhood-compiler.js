...
const DEFAULT_LAYERS = [
  'BOUNDARY','ROADS','SIDEWALKS','BUILDINGS','LANDMARKS','TRANSIT','BUSINESSES','PROPERTIES',
  'NPC_POPULATION','TRAFFIC','MISSIONS','CREATOR_LOCATIONS','MARKETPLACE','JOBS','DAY_NIGHT',
  'ACCESSIBILITY','ECONOMY','STREAMING_LOD'
];
...
const SOURCE_POLICY = Object.freeze({
  ...
  googleMaps:{
    ...
    allowed:['interactive basemap or panorama shown to the user','manual visual QA by authorized editors','orientation, wayfinding and location context','Street View panorama handoff with required attribution','overlay of independently-created TRYAMM objects where provider policy permits'],
...
function buildStreetViewReference(area,options={}){
  ...
  return {
    ...
    mode:'VISUAL_REFERENCE',provider:'GOOGLE_MAPS_PLATFORM',product:'STREET_VIEW',areaId:normalized.id,areaName:normalized.name,
    panoramaSearch:{strategy:'ON_DEMAND_NEAR_SELECTED_LOCATION',persistOnly:['panoId','lat','lng','heading','date','copyright','reportProblemLink']},
    editorUses:['confirm street character and orientation','visually review entrances and storefront context','compare generated TRYAMM scene against public-facing context','flag generated-scene mismatches for human correction'],
    compilerInput:false,machineInterpretation:false,deriveGeometry:false,attributionRequired:true,reportProblemLinkRequired:true
  };
}
...
function buildNeighborhoodSlice(area,options={}){
  const normalized=normalizeArea(area);
  return {
    schemaVersion:2,compiler:'TRYAMM_NEIGHBORHOOD_COMPILER',city:'Chicago',communityArea:normalized,
    target:{status:options.targetStatus||'PLAYABLE',quality:options.quality||'MOBILE_FIRST',mobileFirst:true,oneHandedControls:true,reducedMotionFallback:true},
    ...
    compile:{
      ...
      geometry:{roads:'generate navigable road graph from permitted vector data',sidewalks:'derive playable sidewalk graph from permitted geometry and procedural rules',buildings:'generate TRYAMM-owned building meshes from permitted footprints + procedural facade kits',interiors:'lazy-load only for mission/property/business interiors',landmarks:'use original or properly licensed assets only'},
      simulation:{npcPopulationBudget:Number.isInteger(options.populationBudget)?options.populationBudget:48,trafficVehicleBudget:Number.isInteger(options.trafficBudget)?options.trafficBudget:24,pedestrianLod:true,vehicleLod:true,dayNight:true,weatherHooks:true},
      economy:{businessPassport:true,marketplace:true,delivery:true,middleverseJobs:true,creatorCommerce:true,propertyHooks:true,authoritativeRewards:true,clientCashAwards:false},
      gameplay:{missionAnchors:['TRANSIT','BUSINESS','CREATOR','DELIVERY','PROPERTY','COMMUNITY','SPORTS'],creatorCapture:true,reelHandoff:true,raceHooks:true,mobilityHooks:true}
    },
