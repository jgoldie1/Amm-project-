'use strict';
const {negotiateHolographicSession}=require('./holographic-runtime');
...
module.exports=function installHolographicRuntime({app,auth,io,clean,id,getStore,saveStore}){
  ...
  app.post('/api/holo/runtime/session',auth,async(req,res)=>{
    try{
      const plan=negotiateHolographicSession({
        user:{userId:req.user.id,avatarId:req.body.avatarId,language:req.body.language},
        accessibility:{captions:req.body.captions,audioDescription:req.body.audioDescription,reducedMotion:req.body.reducedMotion,highContrast:req.body.highContrast,voiceControl:req.body.voiceControl},
        device:req.body.device||{},network:req.body.network||{},
        presenceCapture:req.body.presenceCapture||null,
        audioObjects:req.body.audioObjects||[],audioOutput:req.body.audioOutput||{},room:req.body.room||{},listener:req.body.listener||{},
        scene:{worldUri:req.body.worldUri||req.body.world?.uri||'tryamm://home',sceneId:req.body.sceneId||req.body.world?.sceneId||'home'},
        permissions:{role:req.body.role||'member',scopes:req.body.permissionScopes||[]},preferences:req.body.preferences||{}
      });
  ...
  io.on('connection',socket=>{
    socket.on('holo:join',({sessionId})=>{
      ...
      socket.emit('holo:state',{session:s});
      socket.to(`holo:${sessionId}`).emit('holo:presence',{userId:socket.user.id,status:'joined'});
    });

    socket.on('holo:media-ready',payload=>{
      const sessionId=socket.data.holoSessionId;if(!sessionId)return socket.emit('holo:error',{error:'join_session_first'});
      const room=io.sockets.adapter.rooms.get(`holo:${sessionId}`)||new Set();
      for(const socketId of room){if(socketId!==socket.id)socket.emit('holo:peer',{socketId,status:'joined',media:{audio:payload?.audio!==false,video:payload?.video!==false}});}
      socket.to(`holo:${sessionId}`).emit('holo:peer',{socketId:socket.id,status:'joined',media:{audio:payload?.audio!==false,video:payload?.video!==false}});
    });
    ...
    socket.on('holo:signal',({to,data})=>{
      const sessionId=socket.data.holoSessionId;
      if(!sessionId||!to||!data)return socket.emit('holo:error',{error:'invalid_signal'});
      const target=io.sockets.sockets.get(to);
      if(!target||target.data.holoSessionId !== sessionId)return socket.emit('holo:error',{error:'peer_not_in_session'});
      target.emit('holo:signal',{from:socket.id,data});
    });
    ...
    socket.on('disconnect',()=>{
      const sessionId=socket.data.holoSessionId;
      if(sessionId)socket.to(`holo:${sessionId}`).emit('holo:peer',{socketId:socket.id,status:'left'});
    });
  });
};
