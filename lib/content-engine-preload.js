...
Module._load = function tryammLoad(request, parent, isMain) {
  ...
  if (request === './music-api' && typeof loaded === 'function' && parent?.filename?.endsWith('server.js')) {
    return function registerTryammModules(args) {
      ...
      require('./ai-factory-routes')({ app: args.app, auth: args.auth });
      require('./ai-training-routes')({ ...args, admin: safeAdmin });
      require('./poyo-routes')({ app: args.app, auth: args.auth });
      require('./release-flow-routes')({ app: args.app, auth: args.auth, getStore: args.getStore });
      try {
        require.resolve('./global-mobility-routes');
        require('./global-mobility-routes')({ app: args.app, auth: args.auth, clean: args.clean, id: args.id, getStore: args.getStore, saveStore: args.saveStore });
      } catch (error) {
        if (error?.code !== 'MODULE_NOT_FOUND') throw error;
      }
      require('./streetverse-competition-routes')({ app: args.app, auth: args.auth });
    };
  }
  return loaded;
};