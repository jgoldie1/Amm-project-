...
const { summarizeWorld } = require('./world-compiler');
const { velocitySummary, nextCriticalPath } = require('./velocity-planner');
...
module.exports = function registerContentEngine({ app, auth, admin, clean, id, getStore, saveStore }) {
  ...
  app.get('/api/founder/dashboard', auth, async (req, res, next) => {
    try {
      ...
      res.json({
        metrics,
        infrastructure: { supabase: supabase.configured(), render: Boolean(process.env.RENDER), replit: Boolean(process.env.REPL_ID), appUrl: process.env.APP_URL || null },
        world,
        velocity: { ...velocity, nextCriticalPath: nextCriticalPath([]) }
      });
    } catch (error) { next(error); }
  });
};
