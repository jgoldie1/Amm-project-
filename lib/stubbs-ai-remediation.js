...
function evaluateArea(areaName,probeResults=[]){
  ...
  const score=normalize(avg(scored.map(r=>normalize(r.score))));
  const evidenceMissing=scored.filter(r=>!Array.isArray(r.evidenceIds)||r.evidenceIds.length===0).map(r=>r.probe);
  const failures=scored.filter(r=>r.status!=='pass'||normalize(r.score)<spec.target).map(r=>r.probe);
...
function evaluateQualification(allResults={}){
  ...
  const overall=normalize(avg(areas.map(a=>a.score)));
  return {
    overall,
    areas,
