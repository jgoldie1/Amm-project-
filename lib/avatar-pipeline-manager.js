...
function createAvatarPipelineManager({ policy, assetPipeline, io }) {
  ...
  function createDerivedAssetJob(id) {
    const job = get(id);
    if (!job) return null;
    if (!assetPipeline?.createAssetJob) throw new Error("ASSET_PIPELINE_UNAVAILABLE");

    const derived = assetPipeline.createAssetJob({
      gameId: job.gameId,
      assetType: "character",
      name: job.displayName,
      brief: `Respectful user-authorized avatar derived from multi-view scan. Stylization: ${job.stylization}`,
      requestedBy: "avatar-scan-pipeline",
    });

    if (derived && Array.isArray(derived.provenance)) {
      derived.provenance.push({
        source: "avatar-scan",
        avatarJobId: job.id,
