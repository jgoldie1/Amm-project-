...
const {raceProviders,healthSnapshot}=require('./hologpt-race');
...
function providerCandidates(){
  ...
  if(COMPAT_URL&&COMPAT_MODEL)providers.push({name:'openai-compatible',local:true,run:callCompatible});
  if(OLLAMA_BASE&&OLLAMA_MODEL)providers.push({name:'ollama',local:true,run:callOllama});
  if(EXECUTIVE_URL)providers.push({name:'stubbs-gateway',priority:.08,run:callStubbsGateway});
  return providers;
}
...
function status(){
  ...
  return {
    ...
    orchestration:'hedged-model-race',
...
async function runProviders(message,memory,context){
  ...
  if(providers.length){
    try{
      return await raceProviders({
        message,memory,context,providers,
        hedgeDelayMs:Math.max(250,Math.min(1800,Number(process.env.HOLOGPT_HEDGE_DELAY_MS||550))),
        width:Math.max(2,Math.min(4,Number(process.env.HOLOGPT_RACE_WIDTH||3)))
      });
    }catch(error){
      const failures=Array.isArray(error?.attempts)?error.attempts.filter(item=>!item.ok).map(item=>({
        provider:item.provider,status:item.status||null,message:String(item.error||'provider failed').slice(0,500)
      })):[];
  ...
  return {
    ...
    providerErrors:[],
    providerError:null,
    orchestration:{mode:'local-degraded',intent:'general',attempts:[],progress:['UNDERSTANDING','LOCAL_RECOVERY','COMPLETE']}
  };
}
