...
function status(){
  const provider=AI_GATEWAY_AUTH?'vercel-ai-gateway':OPENAI_KEY?'openai-responses':COMPAT_URL&&COMPAT_MODEL?'openai-compatible':OLLAMA_BASE&&OLLAMA_MODEL?'ollama':EXECUTIVE_URL?'stubbs-gateway':'local-degraded';
  return {
    ok:true,
    provider,
...
async function chat({userId,message,context={}}={}){
  ...
  let result;
  try{
    result=AI_GATEWAY_AUTH?await callVercelGateway(clean,memory,context):OPENAI_KEY?await callOpenAI(clean,memory,context):COMPAT_URL&&COMPAT_MODEL?await callCompatible(clean,memory,context):OLLAMA_BASE&&OLLAMA_MODEL?await callOllama(clean,memory,context):EXECUTIVE_URL?await callStubbsGateway(clean,memory,context):{...localFallback(clean,memory,context),provider:'local-degraded',model:null,responseId:null};
  }catch(error){
    try{
      if(AI_GATEWAY_AUTH&&OPENAI_KEY){result=await callOpenAI(clean,memory,context);result.providerError=`Gateway failed: ${error.message}`;}
      else throw error;
    }catch(finalError){
      const fallback=localFallback(clean,memory,context);
      result={answer:`HoloGPT provider error: ${finalError.message}. I switched to diagnostic mode.\n\n${fallback.answer}`,provider:'provider-error-fallback',model:null,responseId:null,providerError:finalError.message,promptVersion:fallback.promptVersion,promptMode:fallback.promptMode,injectionRisk:fallback.injectionRisk};
    }
  }
