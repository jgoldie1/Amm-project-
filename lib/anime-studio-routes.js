...
const { writeProjectPackage } = require('./anime-studio-package');
  ...
  app.post('/api/anime-studio/projects/:id/generate', (req, res) => {
    ...
    setImmediate(async () => {
      try {
        ...
        const downloadPackage = await writeProjectPackage({ project, plan, outputs, manifest });
        job.outputs = { ...outputs, manifest, downloadPackage, productionPlan: plan, provider };
        job.progress = 100;
        job.stage = 'completed';
