...
const STREETVERSE_ALPHA_MISSION = Object.freeze({
  clientMissionId: 'district-01-mobile-safe',
  programMissionId: 'm1',
  characterId: 'founder-alpha-mobile',
  beatId: 'chicago-district-01-checkpoints',
  source: 'streetverse-mobile-safe',
  requiredCheckpoints: Object.freeze(['studio', 'market', 'river', 'stage'])
});
...
function completedMissionStatus(status) {
  return status === 'complete' || status === 'completed';
}

function normalizeVisited(value) {
  if (!Array.isArray(value)) return [];
  const allowed = new Set(STREETVERSE_ALPHA_MISSION.requiredCheckpoints);
  return [...new Set(value.map(item => clean(item, 40)).filter(item => allowed.has(item)))];
}

function validateStreetVerseAlphaCompletion(body = {}) {
  const missionId = clean(body.missionId || body.id, 100);
  const source = clean(body.source, 100);
  const visited = normalizeVisited(body.visited);
  const required = STREETVERSE_ALPHA_MISSION.requiredCheckpoints;
  if (missionId !== STREETVERSE_ALPHA_MISSION.clientMissionId) return { ok: false, reason: 'MISSION_ID_NOT_ALLOWED' };
  if (source !== STREETVERSE_ALPHA_MISSION.source) return { ok: false, reason: 'MISSION_SOURCE_NOT_ALLOWED' };
  if (!required.every(id => visited.includes(id))) return { ok: false, reason: 'MISSION_CHECKPOINTS_INCOMPLETE' };
  if (Number(body.total || required.length) !== required.length) return { ok: false, reason: 'MISSION_TOTAL_INVALID' };
...
module.exports = function registerGetPaidToPlayRoutes({ app, getStore, saveStore }) {
  ...
  async function verifyEvidence(program, userId, body) {
    if (program.evidence === 'streetverse_mission_run') {
      ...
      if (run.mission_id !== program.missionId || !completedMissionStatus(run.status)) return { ok: false, reason: 'MISSION_NOT_COMPLETED' };
      const runtime = run.runtime_state || {};
      const visited = Array.isArray(runtime.visited) ? runtime.visited : [];
      const required = STREETVERSE_ALPHA_MISSION.requiredCheckpoints;
      if (!required.every(id => visited.includes(id)) || Number(runtime.progress || 0) < 100) return { ok: false, reason: 'MISSION_PROOF_INCOMPLETE' };
  ...
  async function applyGameStateReward(userId, reward, reference) {
    ...
    const updated = await supabase.update('player_state', `user_id=eq.${encodeURIComponent(userId)}&revision=eq.${encodeURIComponent(Number(state.revision || 0))}`, {
      xp: nextXp,
      tokens: nextTokens,
      level: nextLevel,
    ...
    if (!next) throw Object.assign(new Error('Player state changed while reward was being applied; retry safely'), { status: 409, code: 'PLAYER_STATE_CONFLICT' });
    return next;
  }
  ...
  app.post('/api/get-paid-to-play/streetverse/complete', rewardAuth, async (req, res, next) => {
    try {
      if (!supabase.configured()) return res.status(503).json({ error: 'Durable mission state is not configured' });
      const proof = validateStreetVerseAlphaCompletion(req.body || {});
      if (!proof.ok) return res.status(422).json({ error: 'Mission completion evidence was not accepted', code: proof.reason });
      const userId = await resolveUserId(req.user);
      const spec = STREETVERSE_ALPHA_MISSION;
      const query = `user_id=eq.${encodeURIComponent(userId)}&character_id=eq.${encodeURIComponent(spec.characterId)}&mission_id=eq.${encodeURIComponent(spec.programMissionId)}&select=*`;
      const existingRows = await supabase.select('streetverse_mission_runs', query);
      const existing = Array.isArray(existingRows) ? existingRows[0] : null;
      const runtimeState = {
        visited: proof.visited,
        progress: 100,
        total: spec.requiredCheckpoints.length,
        source: spec.source,
        clientMissionId: spec.clientMissionId,
        mobileSafeMode: Boolean(req.body?.mobileSafeMode),
        htmlCity: Boolean(req.body?.htmlCity),
        evidenceQuality: 'server-validated-client-telemetry-alpha',
        completedAt: nowIso()
      };
      const row = {
        user_id: userId,
        character_id: spec.characterId,
        mission_id: spec.programMissionId,
        beat_id: spec.beatId,
        status: 'complete',
        choice: { vehicle: Boolean(req.body?.vehicle), clientMissionId: spec.clientMissionId },
        runtime_state: runtimeState,
        updated_at: nowIso()
      };
      let missionRun;
      if (existing) {
        const updated = await supabase.update('streetverse_mission_runs', `id=eq.${encodeURIComponent(existing.id)}&user_id=eq.${encodeURIComponent(userId)}`, row);
        missionRun = Array.isArray(updated) ? updated[0] : updated;
      } else {
        const inserted = await supabase.insert('streetverse_mission_runs', row);
        missionRun = Array.isArray(inserted) ? inserted[0] : inserted;
      }
      if (!missionRun?.id) throw Object.assign(new Error('Authoritative mission run could not be recorded'), { status: 503 });
      return res.status(existing ? 200 : 201).json({
        ok: true,
        applied: !existing,
        missionRun,
        programId: 'streetverse_first_drop',
        claimReady: true,
  ...
  app.post('/api/get-paid-to-play/claim', rewardAuth, async (req, res, next) => {
    try {
      ...
      const durableRows = await supabase.select('game_reward_claims', `idempotency_key=eq.${encodeURIComponent(idempotencyKey)}&select=*`);
      const durableExisting = Array.isArray(durableRows) ? durableRows[0] : null;
      if (durableExisting) {
        return res.json({ ok: true, applied: false, reason: durableExisting.status === 'verified' ? 'ALREADY_CLAIMED' : 'CLAIM_RECONCILIATION_REQUIRED', claim: durableExisting });
      }
      ...
      const durableCashStatus = cashStatus.replaceAll('-', '_');
      const durableInsert = await supabase.insert('game_reward_claims', {
        id: claimId,
        user_id: userId,
        program_id: program.id,
        evidence_type: proof.evidenceType,
        evidence_ref: proof.evidenceRef,
        idempotency_key: idempotencyKey,
        status: 'submitted',
        xp_awarded: gameReward.xp,
        holo_credits_awarded: gameReward.holoCredits,
        cash_minor: cashCents,
        cash_status: durableCashStatus,
        server_determined: true,
        evidence_snapshot: { game: requestedGame || program.game || '', proof, source: 'get-paid-to-play-v2' }
      });
      const durableClaim = Array.isArray(durableInsert) ? durableInsert[0] : durableInsert;
      if (!durableClaim?.id) throw Object.assign(new Error('Durable reward claim could not be reserved'), { status: 503 });

      let playerState;
      try {
        playerState = await applyGameStateReward(userId, gameReward, claimId);
      } catch (error) {
        await supabase.update('game_reward_claims', `id=eq.${encodeURIComponent(claimId)}`, { status: 'rejected', verified_at: nowIso(), evidence_snapshot: { game: requestedGame || program.game || '', proof, source: 'get-paid-to-play-v2', failure: clean(error?.code || error?.message, 120) } });
        throw error;
      }

      const verifiedRows = await supabase.update('game_reward_claims', `id=eq.${encodeURIComponent(claimId)}&status=eq.submitted`, { status: 'verified', verified_at: nowIso() });
      const verifiedDurableClaim = Array.isArray(verifiedRows) ? verifiedRows[0] : verifiedRows;
      if (!verifiedDurableClaim?.id) throw Object.assign(new Error('Reward was applied but durable claim verification needs reconciliation'), { status: 503, code: 'CLAIM_RECONCILIATION_REQUIRED' });
      ...
      await saveStore();
      res.status(201).json({ ok: true, applied: true, claim, durableClaim: verifiedDurableClaim, playerState, notice: cashCents > 0 ? 'Cash earning is pending payout eligibility and is not yet withdrawable.' : 'Gameplay reward issued as XP/Holo Credits only.' });
    } catch (error) { next(error); }
  });

  app.get('/api/get-paid-to-play/player-state', rewardAuth, async (req, res, next) => {
    try {
      if (!supabase.configured()) return res.status(503).json({ error: 'Durable player state is not configured' });
      const userId = await resolveUserId(req.user);
      const rows = await supabase.select('player_state', `user_id=eq.${encodeURIComponent(userId)}&select=*`);
      const playerState = Array.isArray(rows) ? rows[0] : null;
      if (!playerState) return res.status(404).json({ error: 'Player state not found' });
      res.json({ ok: true, playerState, serverDetermined: true });
    } catch (error) { next(error); }
  });
  ...
  app.get('/api/get-paid-to-play/history', rewardAuth, async (req, res, next) => {
    try {
      const userId = await resolveUserId(req.user);
      const durableRows = supabase.configured()
        ? await supabase.select('game_reward_claims', `user_id=eq.${encodeURIComponent(userId)}&select=*&order=created_at.desc&limit=100`)
        : [];
      if (Array.isArray(durableRows) && durableRows.length) return res.json({ claims: durableRows, durable: true });
      const store = getStore();
      ensureRewardCollections(store);
      const claims = store.gameRewardClaims.filter(row => row.userId === userId).slice(-100).reverse();
      res.json({ claims, durable: false });
    } catch (error) { next(error); }
  });
};

module.exports.PROGRAMS = PROGRAMS;
module.exports.STREETVERSE_ALPHA_MISSION = STREETVERSE_ALPHA_MISSION;
module.exports.completedMissionStatus = completedMissionStatus;
module.exports.validateStreetVerseAlphaCompletion = validateStreetVerseAlphaCompletion;
