...
module.exports=function registerStubbsAiRoutes({app,auth}){
  ...
  app.post('/api/hologpt/chat',auth,async(req,res,next)=>{
    try{
      ...
      const result=await hologpt.chat({userId:req.user.id,message,context:{page:cleanText(req.body?.page,300),sessionId:cleanText(req.body?.sessionId,200),checkpointId:cleanText(req.body?.checkpointId,200),resume:req.body?.resume===true}});
      res.json(result);
