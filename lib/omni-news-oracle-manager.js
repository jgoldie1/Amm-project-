...
const SCRAPER_PROFILES = new Set([
  'local_headlines',
  'national_headlines',
...
const htmlScrapingEnabled = () => String(process.env.OMNI_NEWS_HTML_SCRAPING_ENABLED || '').toLowerCase() === 'true';
...
function geographyApproved(source,region){
  const normalized=String(region||'').trim().toLowerCase();
  const approved=(source?.geographies||[]).map(value=>String(value||'').trim().toLowerCase()).filter(Boolean);
  return approved.includes('global')||approved.includes('*')||approved.includes(normalized);
}
...
function createOmniNewsOracleManager() {
  ...
  function purgeExpiredItems(now=Date.now()) {
    let purged=0;
    for (const [id,item] of items.entries()) {
      const expiresAt=Date.parse(String(item.expiresAt||''));
      if (Number.isFinite(expiresAt) && expiresAt <= now) { items.delete(id); purged+=1; }
    }
    return purged;
  }
  ...
  function registerSource(input = {}) {
    ...
    const source = {
      ...
      ingestionMode,
      scraperProfile: ingestionMode === 'html_scrape'
        ? (SCRAPER_PROFILES.has(input.scraperProfile) ? input.scraperProfile : 'metadata_only')
        : null,
      ...
      desks: Array.isArray(input.desks) && input.desks.length ? input.desks.filter((desk) => NEWS_DESKS.has(desk)) : ['international_news'],
      ...
      scrapePolicy: ingestionMode === 'html_scrape' ? {
        robotsRequired: true,
        robotsApprovedAt: input.scrapePolicy?.robotsApprovedAt || null,
        paywallBypassProhibited: true,
        authenticationBypassProhibited: true,
        metadataOnlyUnlessLicensed: !Boolean(input.license?.fullContentAllowed),
        maxPagesPerRun: Math.max(1, Math.min(100, Number(input.scrapePolicy?.maxPagesPerRun) || 20)),
        minIntervalSeconds: Math.max(30, Number(input.scrapePolicy?.minIntervalSeconds) || 300),
        allowedPaths: Array.isArray(input.scrapePolicy?.allowedPaths) ? input.scrapePolicy.allowedPaths.slice(0, 20) : []
      } : null,
      license: {
  ...
  function approveSource(id) {
    ...
    if (source.ingestionMode === 'html_scrape' && !htmlScrapingEnabled()) throw new Error('HTML_SCRAPING_GLOBALLY_DISABLED');
    if (source.ingestionMode === 'html_scrape' && !source.license.scrapingAllowed) throw new Error('SCRAPING_PERMISSION_REQUIRED');
    if (source.ingestionMode === 'html_scrape' && source.scrapePolicy?.robotsRequired && !source.scrapePolicy?.robotsApprovedAt) throw new Error('ROBOTS_REVIEW_REQUIRED');
  ...
  function ingest(input = {}) {
    ...
    const desk = input.desk || source.desks[0] || 'international_news';
    ...
    if (!geographyApproved(source,region)) throw new Error('SOURCE_GEOGRAPHY_NOT_APPROVED');
    if (!source.lanes.includes(lane)) throw new Error('SOURCE_LANE_NOT_APPROVED');
    if (!NEWS_DESKS.has(desk) || !source.desks.includes(desk)) throw new Error('SOURCE_DESK_NOT_APPROVED');
    ...
    const item = {
      ...
      createdAt: new Date().toISOString(),
      expiresAt: new Date(Date.now() + Math.max(1, Number(source.retentionHours) || 72) * 36e5).toISOString()
    };
    items.set(id, item);
  ...
  function submitCommunityReport(input = {}) {
    ...
    const submission = {
      ...
      desk: NEWS_DESKS.has(input.desk) ? input.desk : 'community',
      purposes: Array.isArray(input.purposes) ? input.purposes : ['newsroom'],
  ...
  function reviewItem(id, input = {}) {
    ...
    item.live = Boolean(source.live && ['verified','confirmed','official'].includes(item.verificationStatus) && input.publish === true);
  ...
  function listItems(filters = {}) {
    ...
    return [...items.values()].filter((item) =>
      ...
      (!filters.desk || item.desk === filters.desk) &&
