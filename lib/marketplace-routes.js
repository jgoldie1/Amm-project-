...
module.exports=function registerMarketplace({app,auth,admin,clean,id,getStore,saveStore}){
  ...
  app.patch('/api/marketplace/products/:productId',auth,async(req,res)=>{
  ...
  app.post('/api/marketplace/checkout',auth,async(req,res,next)=>{
    try{
      ...
      const quantity=Math.max(1,Math.min(99,Math.floor(Number(req.body.quantity)||1)));if(p.stock!==null&&p.stock<quantity)return res.status(409).json({error:'Insufficient stock'});
      ...
      const order={id:id('ord'),buyerId:req.user.id,merchantId:m.id,productId:p.id,quantity,subtotalCents:subtotal,platformFeeCents:applicationFee,status:'checkout_created',stripeSessionId:null,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
      const checkout=await stripe.checkout.sessions.create({mode:'payment',customer_email:req.user.email,line_items:[{quantity,price_data:{currency:p.currency,unit_amount:p.priceCents,product_data:{name:p.name,description:p.description||undefined}}}],payment_intent_data:{application_fee_amount:applicationFee,transfer_data:{destination:m.stripeAccountId}},metadata:{orderId:order.id,merchantId:m.id,productId:p.id,buyerId:req.user.id},success_url:`${appUrl}/?marketplace=success&session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${appUrl}/?marketplace=cancelled`});
      order.stripeSessionId=checkout.id;s.marketplaceOrders.push(order);await saveStore();res.status(201).json({order:{...order,stripeSessionId:undefined},url:checkout.url});
    }catch(e){next(e)}
  });
