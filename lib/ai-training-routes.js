...
function evaluationPass(metrics={}){
  const safety=Number(metrics.safetyScore);
  const quality=Number(metrics.qualityScore);
  const hallucination=Number(metrics.hallucinationRate);
...
module.exports=function registerAiTrainingRoutes({app,auth,admin,getStore,saveStore,id}){
  ...
  app.post('/api/ai-training/datasets',auth,requireAdmin,async(req,res)=>{
    const name=clean(req.body?.name,120),source=clean(req.body?.source,240),purpose=clean(req.body?.purpose,240);
  ...
  app.post('/api/ai-training/jobs/:jobId/dispatch',auth,requireAdmin,async(req,res,next)=>{
    ...
    try{
      const response=await fetch(process.env.SOUP_TRAIN_API_URL,{method:'POST',headers:{'content-type':'application/json',...(process.env.SOUP_TRAIN_API_KEY?{'authorization':`Bearer ${process.env.SOUP_TRAIN_API_KEY}`}:{})},body:JSON.stringify({jobId:job.id,datasetId:job.datasetId,baseModel:job.baseModel,method:job.method,hyperparameters:job.hyperparameters})});
      if(!response.ok)return res.status(502).json({error:'Soup trainer rejected dispatch',status:response.status});
      const result=await response.json().catch(()=>({}));job.status='DISPATCHED';job.externalJobId=clean(result.id||result.jobId,180)||null;job.updatedAt=now();await saveStore();res.json({ok:true,job});
  ...
  app.post('/api/ai-training/models/:modelId/deploy',auth,requireAdmin,async(req,res)=>{
    ...
    if(!target)return res.status(400).json({error:'Deployment target is required'});
