...
function buildCameraRig({viewCount=4,viewingConeDeg=90,viewerDistanceMm=500,opticalCenterOffsetMm={x:0,y:0,z:0},fovDeg=45}={}){
  return buildViewSectors({viewCount,viewingConeDeg,startDeg:-viewingConeDeg/2}).map(v=>{
    const a=degToRad(v.azimuthDeg);
    const d=Math.max(50,Number(viewerDistanceMm)||500);
...
function buildPackingPlan({viewCount=4,resolutionPx={width:1920,height:1080},layout='grid'}={}){
  ...
  const w=Math.max(1,Math.floor(Number(resolutionPx.width)||1920));
  const h=Math.max(1,Math.floor(Number(resolutionPx.height)||1080));
...
function calibrateQuantumCone(input={}){
  ...
  if(geometry.viewCount>32) warnings.push('High view count may exceed mobile/browser performance budgets.');
  if(geometry.coneAngleDeg<=10||geometry.coneAngleDeg>=80) warnings.push('Extreme cone angle likely requires optical validation.');
  return {geometry:{...geometry,slantMm,footprintRadiusMm},cameraRig:buildCameraRig(geometry),packing:buildPackingPlan({viewCount:geometry.viewCount,resolutionPx:geometry.resolutionPx,layout:input.layout||'grid'}),warpModel:{type:'calibration-placeholder',requiresMeasuredHomography:true,notes:'Use photographed calibration targets or optical bench measurements to derive per-view homography/distortion matrices.'},validationTargets:['checkerboard alignment','cross-view crosstalk','brightness uniformity','gamma consistency','optical center error','viewer sweet-spot width','ghost-image level'],warnings};
}
