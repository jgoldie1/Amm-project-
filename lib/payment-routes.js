...
const SUPPORTED_CREATOR_KINDS = new Set(['live-gift','live-ticket','live-subscription','pk-gift','pk-prize','debate-gift','debate-ticket','debate-sponsor','debate-prize','family-creator-earning','agency-creator-earning']);

function buildVerifiedSettlementInput({ session, eventId }) {
  const m=session.metadata||{}; const kind=String(m.kind||'live-ticket');
  if(!SUPPORTED_CREATOR_KINDS.has(kind)) return null;
  const creatorId=String(m.creatorId||'').trim(); const amountMinor=cents(session.amount_total);
  if(!creatorId||!amountMinor||!eventId) throw Object.assign(new Error('Verified creator settlement evidence is incomplete'),{status:422});
  const platformShareBps=Number(m.platformShareBps ?? PLATFORM_FEE_BPS);
  if(!Number.isInteger(platformShareBps)||platformShareBps<0||platformShareBps>10000) throw Object.assign(new Error('Invalid platform settlement split'),{status:422});
  return {id:`settlement_${session.id}`,kind,creatorId,amountMinor,currency:String(session.currency||'usd').toUpperCase(),providerVerified:true,providerEventId:eventId,roomId:String(m.roomId||''),platformShareBps};
}

function applyVerifiedCreatorSettlement({store,session,eventId,now}){
 const input=buildVerifiedSettlementInput({session,eventId}); if(!input)return null;
 store.settlements=Array.isArray(store.settlements)?store.settlements:[];
 const prior=store.settlements.find(x=>x.providerEventId===eventId); if(prior)return prior;
 const creator=(store.users||[]).find(u=>u.id===input.creatorId); if(!creator)throw Object.assign(new Error('Settlement creator account not found'),{status:422});
 const platform=Math.floor(input.amountMinor*input.platformShareBps/10000),net=input.amountMinor-platform;
 const settlement={...input,payable:true,platformMinor:platform,creatorMinor:net,status:'verified',createdAt:now}; store.settlements.push(settlement); return settlement;
}
...
module.exports = function registerPaymentRoutes({ app, auth, getStore, saveStore }) {
  ...
  app.post('/api/stripe/webhook', async (req, res, next) => {
    try {
      ...
      const settlement=applyVerifiedCreatorSettlement({store,session,eventId:event.id,now:new Date().toISOString()});
      const result = applyPaidCheckout({ store, session, eventId: event.id });
      if (result.applied || settlement) await saveStore();
...
module.exports.buildVerifiedSettlementInput = buildVerifiedSettlementInput;
module.exports.applyVerifiedCreatorSettlement = applyVerifiedCreatorSettlement;
