...
const intelligence=require('./quantum-intelligence');
...
function localResults(q,getStore){const store=getStore(),items=[],text=q.toLowerCase();for(const[collection,type]of[['publications','tryamm'],['networkPosts','creator'],['teamMembers','tryamm']])for(const item of store[collection]||[]){const hay=JSON.stringify(item).toLowerCase();if(hay.includes(text))items.push({id:item.id,title:item.title||item.name||item.body?.slice(0,90)||collection,summary:item.abstract||item.bio||item.body||'',url:item.documentUrl||'/tryamm-hub.html',host:'tryamm.online',sourceType:type,sourceLabel:SOURCE_LABELS[type],verified:type==='tryamm',score:65});}return items;}
function discoveryResults(q){return[
...
async function gather(q,getStore,limit=20){const items=[...localResults(q,getStore),...discoveryResults(q),...await academicSearch(q,8)];const mixed=diversify(items,limit);mixed.results=intelligence.semanticRank(q,mixed.results);return mixed;}

module.exports=function registerQuantumInternet({app,auth,getStore}){
  app.get('/api/quantum-internet/about',(_req,res)=>res.json({name:'TRYAMM Quantum Internet',assistant:'HoloGPT',status:'research preview',principles:['source provenance','filter-bubble resistance','academic evidence','small-web discovery','historical comparison','community context','semantic ranking','contradiction visibility','entity graph','user control','privacy by default'],sourceLabels:SOURCE_LABELS,smallWebSignals:SMALL_WEB,mailPublicName:'TRYAMM Mail',mailInternalCodename:'Quantum Mail'}));
  app.get('/api/quantum-internet/search',async(req,res)=>{try{
    const q=norm(req.query.q,300);if(!q)return res.status(400).json({error:'Search query required'});
    const limit=Math.min(Math.max(Number(req.query.limit)||20,5),50),mixed=await gather(q,getStore,limit),evaluation=intelligence.evaluateResultSet(mixed.results);
    res.json({query:q,...mixed,evaluation,filterBubbleBreaker:{enabled:true,personalizationDefault:'low',domainCap:2,lanes:['TRYAMM','academic','small web','community','archive','live web'],note:'Ranking intentionally reserves room for independent, academic, community and dissenting sources rather than optimizing only for engagement.'}});
  }catch(e){res.status(502).json({error:'Search aggregation failed'});}});
  app.get('/api/quantum-internet/research',async(req,res)=>{try{const q=norm(req.query.q,300);if(!q)return res.status(400).json({error:'Research query required'});const mixed=await gather(q,getStore,30),map=intelligence.evidenceMap(q,mixed.results),contradictions=intelligence.contradictionSignals(map.ranked),graph=intelligence.entityGraph(map.ranked),plan=intelligence.synthesisPlan(q,map);res.json({query:q,evidence:map,contradictions,graph,synthesisPlan:plan,evaluation:intelligence.evaluateResultSet(map.ranked),note:'This endpoint prepares evidence for HoloGPT synthesis. It does not claim that heuristic contradiction signals establish truth or falsity.'});}catch(e){res.status(502).json({error:'Research synthesis preparation failed'});}});
  app.get('/api/quantum-internet/graph',async(req,res)=>{try{const q=norm(req.query.q,300);if(!q)return res.status(400).json({error:'Graph query required'});const mixed=await gather(q,getStore,30);res.json({query:q,...intelligence.entityGraph(mixed.results)});}catch(e){res.status(502).json({error:'Entity graph failed'});}});
  app.get('/api/quantum-internet/evaluate',async(req,res)=>{try{const q=norm(req.query.q,300);if(!q)return res.status(400).json({error:'Evaluation query required'});const mixed=await gather(q,getStore,30);res.json({query:q,metrics:intelligence.evaluateResultSet(mixed.results),principle:'TRYAMM should earn claims through benchmarked relevance, freshness, diversity, provenance accuracy, latency and user control—not slogans.'});}catch(e){res.status(502).json({error:'Evaluation failed'});}});
  app.get('/api/quantum-internet/academic',async(req,res)=>{try{const q=norm(req.query.q,300);if(!q)return res.status(400).json({error:'Academic search query required'});const results=intelligence.semanticRank(q,await academicSearch(q,Math.min(Number(req.query.limit)||20,25)));res.json({query:q,results,source:'Crossref metadata',sourceLabel:SOURCE_LABELS.academic,note:'Crossref returns scholarly metadata. HoloGPT should distinguish peer-reviewed articles, proceedings, books and preprints when metadata permits, and should not treat citation count as proof of correctness.'});}catch(e){res.status(502).json({error:'Academic search failed'});}});
