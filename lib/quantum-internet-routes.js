...
const SOURCE_LABELS={tryamm:'TRYAMM VERIFIED',archive:'ARCHIVED',reddit:'COMMUNITY SOURCE',smallweb:'SMALL WEB',external:'LIVE WEB',creator:'CREATOR SOURCE',business:'BUSINESS SOURCE',ai:'AI INFERENCE'};
...
function diversify(items,limit=20){
  ...
  const pools={mainstream:0,smallweb:0,community:0,archive:0,tryamm:0};
  for(const item of items.map(x=>({...x,rank:score(x)})).sort((a,b)=>b.rank-a.rank)){
    ...
    const lane=item.sourceType==='tryamm'?'tryamm':item.sourceType==='smallweb'?'smallweb':item.sourceType==='reddit'?'community':item.sourceType==='archive'?'archive':'mainstream';
...
module.exports=function registerQuantumInternet({app,auth,getStore}){
  app.get('/api/quantum-internet/about',(_req,res)=>res.json({name:'TRYAMM Quantum Internet',assistant:'HoloGPT',principles:['source provenance','filter-bubble resistance','small-web discovery','historical comparison','community context','user control','privacy by default'],sourceLabels:SOURCE_LABELS,smallWebSignals:SMALL_WEB,mailPublicName:'TRYAMM Mail',mailInternalCodename:'Quantum Mail'}));
  app.get('/api/quantum-internet/search',async(req,res)=>{
    ...
    // External engines are adapter-based. Do not scrape services that require approved API access.
    const discovery=[
    ...
    res.json({query:q,...mixed,filterBubbleBreaker:{enabled:true,personalizationDefault:'low',domainCap:2,lanes:['TRYAMM','small web','community','archive','live web'],note:'Ranking intentionally reserves room for independent and dissenting sources rather than optimizing only for engagement.'}});
  });
