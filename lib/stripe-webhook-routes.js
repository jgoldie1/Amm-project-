 ...
 app.post('/api/webhooks/stripe',async(req,res)=>{
  ...
  try{
   if(tx.configured()){
    ...
    if(!seen.created&&(seen.event?.status==='processed'||seen.event?.status==='ignored'))return res.json({received:true,idempotent:true});
   }
   ...
   if(event.type==='checkout.session.completed'&&object.payment_status==='paid'&&orderId){
    if(tx.configured()){await tx.commitOrder(orderId,object.payment_status);await tx.emitEvent({type:'order.paid',aggregateType:'marketplace_order',aggregateId:orderId,payload:{provider:'stripe',eventId:event.id,sessionId:object.id}});}
    const s=getStore(),order=(s.marketplaceOrders||[]).find(o=>o.id===orderId);
    if(order){
      if(!order.inventoryApplied){const product=(s.marketplaceProducts||[]).find(p=>p.id===order.productId);if(product&&product.stock!==null)product.stock=Math.max(0,Number(product.stock||0)-Number(order.quantity||1));order.inventoryApplied=true;}
      order.status='paid';order.paidAt=order.paidAt||new Date().toISOString();order.stripePaymentStatus=object.payment_status;order.updatedAt=new Date().toISOString();await saveStore();
    }
    ...
    const s=getStore(),order=(s.marketplaceOrders||[]).find(o=>o.id===orderId);if(order){order.status=event.type==='checkout.session.expired'?'expired':'payment_failed';order.inventoryReservation=event.type==='checkout.session.expired'?'expired':'released';order.updatedAt=new Date().toISOString();await saveStore();}
    ...
    const s=getStore(),order=(s.marketplaceOrders||[]).find(o=>o.id===oid);if(order){order.status=event.type==='charge.refunded'?'refunded':'disputed';order.updatedAt=new Date().toISOString();await saveStore();}
   }
