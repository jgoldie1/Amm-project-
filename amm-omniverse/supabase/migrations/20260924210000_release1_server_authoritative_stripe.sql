...
-- reaching the service-role-only RPC may create a transaction, entitlement, or ledger posting.
...
create table if not exists public.commerce_orders (
  ...
  status text not null default 'pending_payment' check (status in ('pending_payment','checkout_created','paid','payment_failed','cancelled','refunded')),
...
create table if not exists public.commerce_seller_allocations (
  ...
  transfer_status text not null default 'blocked' check (transfer_status in ('blocked','ready','transferred','refunded')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
create table if not exists public.commerce_transactions (
  ...
  status text not null default 'succeeded' check (status in ('succeeded','refunded','partially_refunded','reversed')),
  posting_id uuid,
  created_at timestamptz not null default now()
);

create unique index if not exists commerce_transactions_provider_session_uidx
  on public.commerce_transactions(provider_session_id);
create unique index if not exists commerce_transactions_provider_payment_uidx
  on public.commerce_transactions(provider_payment_id)
  where provider_payment_id is not null and provider_payment_id <> '';
create unique index if not exists commerce_transactions_provider_event_uidx
  on public.commerce_transactions(provider_event_id);
...
create table if not exists public.commerce_entitlements (
  ...
  transaction_id uuid not null references public.commerce_transactions(id) on delete restrict,
  product_id text not null,
  entitlement_key text not null,
  state text not null default 'active' check (state in ('active','revoked','refunded')),
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);

create index if not exists commerce_entitlements_buyer_idx on public.commerce_entitlements(buyer_id,created_at desc);
...
do $$ begin
  create policy "commerce buyers read own orders" on public.commerce_orders
    for select to authenticated using ((select auth.uid()) = buyer_id);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "commerce buyers read own items" on public.commerce_order_items
...
do $$ begin
  create policy "commerce buyers read own transactions" on public.commerce_transactions
    for select to authenticated using ((select auth.uid()) = buyer_id);
exception when duplicate_object then null; end $$;

do $$ begin
  create policy "commerce buyers read own entitlements" on public.commerce_entitlements
    for select to authenticated using ((select auth.uid()) = buyer_id);
exception when duplicate_object then null; end $$;
...
declare
  ...
  v_tx public.commerce_transactions%rowtype;
  v_posting uuid;
...
begin
  ...
  insert into public.commerce_payment_events(
    provider,provider_event_id,event_type,verified,order_id,payload
  ) values (
    'stripe',p_provider_event_id,coalesce(p_event_type,'unknown'),true,p_order_id,coalesce(p_event_payload,'{}'::jsonb)
  )
  ...
  insert into public.commerce_transactions(
    order_id,buyer_id,provider,provider_session_id,provider_payment_id,provider_event_id,
    amount_cents,currency,status
  ) values (
    p_order_id,p_buyer_id,'stripe',p_provider_session_id,nullif(p_provider_payment_id,''),p_provider_event_id,
    p_amount_cents,upper(p_currency),'succeeded'
  )
  returning * into v_tx;

  insert into public.commerce_entitlements(
    buyer_id,order_id,order_item_id,transaction_id,product_id,entitlement_key,state,metadata
  )
  select
    p_buyer_id,
    p_order_id,
    ...
    v_tx.id,
    oi.product_id,
    'commerce.product.' || oi.product_id,
    'active',
    jsonb_build_object('quantity',oi.quantity,'seller_key',oi.seller_key)
