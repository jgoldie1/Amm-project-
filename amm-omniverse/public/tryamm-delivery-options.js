(()=>{
  const VERSION='20260903-delivery-v2'
  const state={
  ...
  function evaluate(input={}){
    ...
    const paidOvernightFee=round(Math.max(base,quotedCarrier)+coldFee+handling+margin)
    ...
    return {
      ...
      freeOvernight,
      orderTotal,
      pricing:{paidOvernightFee,base,coldFee,handling,margin,carrierCost:quotedCarrier,freeOvernightOrderMinimum:minimum},
      options:[
        {id:'paid-overnight',label:'OVERNIGHT DELIVERY',price:paidOvernightFee,eligible:overnightAvailable,reason:overnightAvailable?`Estimated overnight fee ${'$'}${paidOvernightFee.toFixed(2)} before final carrier confirmation.`:reasons.filter(r=>!r.includes('free overnight')&&!r.includes('subsidy')).join(' • ')},
      ...
      truthBoundary:'Overnight fees are configurable estimates until carrier rates and address eligibility are verified. Free overnight remains an optional funded promotion or membership benefit, not an unlimited promise.'
    }
  }
