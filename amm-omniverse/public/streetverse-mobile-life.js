(()=>{
  ...
  #tryamm-mobile-life .sv-person{position:absolute;width:14px;height:31px;will-change:transform;animation:svWalk 3.4s ease-in-out infinite alternate;transition:transform .45s ease,filter .25s ease,opacity .25s ease}
  ...
  #tryamm-mobile-life .sv-person[data-state="shop"]{animation:none;transform:translate(14px,-5px) scale(.94)}
  ...
  @keyframes svTravel{from{transform:translateX(-18px)}to{transform:translateX(28px)}}
  ...
  function cycleBehaviors(layer){
    ...
    const stamp=Math.floor(Date.now()/4500)
    ...
    window.dispatchEvent(new CustomEvent('tryamm:streetverse-npc-behavior',{detail:{npcOnly:true,states:counts,cycleSeconds:4.5}}))
  }
  function mount(){
    if(mounted)return
    ...
    const people=[1,2,3,4,5,6,7].map(i=>`<div class="sv-person p${i}" data-state="walk"><div class="head"></div><div class="body"></div><div class="legs"></div></div>`).join('')
    ...
    behaviorTimer=window.setInterval(()=>cycleBehaviors(layer),4500)
    window.dispatchEvent(new CustomEvent('tryamm:streetverse-mobile-life-ready',{detail:{traffic:4,pedestrians:7,storefronts:3,ambientEvents:1,behaviorStates:behaviorStates.length,phase,weather,weatherSource:'simulated-world-weather',reducedMotion:reduceMotion(),npcOnly:true}}))
  }
