(()=>{
  const VERSION='20260903-studio-release-v1'
  ...
  const defaultRelease=()=>({
    version:VERSION,id:uid('ALBUM'),albumTitle:'',primaryArtist:'',label:'',releaseDate:'',genre:'',subgenre:'',territory:'Worldwide',copyrightYear:new Date().getFullYear(),artworkFileName:'',artworkRights:'unknown',upc:'',tracks:[defaultTrack()],updatedAt:new Date().toISOString()
  })
  function load(){try{const x=JSON.parse(localStorage.getItem(KEY)||'null');if(x?.tracks)return x}catch{}return defaultRelease()}
  let state=load()
  function save(){state.updatedAt=new Date().toISOString();localStorage.setItem(KEY,JSON.stringify(state));return snapshot()}
  ...
  function csv(){
    ...
    const rows=[['Track','Title','Version','Primary Artist','Featured Artists','Writers','Writer Splits','PRO','IPI/CAE','ISRC','ISWC','Master Owner','Composition Owner','Producer','Mix Engineer','Mastering Engineer','Explicit','Rights Type','Sample Status','Audio File']]
    state.tracks.forEach((t,i)=>rows.push([i+1,t.title,t.version,t.primaryArtist,t.featuredArtists,(t.writers||[]).map(w=>w.name).join('; '),(t.writers||[]).map(w=>`${w.name}:${num(w.split)}%`).join('; '),t.pro,t.ipi,t.isrc,t.iswc,t.masterOwner,t.compositionOwner,t.producer,t.mixEngineer,t.masteringEngineer,t.explicit?'yes':'no',t.rightsType,t.sampleStatus,t.audioFileName]))
  ...
  window.TRYAMMStudioRelease={version:VERSION,load:snapshot,save,setRelease,addTrack,updateTrack,removeTrack,trackReadiness,releaseReadiness,proPrep,copyrightPrep,distributorManifest,royaltyMap,csv,credits,reset}
  save()
  window.dispatchEvent(new CustomEvent('tryamm:studio-release-ready',{detail:{version:VERSION,features:['album-metadata','bmi-ascap-prep','sample-clearance-gate','copyright-prep','distrokid-ready-manifest','mlc-royalty-lane','soundexchange-royalty-lane','csv-json-credits-export'],externalSubmissions:false}}))
})()
