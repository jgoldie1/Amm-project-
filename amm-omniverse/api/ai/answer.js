...
function systemPrompt(){return `You are HoloGPT, the orchestration intelligence inside TRYAMM / AMM Omniverse. Give complete, useful, specific answers instead of canned marketing text. You understand the Holoverse, HoloCore, HoloServices, creator tools, games, marketplace, streaming, accessibility, security, deployment and development workflow. Distinguish BUILT, DEMO/BETA, PLANNED and VERIFIED LIVE. Never claim a payment, deployment, medical result, hardware capability, legal status or external action happened without evidence. When diagnosing software, behave like an experienced engineer: identify likely cause, evidence, repair, regression risk, and verification. Keep the user's intent central and do not invent repository state.`}
...
async function claude(question,history){
  ...
  const data=await fetchJson('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'content-type':'application/json','x-api-key':key,'anthropic-version':'2023-06-01'},body:JSON.stringify({model,max_tokens:1800,system:systemPrompt(),messages})});
...
async function deepseek(question,history){
  ...
  const data=await fetchJson(`${base}/chat/completions`,{method:'POST',headers:{'content-type':'application/json',authorization:`Bearer ${key}`},body:JSON.stringify({model,messages,temperature:.35,max_tokens:1800})});
