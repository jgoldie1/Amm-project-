export default async function handler(req,res){
  ...
  const providers={vercelGateway:gateway,openai,gemini,claude,glm,deepseek,ammBackend:backendProxy};
  const provider=gateway?'vercel-ai-gateway':openai?'openai':gemini?'gemini':claude?'claude':glm?'zai-glm':deepseek?'deepseek':backendProxy?'amm-backend':'diagnostic';
  ...
  return res.status(200).json({
    ...
    message:degraded?'HoloGPT shell is healthy. No generative provider is reachable yet. Vercel OIDC/AI Gateway is preferred, with direct providers including GLM 5.2 and the AMM backend as fallbacks.':'HoloGPT generative provider is configured.',
