...
export default async function handler(req,res){
  ...
  try{
    ...
    if(op==='revoke'){
      const id=String(req.body?.credentialId||'');await adminRest('security_passkeys',{method:'PATCH',query:{credential_id:`eq.${id}`,user_id:`eq.${user.id}`},body:{revoked_at:new Date().toISOString()}});await audit(user.id,'passkey.revoked','warning');return json(res,200,{ok:true});
    }
