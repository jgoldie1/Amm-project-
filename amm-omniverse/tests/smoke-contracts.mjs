...
const sustainability=read('src/platform/sustainabilityTelemetry.ts')
const moneySandbox=read('src/platform/moneySandbox.ts')
const persistence=read('src/platform/persistenceContracts.ts')
...
must(sustainability.includes('TRYAMM_SUSTAINABILITY_TARGET = 3'),'sustainability engine must target 3.00x')
must(sustainability.includes("'excluded_revenue'"),'sustainability engine must exclude obligations from platform revenue')
must(sustainability.includes('calculateFeatureEconomics'),'feature contribution economics must be measurable')
must(moneySandbox.includes('validateJournal'),'sandbox Money Engine must enforce balanced journals')
must(moneySandbox.includes("'hologpt_credit_liability'"),'HoloGPT Credits must remain a separate liability ledger')
must(moneySandbox.includes('transitionPayout'),'payout holds/state transitions must exist')
must(moneySandbox.includes('validateGift'),'AI Actions/Holo Gifts/creator gifts must have validation')
must(persistence.includes('AccountRepository'),'canonical account persistence contract must exist')
must(persistence.includes('OrderRepository'),'marketplace/delivery order persistence contract must exist')
must(persistence.includes('AuditRepository'),'audit persistence contract must exist')
must(persistence.includes('ProviderAdapterStatus'),'external providers must declare sandbox/gated/production state')
must(persistence.includes('authenticated ownership/RLS'),'production persistence must require server-side ownership/RLS')
