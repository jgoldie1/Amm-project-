...
for(const path of ['/api/streetverse/legacy/scene','/api/streetverse/legacy/memory','/api/streetverse/legacy/rights','/api/streetverse/legacy/eve-directive','/api/streetverse/legacy/smoke']) must(legacyApi.includes(path),`${path} frontend API contract must exist`)
must(backendLegacy.includes("action === 'smoke'") && backendLegacy.includes("action === 'scene'") && backendLegacy.includes("action === 'memory'"),'StreetVerse legacy backend router must implement smoke, scene and memory')
must(legacyMigration.includes('enable row level security') && legacyMigration.includes('auth.uid() = user_id'),'legacy persistence must be protected by Supabase RLS')
