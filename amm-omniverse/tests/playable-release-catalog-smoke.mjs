...
for(const token of ['StreetVerse','Court Kings','Volcano: Last Route','Battle Deck: Holo Champions'])must(catalog.includes(token),`${token} catalog`)
for(const token of ['StreetVerse + Court Kings + Volcano + Battle Deck','TIER‑1 PLAY','TIER_ONE_RELEASE_GAMES'])must(hub.includes(token),`dynamic catalog UI ${token}`)
