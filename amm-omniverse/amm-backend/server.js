...
app.get('/', (_req, res) => {
  res.json({
    ...
    systems: ['stripe','supabase','livekit','living-worlds','ai-cafe','workforce','kingdoms-press','app-store','stubbs-ai'],
  })
})
...
// New convergence APIs. Existing routes remain below for compatibility.
  ...
  try {
    switch (event.type) {
      case 'checkout.session.completed': {
        const { userId, plan, type } = object.metadata || {}
        if (!userId) break
...
app.post('/api/marketplace/sale', async (req, res) => {
  try {
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      transfer_data: { amount: creatorCut, destination: creatorStripeAccountId },
...
app.post('/api/player/:userId/save', async (req, res) => {
  try {
    ...
    const { error } = await supabase.from('player_state').upsert({
      ...
      missions: JSON.stringify(missions), vehicles: JSON.stringify(vehicles), avatar, cards: JSON.stringify(cards),
...
app.post('/api/marketplace/products', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('products').insert({
      name, description, price, category, is_dropship: isDropship,
      supplier_id: supplier || null, creator_id: userId, status: 'active', sold_count: 0,
...
app.post('/api/businesses', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('businesses').insert({
      ...
      owner_id: ownerId, status: 'active', rating: 0, review_count: 0, verified: false,
      created_at: new Date().toISOString(),
    }).select().single()
    if (error) throw error
...
app.post('/api/music/tracks', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('tracks').insert({
      title, genre, scripture, bpm, creator_id: creatorId, file_url: fileUrl, duration,
