...
async function getStripeSettlement(stripe, session){
  const result={ fee:null, net:null, balanceTransactionId:null, exact:false }
  if(!stripe||!session?.payment_intent) return result
  try{
...
async function postCheckoutToTreasury({supabase,stripe,session}){
  ...
  await upsertEntry(supabase,{
    entryType:'revenue', sourceSystem:'stripe-checkout', sourceRef, grossAmount:gross, credit:gross, currency,
  ...
  if(settlement.fee!=null&&settlement.fee>0){
    await upsertEntry(supabase,{
      entryType:'provider_fee',sourceSystem:'stripe-checkout',sourceRef,grossAmount:gross,debit:settlement.fee,currency,
      description:'Stripe processing fee',metadata:{exact:settlement.exact,balanceTransactionId:settlement.balanceTransactionId}
    })
  }

  // Contract splits are posted only when trusted server-side checkout metadata contains approved basis points.
  const splits=[
    ['creator_payable','creatorShareBps','creatorUserId'],
    ['talent_payable','talentShareBps','talentUserId'],
    ['royalty_payable','royaltyShareBps','royaltyPartyId'],
  ]
  for(const [entryType,bpsKey,partyKey] of splits){
    ...
    if(bps>0&&party){
      ...
      if(amount>0) await upsertEntry(supabase,{
        entryType,sourceSystem:'stripe-checkout',sourceRef,grossAmount:gross,debit:amount,currency,
...
async function postRefundToTreasury({supabase,eventObject,eventId}){
  ...
  if(amount<=0) return
  await upsertEntry(supabase,{
    entryType:'refund',sourceSystem:'stripe-refund',sourceRef:String(eventId),grossAmount:amount,debit:amount,
    currency:String(eventObject.currency||'usd').toUpperCase(),description:'Stripe refund/credit adjustment',
...
async function postDisputeToTreasury({supabase,dispute,eventId}){
  ...
  if(amount<=0) return
  await upsertEntry(supabase,{
    entryType:'chargeback',sourceSystem:'stripe-dispute',sourceRef:String(eventId),grossAmount:amount,debit:amount,
...
module.exports={postCheckoutToTreasury,postRefundToTreasury,postDisputeToTreasury}
