...
export default function FamilyBusinessPublicSite({slug,onClose}:{slug:string;onClose:()=>void}){
 ...
 if(!profile)return <div style={{minHeight:'100vh',background:'#05080d',color:'#fff',display:'grid',placeItems:'center',padding:24}}><div><h1>Business page not found</h1><button onClick={onClose}>Back to TRYAMM</button></div></div>
 const primary=profile.ventures[0]
 return <main style={{minHeight:'100vh',background:'linear-gradient(145deg,#04070b,#0a1720 55%,#151008)',color:'#fff',fontFamily:'Inter,system-ui,sans-serif'}}>
  <section style={{maxWidth:1120,margin:'0 auto',padding:'28px 20px 56px'}}>
   ...
   <div style={{padding:'72px 0 38px'}}><div style={{fontSize:12,color:'#4fe3ff',fontWeight:900,textTransform:'uppercase'}}>{profile.region}</div><h1 style={{fontSize:'clamp(38px,7vw,78px)',lineHeight:.98,margin:'10px 0 18px',maxWidth:900}}>{profile.owner}</h1><h2 style={{fontSize:'clamp(20px,3vw,34px)',fontWeight:650,color:'#d7e4eb',margin:0,maxWidth:800}}>{primary}</h2><p style={{maxWidth:720,lineHeight:1.7,color:'#9fb1be',fontSize:15}}>A TRYAMM-powered business profile prepared for website, booking or commerce, customer relationship tools, Holo Ads, StreetVerse presence and analytics. Service availability depends on the modules selected for this business.</p><div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:22}}><button onClick={()=>window.dispatchEvent(new CustomEvent('tryamm:family-business-contact',{detail:{id:profile.id}}))} style={primaryBtn}>CONTACT / REQUEST SERVICE</button>{profile.modules.includes('booking')||profile.modules.includes('service-booking')?<button onClick={()=>window.dispatchEvent(new CustomEvent('tryamm:family-business-booking',{detail:{id:profile.id}}))} style={btn}>BOOK</button>:null}{profile.modules.includes('store')?<button onClick={()=>window.dispatchEvent(new CustomEvent('tryamm:family-business-store',{detail:{id:profile.id}}))} style={btn}>SHOP</button>:null}</div></div>
