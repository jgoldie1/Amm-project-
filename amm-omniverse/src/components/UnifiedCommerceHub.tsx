...
{name:'Local grass-fed beef box',category:'Local beef',target:99,margin:'18–30%',source:'Eatwild discovery / verified U.S. farm / inspected processor'},
{name:'Pasture-raised chicken box',category:'Local poultry',target:69,margin:'18–30%',source:'Eatwild discovery / verified U.S. farm / inspected processor'},
{name:'Local pork box',category:'Local pork',target:79,margin:'18–30%',source:'Eatwild discovery / verified U.S. farm / inspected processor'},
{name:'Local lamb or goat box',category:'Local meat',target:109,margin:'18–30%',source:'Eatwild discovery / verified U.S. farm / inspected processor'},
{name:'Pasture-raised eggs',category:'Eggs',target:6.99,margin:'20–35%',source:'Verified local farm / regional distributor'},
{name:'Local dairy bundle',category:'Dairy',target:24.99,margin:'18–30%',source:'Verified local dairy / regional distributor'},
{name:'CSA produce box',category:'Fresh produce',target:34.99,margin:'15–28%',source:'Verified U.S. farm / CSA / farmers market'},
{name:'Local honey + pantry bundle',category:'Local pantry',target:29.99,margin:'25–40%',source:'Verified U.S. producer / cooperative'}]
...
{title:'Local Farm & Meat Market',price:'Quote-based',detail:'Verified local/regional farm sourcing for inspected meat/poultry, eggs, dairy, produce, honey and CSA bundles with cold-chain and origin gates.'}]
...
export default function UnifiedCommerceHub(){
 ...
 return <div role="dialog" aria-modal="true" aria-label="TRYAMM Living Commerce" style={{position:'fixed',inset:0,zIndex:23050,background:'rgba(2,4,10,.97)',color:'#fff',overflow:'auto'}}><div style={{maxWidth:1180,margin:'0 auto',padding:18}}>
  ...
  {lane!=='business'?<><section style={panel}><h2>{lane==='grocery'?'Healthy-value + local American farm catalog':'High-margin beauty launch catalog'}</h2>{lane==='grocery'&&<p style={{color:'#b9d9c4'}}>Local beef, poultry, pork, lamb/goat, eggs, dairy, produce and honey are discovery/launch lanes. Products remain held until supplier, origin, inspection/processing and cold-chain requirements are verified.</p>}<div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}><thead><tr>{['Product','Category','Target retail','Target gross margin','Supplier lane'].map(x=><th key={x} style={th}>{x}</th>)}</tr></thead><tbody>{products.map(p=><tr key={p.name}><td style={td}><b>{p.name}</b></td><td style={td}>{p.category}</td><td style={td}>${p.target.toFixed(2)}</td><td style={td}>{p.margin}</td><td style={td}>{p.source}</td></tr>)}</tbody></table></div></section>
  <section style={panel}><h2>Dynamic floor-price engine</h2><p style={{color:'#9fb3c6'}}>WHOLESALE + FREIGHT + PICK/PACK + PAYMENT FEE + SPOILAGE RESERVE + DELIVERY + REQUIRED MARGIN = FLOOR PRICE. The catalog must never publish below the higher of this floor or an applicable supplier MAP constraint.</p><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:8}}>{Object.entries(cost).map(([k,v])=><label key={k} style={card}>{k}<input type="number" value={v} onChange={e=>setCost(c=>({...c,[k]:Number(e.target.value)||0}))} style={input}/></label>)}</div><div style={{marginTop:12,fontSize:22,fontWeight:950,color:'#E8B944'}}>SAFE FLOOR: ${floor.toFixed(2)}</div></section></>:<><section style={panel}><h2>Supply Plug Global · Business-in-a-Box</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(230px,1fr))',gap:8}}>{startup.map(x=><div key={x.title} style={card}><h3 style={{marginTop:0}}>{x.title}</h3><div style={{fontSize:22,color:'#E8B944',fontWeight:950}}>{x.price}</div><p style={{color:'#9fb3c6'}}>{x.detail}</p></div>)}</div></section><section style={panel}><h2>Entrepreneur engine</h2><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:8}}>{['STARTUP BUDGET','STORE TYPE','LOCATION','PRODUCT MIX','VERIFIED WHOLESALERS / FARMS','MOQ','LANDED COST','OPENING INVENTORY','FLOOR PRICE','RETAIL PRICE','TRYAMM STOREFRONT','LIVE COMMERCE','LOCAL DELIVERY','AUTOMATIC REORDER'].map((x,i)=><div key={x} style={card}><b>{i+1}. {x}</b></div>)}</div></section></>}
  <section style={panel}><h2>Supplier routing</h2><p style={{color:'#9fb3c6',lineHeight:1.6}}>Grocery: AWG for cooperative scale/private brands when membership economics fit; KeHE for specialty/natural/fresh and small-parcel expansion; UNFI for scaled conventional/natural distribution after order volume supports its minimums. Local American farm lane: Eatwild for discovery plus verified farms, ranches, CSA programs, farmers markets and regional processors/distributors; each actual sellable SKU still requires supplier approval, origin evidence, applicable inspection/processing, labeling, inventory and cold-chain verification. Beauty: Jinny, Bee Sales, PK International and specialist nail wholesalers, subject to account approval, authenticity, MAP and current quotes.</p></section>
  <section style={panel}><b>PRODUCT FLOW</b><div style={{marginTop:8,color:'#9fb3c6'}}>VERIFIED SUPPLIER / FARM → SUPPLY PLUG GLOBAL → PRODUCT PASSPORT → LANDED COST → VIRTUAL WAREHOUSE → STOREFRONT / STREETVERSE STORE → ORDER RESERVATION → VERIFIED PAYMENT → PICK/PACK + COLD CHAIN WHEN NEEDED → DELIVERY/PICKUP → PROOF → INVENTORY DECREMENT → REORDER</div></section>
