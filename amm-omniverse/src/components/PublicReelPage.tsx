...
export default function PublicReelPage({slug}:{slug:string}){
  ...
  const [bufferPolicy,setBufferPolicy]=useState('BALANCED')
  const [warpPolicy,setWarpPolicy]=useState('READY')
  ...
  useEffect(()=>{const sync=(event:Event)=>{const detail=(event as CustomEvent).detail||{};if(detail.enabled===false){setLagStatus('OFF');setBufferPolicy('NORMAL');return}const rtt=detail.metrics?.rtt,jitter=detail.metrics?.jitter,downlink=detail.metrics?.downlink;const stressed=rtt!=null&&(rtt>160||jitter>35)||(downlink!=null&&downlink<2);setLagStatus(!detail.metrics?.online?'OFFLINE':stressed?'DATA SAVER':detail.mode==='streaming'?'STREAMING':'ADAPTIVE');setBufferPolicy(!detail.metrics?.online?'PAUSED':stressed?'BUFFER AHEAD':detail.mode==='streaming'?'CONTINUITY':'BALANCED');setWarpPolicy(!detail.metrics?.online?'CHECKPOINT':stressed?'LIGHT PRELOAD':'PREDICTIVE PRELOAD')};window.addEventListener('tryamm:quantum-lag-buster',sync);return()=>window.removeEventListener('tryamm:quantum-lag-buster',sync)},[])
