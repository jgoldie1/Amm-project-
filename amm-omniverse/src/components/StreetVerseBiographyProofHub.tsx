...
import { loadRejoinState, runStreetVerseLifeSmoke, saveArchiveProgress, saveBiography, saveCreatorWork, simulateReturn } from '../services/streetVerseLifeApi'
...
export default function StreetVerseBiographyProofHub(){
 const [open,setOpen]=useState(false),[status,setStatus]=useState('READY'),[log,setLog]=useState<string[]>([]),[rejoin,setRejoin]=useState<any>(null)
 ...
 const fullProof=async()=>{setStatus('RUNNING');try{await runStreetVerseLifeSmoke();push('PUBLIC API SMOKE: GREEN');await saveCurrent();await returnToWorld();await solveArchive();await createWork();const result=await loadRejoinState(characterId);setRejoin(result);setStatus('GREEN');push('END-TO-END AUTHENTICATED PROOF: GREEN')}catch(error){setStatus('FAILED');push(`FULL PROOF STOPPED: ${error instanceof Error?error.message:'failed'}`)}}
 return <><button type="button" aria-label="Open StreetVerse biography proof" onClick={()=>setOpen(true)} style={{position:'fixed',left:12,bottom:272,zIndex:8998,border:'1px solid #78ffb488',borderRadius:999,background:'linear-gradient(135deg,#0a2a20,#102333)',color:'#fff',padding:'10px 14px',fontFamily:'monospace',fontSize:10,fontWeight:900}}>🧬 WORLD MEMORY PROOF</button>
 {open&&<div role="dialog" aria-modal="true" aria-label="StreetVerse World Memory Proof" style={{position:'fixed',inset:0,zIndex:10130,overflowY:'auto',background:'radial-gradient(circle at top,#102c2c,#04070b 58%,#020204)',color:'#fff',padding:16}}><div style={{maxWidth:1100,margin:'0 auto'}}>
