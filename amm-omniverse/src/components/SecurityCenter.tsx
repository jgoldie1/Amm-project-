...
export default function SecurityCenter({onClose}:Props){
  ...
  const enroll=async()=>{setBusy(true);setStatus('');try{
    ...
    let stepUpToken=''
    if(passkeys.length){const verified=await stepUp('add-passkey');stepUpToken=verified.stepUpToken}
    const o=await api('/api/security/passkeys',{method:'POST',headers:stepUpToken?{'x-step-up-token':stepUpToken}:{},body:JSON.stringify({op:'register-options'})})
    ...
    const response=credential.response as AuthenticatorAttestationResponse & {getPublicKey?:()=>ArrayBuffer|null;getTransports?:()=>string[];getAuthenticatorData?:()=>ArrayBuffer}
    const publicKey=response.getPublicKey?.();const authData=response.getAuthenticatorData?.();if(!publicKey||!authData)throw new Error('Browser did not expose the required passkey registration data.')
    await api('/api/security/passkeys',{method:'POST',body:JSON.stringify({op:'register-verify',label:'My passkey',response:{credentialId:credential.id,clientDataJSON:b64u(response.clientDataJSON),authenticatorData:b64u(authData),publicKey:b64u(publicKey),transports:response.getTransports?.()||[]}})})
  ...
  const revoke=async(id:string)=>{setBusy(true);try{const verified=await stepUp('remove-passkey');await api('/api/security/passkeys',{method:'POST',headers:{'x-step-up-token':verified.stepUpToken},body:JSON.stringify({op:'revoke',credentialId:id})});setStatus('Passkey revoked.');await refresh()}catch(e:any){setStatus(e.message)}finally{setBusy(false)}}
