...
const sourceControls = [
  ['LOW MOQ','Request sample + 25/50/100/250/500 pricing where supported; compare cash at risk and reorder flexibility.'],
  ['QUANTUM TARIFF BUSTER','Compare lawful tariff/duty/tax/brokerage scenarios, origin options and verified preference-program eligibility. Never misclassify goods or understate customs value.'],
  ['QUANTUM PADDING','Select packaging, padding, moisture/tamper protection, shock/tilt controls and verified drop/vibration test evidence.'],
  ['PROOF TRACKING','Track pickup → handoff → customs → delivery → return/exception with carrier status and condition events.'],
  ['PHOTO + VIDEO VERIFY','Attach evidence at pre-pack, packed, pickup, customs, delivery, return and damage-claim stages; preserve timestamps and integrity hashes when available.'],
  ['DTC FULFILLMENT','Compare stock, micro-batch, made-to-order, POD and verified dropship/direct-fulfillment routes before scaling inventory.'],
] as const
...
export default function HoloStyleCenter({ onClose }: HoloStyleCenterProps) {
  ...
  const tabs = useMemo(() => [
    ['studio', 'HOLOSTYLE'],['development', 'DEVELOPMENT'],['academy', 'ACADEMY'],['brand', 'BRAND'],['source', 'QUANTUM SOURCE'],['streetverse', 'STREETVERSE SALES'],
  ] as const, [])

  return <main style={{minHeight:'100dvh',background:'radial-gradient(circle at 50% 0%,#1c1538 0,#080817 38%,#02020a 100%)',color:'#fff',fontFamily:'Inter,system-ui,sans-serif',padding:'20px 16px 80px',overflowY:'auto'}}><div style={{maxWidth:1120,margin:'0 auto'}}>
    <header style={{display:'flex',gap:14,alignItems:'flex-start',justifyContent:'space-between',marginBottom:18}}><div><div style={{fontSize:11,fontWeight:900,letterSpacing:4,color:'#72efff'}}>TRYAMM • BENNY + CONSTRUCT</div><h1 style={{fontSize:'clamp(28px,6vw,54px)',margin:'8px 0 5px',lineHeight:.95}}>HoloStyle Design → Create → Sell</h1><p style={{margin:0,maxWidth:760,color:'#b9bfd3',lineHeight:1.6}}>Create an original look, build the brand, train the creator, prepare sourcing, sell through StreetVerse and route verified revenue to the TRYAMM ledger.</p></div><button aria-label="Close HoloStyle" onClick={onClose} style={{width:42,height:42,borderRadius:'50%',border:'1px solid #ffffff33',background:'#111225',color:'#fff',fontSize:20,cursor:'pointer'}}>×</button></header>
    <nav aria-label="HoloStyle sections" style={{display:'flex',gap:8,overflowX:'auto',paddingBottom:10,marginBottom:18}}>{tabs.map(([id,label])=><button key={id} onClick={()=>setSection(id)} aria-pressed={section===id} style={{whiteSpace:'nowrap',borderRadius:999,border:section===id?'1px solid #70edff':'1px solid #28304a',background:section===id?'#123548':'#0c0f1d',color:section===id?'#9ff5ff':'#c5cad9',padding:'10px 14px',fontWeight:900,fontSize:10,letterSpacing:1,cursor:'pointer'}}>{label}</button>)}</nav>

    {section==='studio'&&<section><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:14}}><article style={card}><div style={eyebrow}>BENNY PERSONAL STYLIST</div><h2 style={h2}>Tell Benny the reality you want.</h2><textarea value={brief} onChange={e=>setBrief(e.target.value)} placeholder="Example: Build a luxury adaptive streetwear look for a 2050 Chicago runway, with matching jewelry, shoes and environment." style={{width:'100%',minHeight:118,resize:'vertical',boxSizing:'border-box',borderRadius:14,border:'1px solid #2b3854',background:'#070b14',color:'#fff',padding:12,lineHeight:1.5}}/><button style={primaryButton} onClick={()=>setBrief(v=>v.trim()||'Original HoloStyle collection concept')}>CREATE DESIGN BRIEF</button></article><article style={card}><div style={eyebrow}>TRANSFORMATION STACK</div><h2 style={h2}>Self + world wrapping</h2><p style={body}>Avatar/body presentation, garments, shoes, jewelry, watches, glasses, hair, makeup, bags, accessories, vehicle, room, storefront, lighting and era can share one scene specification.</p><div style={flow}>BENNY → CONSTRUCT → AI TWIN → HOLO WRAP → SELL</div></article></div></section>}
    ...
    {section==='source'&&<section><div style={eyebrow}>QUANTUM SOURCE • GLOBAL SUPPLY CONTROL</div><h2 style={sectionTitle}>Compare cost, tariff exposure, packaging risk and proof — not only factory price.</h2><div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:14}}>{sourceRegions.map(r=><button key={r} onClick={()=>setSelectedRegion(r)} style={{...chip,background:selectedRegion===r?'#173c43':'#0b101b',borderColor:selectedRegion===r?'#70edff':'#28334a'}}>{r}</button>)}</div><div style={card}><strong>{selectedRegion} sourcing lane</strong><p style={body}>Quantum Bid compares unit cost + MOQ + sample/tooling + packaging + QC + freight + lawful duties/tariffs/taxes + insurance + expected defect allowance = estimated landed cost.</p><div style={flow}>RIGHTS VAULT → LOW-MOQ RFQ → TARIFF SCENARIO → LANDED COST → SAMPLE/QC → QUANTUM PADDING → PHOTO/VIDEO PACK PROOF → SHIPMENT TRACKING → CUSTOMS → DELIVERY PROOF → DTC CUSTOMER → CLAIM/RETURN EVIDENCE → LEDGER → SMART REORDER</div></div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:10,marginTop:12}}>{sourceControls.map(([title,desc])=><article key={title} style={smallCard}><div style={eyebrow}>{title}</div><p style={body}>{desc}</p></article>)}</div><div style={{...card,marginTop:12}}><div style={eyebrow}>BENNY DECISION EXPLAINER</div><p style={body}>The intended recommendation engine ranks verified offers by landed margin, MOQ/cash-at-risk, lead time, supplier risk, quality evidence, packaging protection and fulfillment fit. Example: a higher unit-cost supplier can still win when it requires less inventory, has lower tariff/freight exposure or provides safer DTC fulfillment.</p><p style={body}>Tariff rates, HS classification, trade-program eligibility and customs treatment require current authoritative verification. TRYAMM should optimize legally — never evade tariffs, falsify origin, misclassify products or understate customs value.</p></div></section>}
