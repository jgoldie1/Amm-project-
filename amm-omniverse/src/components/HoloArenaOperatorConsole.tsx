...
import { HOLOARENA_COLOSSEUM, COLOSSEUM_PAYOUT_CHAIN } from '../holoarena/ColosseumDistrict'
...
export default function HoloArenaOperatorConsole({onClose}:{onClose:()=>void}){
 return <div role="dialog" aria-label="TRYAMM HoloArena" style={{position:'fixed',inset:0,zIndex:12500,overflowY:'auto',background:'radial-gradient(circle at top,#10253a,#03040c 55%,#010106)',color:'#fff',padding:18}}><div style={{maxWidth:1180,margin:'0 auto'}}>
  ...
  <section style={panel}><h2>{HOLOARENA_COLOSSEUM.name}</h2><p style={muted}>{HOLOARENA_COLOSSEUM.purpose}</p><div style={grid}>{HOLOARENA_COLOSSEUM.zones.map(x=><article key={x} style={card}><b>{x}</b></article>)}</div><p style={accent}>{HOLOARENA_COLOSSEUM.eventFlow.join(' → ')}</p><p style={muted}>Venue: {HOLOARENA_COLOSSEUM.modes.venue}</p><p style={muted}>Home XR: {HOLOARENA_COLOSSEUM.modes.homeXR}</p><p style={muted}>Mobile: {HOLOARENA_COLOSSEUM.modes.mobile}</p><p style={muted}>Tailgate: {HOLOARENA_COLOSSEUM.tailgate.activities.join(' • ')}</p><p style={{...muted,color:'#ffcf66'}}>Payout chain: {COLOSSEUM_PAYOUT_CHAIN}</p></section>
