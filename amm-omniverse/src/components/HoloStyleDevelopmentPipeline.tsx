...
export default function HoloStyleDevelopmentPipeline() {
  ...
  return <section aria-label="HoloStyle product development pipeline">
    ...
    <div style={{...panel,marginBottom:16}}><div style={eyebrow}>LOW-MOQ + DIRECT-TO-CONSUMER ENGINE</div><h3 style={{fontSize:24,margin:'7px 0 8px'}}>Prove demand before tying up cash in inventory.</h3><p style={muted}>Quantum Source should request an MOQ ladder from each verified supplier — sample quantity, smallest production run, 25/50/100/250/500-unit pricing where applicable — then compare landed margin, lead time, quality and risk rather than automatically choosing the cheapest quote.</p><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:10,margin:'14px 0'}}><label style={label}>Target MOQ<input type="number" min="1" value={moqTarget} onChange={e=>setMoqTarget(Number(e.target.value)||1)} style={input}/></label><label style={label}>Estimated unit cost ($)<input type="number" min="0" step="0.01" value={unitCost} onChange={e=>setUnitCost(Number(e.target.value)||0)} style={input}/></label><label style={label}>DTC retail price ($)<input type="number" min="0" step="0.01" value={retailPrice} onChange={e=>setRetailPrice(Number(e.target.value)||0)} style={input}/></label></div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:9}}><div style={metric}><span style={mini}>INITIAL PRODUCT COST</span><strong>${inventoryCash.toLocaleString()}</strong></div><div style={metric}><span style={mini}>POTENTIAL GROSS SALES</span><strong>${grossSales.toLocaleString()}</strong></div><div style={metric}><span style={mini}>BEFORE OTHER COSTS</span><strong>${grossBeforeOtherCosts.toLocaleString()}</strong></div></div><p style={{...muted,fontSize:11}}>Illustrative planning math only. Freight, duties, taxes, payment fees, returns, discounts, packaging, fulfillment, defects, marketing and other costs can materially reduce margin.</p><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:9,marginTop:12}}>{inventoryModes.map(([name,desc])=><div key={name} style={subPanel}><strong style={{color:'#8ff4ff'}}>{name}</strong><p style={muted}>{desc}</p></div>)}</div><div style={flow}>DESIGN → SAMPLE → DTC WAITLIST → PREORDER/DEPOSIT → LOW-MOQ RUN → STREETVERSE + REELS + LIVE → FULFILL → SELL-THROUGH CHECK → SMART REORDER</div></div>
