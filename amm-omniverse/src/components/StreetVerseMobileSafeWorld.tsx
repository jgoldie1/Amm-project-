...
export default function StreetVerseMobileSafeWorld({onClose}:{onClose:()=>void}){
 ...
 useEffect(()=>{
  ...
  setStatus('MOBILE SAFE CITY • LIVE');setMessage('Chicago StreetVerse loaded in mobile-safe mode. Use the one-hand joystick to move.');window.dispatchEvent(new CustomEvent('tryamm:streetverse-enter',{detail:{district:'01',mobileSafe:true,residents:0,vehicles:cars.length,cityVisible:true}}));animate()
 ...
 const applyStick=(x:number,y:number)=>{const dead=.16;moveRef.current={up:y<-dead,down:y>dead,left:x<-dead,right:x>dead};setStick({x,y})}
 const moveStick=(e:React.PointerEvent<HTMLDivElement>)=>{if(joystickPointer.current!==e.pointerId)return;const el=joystickRef.current;if(!el)return;const r=el.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,dx=e.clientX-cx,dy=e.clientY-cy,max=r.width*.32,len=Math.hypot(dx,dy)||1,scale=Math.min(1,max/len);applyStick((dx*scale)/max,(dy*scale)/max)}
 const startStick=(e:React.PointerEvent<HTMLDivElement>)=>{e.preventDefault();joystickPointer.current=e.pointerId;e.currentTarget.setPointerCapture?.(e.pointerId);moveStick(e)}
 const stopStick=(e:React.PointerEvent<HTMLDivElement>)=>{if(joystickPointer.current!==e.pointerId)return;joystickPointer.current=null;applyStick(0,0)}
 if(webglFailed)return <StreetVerseMobilePlayableWorld onClose={onClose}/>
 return <div style={{position:'fixed',inset:0,background:'#07101d',zIndex:15000}}><div ref={mountRef} style={{position:'absolute',inset:0}}/><div style={{position:'fixed',top:14,left:14,zIndex:15010,padding:'9px 11px',borderRadius:10,background:'#07131ddd',border:'1px solid #66e6ff99',color:'#fff',fontFamily:'system-ui',fontSize:11,fontWeight:800}}>{status}<div style={{opacity:.72,marginTop:3,maxWidth:260}}>{message}</div></div><button onClick={onClose} style={{position:'fixed',top:14,right:14,zIndex:15012,minHeight:44,padding:'0 14px',borderRadius:12,border:'1px solid #ffffff55',background:'#111827dd',color:'#fff',fontWeight:900}}>EXIT</button><div ref={joystickRef} role="application" aria-label="StreetVerse one-hand movement joystick" onPointerDown={startStick} onPointerMove={moveStick} onPointerUp={stopStick} onPointerCancel={stopStick} style={{position:'fixed',left:18,bottom:24,zIndex:15012,width:138,height:138,borderRadius:'50%',border:'1px solid #6fe8ff88',background:'radial-gradient(circle,#153447cc,#07131de8 68%)',touchAction:'none',boxShadow:'0 8px 28px #0009',WebkitTapHighlightColor:'transparent'}}><div aria-hidden="true" style={{position:'absolute',left:'50%',top:'50%',width:58,height:58,borderRadius:'50%',border:'2px solid #b9f4ffcc',background:'#1b536be8',transform:`translate(calc(-50% + ${stick.x*38}px), calc(-50% + ${stick.y*38}px))`,boxShadow:'0 0 18px #6fe8ff66',pointerEvents:'none'}}/><div style={{position:'absolute',left:0,right:0,bottom:-20,textAlign:'center',color:'#bfefff',fontSize:10,fontWeight:900,letterSpacing:'.08em',pointerEvents:'none'}}>ONE-HAND MOVE</div></div></div>
}
