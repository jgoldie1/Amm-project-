...
type Lane = 'All' | 'LIVE Replays' | 'Originals' | 'Showcase' | 'StarVerse' | 'Debate' | 'Games' | 'Podcasts' | 'Music' | 'News' | 'Sports' | 'Faith' | 'Marketplace'
type Network='Isaiah AI TV'|'All American Network TV'|'Servants of Christ TV'

type Program = {id:string;title:string;lane:Exclude<Lane,'All'>;rating:string;access:'FREE'|'MEMBER'|'PPV';network:Network;description:string}
...
const PROGRAMS: Program[] = [
  ...
  { id:'hologame', title:'Holo Game Night', lane:'Games', rating:'TV-PG', access:'FREE', network:'Isaiah AI TV', description:'GameVerse competition, tournaments and interactive audience play.' },
  ...
  { id:'news', title:'All American News Desk', lane:'News', rating:'TV-PG', access:'FREE', network:'All American Network TV', description:'Local-to-global news, community reporting and creator correspondents with source links and editorial controls.' },
  { id:'sports', title:'All American Sports', lane:'Sports', rating:'TV-G', access:'FREE', network:'All American Network TV', description:'Sports shows, highlights, athlete stories and GameVerse tie-ins.' },
  ...
  { id:'market', title:'All American Marketplace LIVE', lane:'Marketplace', rating:'TV-G', access:'FREE', network:'All American Network TV', description:'Verified business showcases, product demos and creator commerce programming.' },
]

const LANES: Lane[] = ['All','LIVE Replays','Originals','Showcase','StarVerse','Debate','Games','Podcasts','Music','News','Sports','Faith','Marketplace']
...
export default function OTTIsaiahTV({ onClose }: Props) {
  ...
  const [network,setNetwork]=useState<Network>('Isaiah AI TV')
  ...
  function dispatch(action: string, detail: Record<string, unknown> = {}) {window.dispatchEvent(new CustomEvent('tryamm:ott-action', { detail: { action, network, ...detail } }))}
  ...
  return <div role="dialog" aria-modal="true" aria-label="TRYAMM TV Core" style={{position:'fixed',inset:0,zIndex:12100,background:'linear-gradient(180deg,#03040b,#080d1b 48%,#02030a)',color:'#fff',overflowY:'auto',fontFamily:'system-ui,sans-serif'}}>
    <header style={{position:'sticky',top:0,zIndex:2,display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,padding:'16px 20px',background:'#050713ee',backdropFilter:'blur(14px)',borderBottom:'1px solid #25314d'}}>
      <div><div style={{fontSize:11,letterSpacing:2,color:'#8cecff'}}>TRYAMM OTT / OMNI BOX MEDIA CORE</div><h1 style={{margin:'3px 0'}}>{network}</h1><div style={{opacity:.68,fontSize:13}}>LIVE → replay → episode → podcast → Holo Clip → translated/accessibility editions → distribution</div></div>
    ...
    <main style={{maxWidth:1100,margin:'0 auto',padding:20,display:'grid',gap:18}}>
      ...
      <section style={{padding:18,border:'1px solid #26496a',borderRadius:20,background:'radial-gradient(circle at 80% 20%,#153b58,#08111f 55%)'}}>
        <div style={{fontSize:12,color:'#ff8fa4',fontWeight:900}}>● LIVE + PREMIERES</div><h2 style={{fontSize:'clamp(26px,5vw,52px)',margin:'8px 0'}}>One broadcast. Many products.</h2>
        <p style={{maxWidth:760,opacity:.75,lineHeight:1.6}}>A TryAMM LIVE room can become a replay, TV episode, podcast edition, captioned/translated edition, audio-described version and short-form Holo Clip package. Recording/egress, licensed content and storage providers must be configured before automatic publishing is marked live.</p>
        <div style={{display:'flex',gap:10,flexWrap:'wrap'}}><button onClick={()=>{dispatch('open-live');(window as any).__showTryAMMLive?.()}} style={primary}>● OPEN LIVE STUDIO</button><button onClick={()=>dispatch('request-publish-workflow')} style={secondary}>CREATE OTT RELEASE</button><button onClick={()=>window.dispatchEvent(new CustomEvent('tryamm:social-share-open',{detail:{source:'tv',network}}))} style={secondary}>SHARE EVERYWHERE</button></div>
      </section>
