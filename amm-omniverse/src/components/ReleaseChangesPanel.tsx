...
export default function ReleaseChangesPanel(){
  ...
  const run=(name:'play'|'media'|'campus')=>{
    ...
    if(name==='campus'){document.querySelector<HTMLButtonElement>('[aria-label="Open Greenville University Class of 2031 welcome board"]')?.click()}
  }
  ...
  return <div role="dialog" aria-label="TRYAMM latest release changes" style={{position:'fixed',inset:0,zIndex:15120,background:'#02050bf5',color:'#fff',overflow:'auto',fontFamily:'Inter,system-ui,sans-serif'}}>
    <div style={{maxWidth:920,margin:'0 auto',padding:'22px 14px 80px'}}>
      ...
      <div style={{display:'flex',gap:9,flexWrap:'wrap',marginBottom:15}}><button onClick={()=>run('play')} style={primary}>🎮 PLAY STREETVERSE</button><button onClick={()=>run('media')} style={secondary}>🎬 CREATE CONTENT</button><button onClick={()=>run('campus')} style={secondary}>🎓 CLASS OF 2031</button></div>
