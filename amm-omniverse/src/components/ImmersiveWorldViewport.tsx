...
const modes:Record<Mode,{label:string;sky:number;ground:number;gravity:string;description:string}>={
 city:{label:'Global City',sky:0x030714,ground:0x151923,gravity:'1.0g',description:'StreetVerse city immersion: streets, buildings, business, creator, mission and social lanes.'},
 wilderness:{label:'Worldwide Wilderness',sky:0x07180f,ground:0x18351d,gravity:'1.0g',description:'Biosphere prototype for wildlife observation, tracking, fishing and conservation simulation.'},
 ...
 chrono:{label:'Chrono Corridor',sky:0x050019,ground:0x12102a,gravity:'variable',description:'Historical reconstructions, future simulations and alternate timelines remain clearly labeled.'},
}
...
export default function ImmersiveWorldViewport({onClose}:Props){
 ...
 const [xrActive,setXrActive]=useState<XRMode|null>(null)

 useEffect(()=>{
  let alive=true
  const xr=(navigator as any).xr
  if(!xr?.isSessionSupported){setXrSupport({vr:false,ar:false,checked:true});return}
  Promise.allSettled([xr.isSessionSupported('immersive-vr'),xr.isSessionSupported('immersive-ar')]).then(results=>{
   if(!alive)return
   setXrSupport({vr:results[0].status==='fulfilled'&&results[0].value===true,ar:results[1].status==='fulfilled'&&results[1].value===true,checked:true})
  })
  return()=>{alive=false}
 },[])
 ...
 useEffect(()=>{
  ...
  const addCity=()=>{for(let x=-5;x<=5;x++)for(let z=-5;z<=5;z++){if(Math.abs(x)%2===0||Math.abs(z)%2===0)continue;const h=3+((Math.abs(x*17+z*13))%10);const b=new THREE.Mesh(new THREE.BoxGeometry(5,h,5),new THREE.MeshStandardMaterial({color:new THREE.Color().setHSL(((x+z+12)%12)/12,.35,.22)}));b.position.set(x*8,h/2,z*8);b.castShadow=true;b.receiveShadow=true;group.add(b)}const roadMat=new THREE.MeshStandardMaterial({color:0x111217,roughness:1});for(let i=-5;i<=5;i+=2){const r1=new THREE.Mesh(new THREE.BoxGeometry(2,.05,100),roadMat);r1.position.set(i*8,.03,0);group.add(r1);const r2=new THREE.Mesh(new THREE.BoxGeometry(100,.05,2),roadMat);r2.position.set(0,.03,i*8);group.add(r2)}const marker=new THREE.Mesh(new THREE.CylinderGeometry(.8,.8,3,16),new THREE.MeshStandardMaterial({color:0x4fe3ff,emissive:0x123b44}));marker.position.set(0,1.5,-8);group.add(marker)}
  ...
  return()=>{renderer.setAnimationLoop(null);removeEventListener('keydown',keydown);removeEventListener('keyup',keyup);removeEventListener('resize',resize);rendererRef.current=null;renderer.dispose();scene.traverse(o=>{const m=o as THREE.Mesh;if(m.geometry)m.geometry.dispose();const mat=m.material;if(Array.isArray(mat))mat.forEach(x=>x.dispose());else mat?.dispose?.()});if(renderer.domElement.parentNode===host)host.removeChild(renderer.domElement)}
 },[mode])

 const enterXR=async(kind:XRMode)=>{
  const xr=(navigator as any).xr;const renderer=rendererRef.current
  if(!xr||!renderer){setStatus('XR is not available in this browser. Desktop/mobile StreetVerse remains active.');return}
  try{
   ...
   const session=await xr.requestSession(kind,init);sessionRef.current=session;setXrActive(kind);setStatus(kind==='immersive-vr'?'StreetVerse VR session active. Move safely and keep your play area clear.':'StreetVerse AR session active. Digital StreetVerse can now share your physical view.')
   session.addEventListener('end',()=>{sessionRef.current=null;setXrActive(null);setStatus('XR session ended. Desktop/mobile StreetVerse remains active.')},{once:true})
 ...
 const exitXR=async()=>{try{await sessionRef.current?.end()}catch(error){console.error('StreetVerse XR exit failed',error)}}
 ...
 return <div style={{position:'fixed',inset:0,zIndex:10080,background:'#010105',color:'#fff',fontFamily:'monospace'}}><div style={{position:'absolute',top:10,left:10,right:10,zIndex:2,display:'flex',gap:6,flexWrap:'wrap',alignItems:'center'}}><button style={button} onClick={onClose}>← Exit</button>{(Object.keys(modes) as Mode[]).map(m=><button key={m} style={{...button,background:m===mode?'#164b5a':'#101b35'}} onClick={()=>{setMode(m);setStatus(modes[m].description)}}>{modes[m].label}</button>)}{xrActive?<button style={{...button,background:'#5a1b2b',borderColor:'#ff7899'}} onClick={exitXR}>EXIT XR</button>:<><button disabled={!xrSupport.vr} style={{...button,opacity:xrSupport.vr?1:.45}} onClick={()=>enterXR('immersive-vr')}>ENTER VR</button><button disabled={!xrSupport.ar} style={{...button,opacity:xrSupport.ar?1:.45}} onClick={()=>enterXR('immersive-ar')}>ENTER AR</button></>}</div><div ref={mount} style={{width:'100%',height:'100%'}}/><div style={{position:'absolute',left:12,bottom:12,maxWidth:680,zIndex:2,background:'rgba(2,2,18,.78)',border:'1px solid #78d5ff55',borderRadius:10,padding:10}}><b style={{color:'#78d5ff'}}>{modes[mode].label} • {modes[mode].gravity} • {xrActive?xrActive.toUpperCase():xrSupport.checked?(xrSupport.vr||xrSupport.ar?'XR READY':'3D FALLBACK'):'XR CHECKING'}</b><div style={{fontSize:11,color:'#bbb',marginTop:4}}>{status}</div><div style={{fontSize:9,color:'#777',marginTop:5}}>Progressive immersion gateway: standard phone/desktop 3D stays available, while compatible WebXR hardware can enter VR or AR without a separate account or world. Production multiplayer, avatar hand interaction and city streaming remain staged work.</div></div></div>
}
