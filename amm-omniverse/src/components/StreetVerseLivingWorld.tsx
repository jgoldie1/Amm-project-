...
export default function StreetVerseLivingWorld({onClose}:{onClose:()=>void}){
  ...
  useEffect(()=>{
    ...
    const enterCar=()=>{
      ...
      driver=controlled;activeCar=cars[nearest.index];activeCarIndex=nearest.index;activeCar.userData.aiTraffic=false;driveSpeed=Math.max(0,(activeCar.userData.traffic as Traffic).speed*.45);activeCar.userData.driveSpeed=driveSpeed;const f=new THREE.Vector3(Math.cos(activeCar.rotation.y),0,-Math.sin(activeCar.rotation.y));velocityX=f.x*driveSpeed;velocityZ=f.z*driveSpeed;driver.visible=false;controlled=activeCar
      setDriveStatus(`DRIVING • CAR ${String(activeCarIndex+1).padStart(2,'0')}`);setMessage('Vehicle active. W gas • S brake/reverse • A/D steer • SPACE handbrake • H horn • E exit.')
    ...
    const exitCar=()=>{
      ...
      if(collisionBoxes.some(box=>box.containsPoint(tmp.set(ex,2,ez)))||roadblockBoxes.some(box=>box.containsPoint(tmp.set(ex,2,ez)))||Array.from(responderBoxes.values()).some(box=>box.containsPoint(tmp.set(ex,2,ez)))){ex=THREE.MathUtils.clamp(car.position.x-side.x,-86,86);ez=THREE.MathUtils.clamp(car.position.z-side.z,-86,86)}
      ...
      setDriveStatus('ON FOOT');setMessage('Exited vehicle. Character control restored; your car stays parked for re-entry.')
    ...
    const collides=(x:number,z:number)=>collisionBoxes.some(box=>box.containsPoint(tmp.set(x,2,z)))||roadblockBoxes.some(box=>box.containsPoint(tmp.set(x,2,z)))||Array.from(responderBoxes.values()).some(box=>box.containsPoint(tmp.set(x,2,z)))
    const carHitsTraffic=(car:THREE.Group,x:number,z:number)=>cars.some(other=>other!==car&&Math.hypot(other.position.x-x,other.position.z-z)<3.3)||aiRacerDefs.some(r=>r.car.visible&&r.car!==car&&Math.hypot(r.car.position.x-x,r.car.position.z-z)<3.5)||Array.from(responderVehicles.values()).some(r=>Math.hypot(r.position.x-x,r.position.z-z)<3.7)
    window.dispatchEvent(new CustomEvent('tryamm:streetverse-enter',{detail:{district:'01',residents:npcs.length,vehicles:cars.length,smartIntersections:true,chicagoTransitAirportDepth:true,drivableVehicles:true,lateralGripPhysics:true,integratedAIRacers:true,surfaceGrip:true,emergencyCityReaction:true,roadblockGeometry:true,nativeResponderVehicles:true}}))
    ...
    const animate=()=>{
      ...
      if(activeCar&&!paused){
        ...
        if(!hitWorld&&!hitTraffic){activeCar.position.x=nx;activeCar.position.z=nz}else{const impact=Math.hypot(velocityX,velocityZ);velocityX*=-.2;velocityZ*=-.2;longitudinal*=-.18;lateral*=-.12;if(performance.now()-lastImpact>350){lastImpact=performance.now();window.dispatchEvent(new CustomEvent('tryamm:streetverse-drive-sound',{detail:{kind:'impact',speed:impact}}));window.dispatchEvent(new CustomEvent('tryamm:streetverse-vehicle-collision',{detail:{kind:hitTraffic?'traffic':'world',speed:impact,x:activeCar.position.x,z:activeCar.position.z}}))}}
