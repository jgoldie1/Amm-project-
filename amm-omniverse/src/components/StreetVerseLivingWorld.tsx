...
import StreetVerseOutlawFreeRoam from './StreetVerseOutlawFreeRoam'
...
export default function StreetVerseLivingWorld({onClose}:{onClose:()=>void}){
  ...
  useEffect(()=>{
    ...
    const weatherRenderer=createStreetVerseWeatherRenderer(scene,hemi,sun,{radius:100})
    let weatherGripMultiplier=1,weatherAccelMultiplier=1,weatherTrafficMultiplier=1
    const applyWeather=(state:StreetVerseWeatherState)=>{
      weatherRenderer.apply(state)
      const visual=weatherRenderer.getVisual()
      weatherGripMultiplier=visual.roadGripMultiplier
      weatherAccelMultiplier=visual.accelerationMultiplier
      weatherTrafficMultiplier=visual.trafficSpeedMultiplier
      setWeatherStatus('WEATHER • '+weatherBadge(state))
    }
    ...
    const animate=()=>{
      ...
      Array.from(responderNPCs.values()).forEach((npc,i)=>{const moving=String(npc.userData.responderStatus||'').toLowerCase()==='moving';npc.position.y=moving?Math.sin(elapsed*9+i)*.035:0})
      ...
      if(activeCar&&!paused){
        ...
        const baseSurface=surfaceAt(activeCar.position.x,activeCar.position.z),surface={...baseSurface,grip:baseSurface.grip*weatherGripMultiplier,accel:baseSurface.accel*weatherAccelMultiplier}
      ...
      const day=(Math.sin(elapsed*.03)+1)/2,weatherVisual=weatherRenderer.getVisual();sun.intensity=(1.5+day*2.4)*weatherVisual.lightMultiplier;hemi.intensity=(1.4+day*1.6)*weatherVisual.lightMultiplier;streetLights.forEach(g=>{g.visible=day<.56||weatherVisual.kind==='storm'||weatherVisual.kind==='fog'});weatherRenderer.tick(dt,controlled.position)
