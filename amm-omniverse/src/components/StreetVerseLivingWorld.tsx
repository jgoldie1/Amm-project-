...
export default function StreetVerseLivingWorld({onClose}:{onClose:()=>void}){
  ...
  useEffect(()=>{
    ...
    const exitCar=()=>{
      ...
      if(collisionBoxes.some(box=>box.containsPoint(tmp.set(ex,2,ez)))||roadblockBoxes.some(box=>box.containsPoint(tmp.set(ex,2,ez)))){ex=THREE.MathUtils.clamp(car.position.x-side.x,-86,86);ez=THREE.MathUtils.clamp(car.position.z-side.z,-86,86)}
    ...
    const collides=(x:number,z:number)=>collisionBoxes.some(box=>box.containsPoint(tmp.set(x,2,z)))||roadblockBoxes.some(box=>box.containsPoint(tmp.set(x,2,z)))
