...
export default function StreetVerseLivingWorld({onClose}:{onClose:()=>void}){
  ...
  useEffect(()=>{
    ...
    const roadMat=material(0x1c2029,0,.98),sidewalkMat=material(0x6c7073,0,.95),laneMat=new THREE.MeshBasicMaterial({color:0xe8d97a})
    for(const z of [-48,0,48]){const road=new THREE.Mesh(new THREE.BoxGeometry(190,.12,14),roadMat);road.position.set(0,.07,z);scene.add(road);for(const dz of [-9,9]){const walk=new THREE.Mesh(new THREE.BoxGeometry(190,.2,3),sidewalkMat);walk.position.set(0,.12,z+dz);scene.add(walk)}for(let x=-84;x<=84;x+=12){const mark=new THREE.Mesh(new THREE.BoxGeometry(6,.03,.16),laneMat);mark.position.set(x,.145,z);scene.add(mark)}}
    for(const x of [-48,0,48]){const road=new THREE.Mesh(new THREE.BoxGeometry(14,.12,190),roadMat);road.position.set(x,.07,0);scene.add(road);for(const dx of [-9,9]){const walk=new THREE.Mesh(new THREE.BoxGeometry(3,.2,190),sidewalkMat);walk.position.set(x+dx,.12,0);scene.add(walk)}for(let z=-84;z<=84;z+=12){const mark=new THREE.Mesh(new THREE.BoxGeometry(.16,.03,6),laneMat);mark.position.set(x,.145,z);scene.add(mark)}}
    ...
    buildingPositions.forEach(([x,z],i)=>{const h=14+((i*11)%30),w=16+(i%3)*3,d=16+((i+1)%3)*3;const building=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),material(districtColors[i%districtColors.length],.18,.68));building.position.set(x,h/2,z);building.castShadow=true;building.receiveShadow=true;scene.add(building);collisionBoxes.push(new THREE.Box3().setFromObject(building).expandByScalar(1.2));for(let y=4;y<h-1;y+=4)for(const sx of [-1,1]){const light=new THREE.Mesh(new THREE.PlaneGeometry(1.15,.65),new THREE.MeshBasicMaterial({color:i%2?0xffd58c:0x67dfff}));light.position.set(x+sx*(w/2+.01),y,z+(y%8?2.6:-2.6));light.rotation.y=sx>0?Math.PI/2:-Math.PI/2;scene.add(light)}if(i%4===0){const sign=new THREE.Mesh(new THREE.BoxGeometry(w*.72,1.1,.22),new THREE.MeshBasicMaterial({color:i%8?0x45e6ff:0xff54b4}));sign.position.set(x,3,z+d/2+.15);scene.add(sign)}})

    const shopDefs=[{name:'64 TRACK STUDIO',x:-42,z:-30,c:0xb96cff},{name:'ALL AMERICAN MARKET',x:42,z:-24,c:0x5be7ff},{name:'ALL AMERICAN NETWORK',x:38,z:38,c:0xffc95b},{name:'CHICAGO AFTER DARK',x:-38,z:38,c:0xff4f9a}]
    shopDefs.forEach(shop=>{const base=new THREE.Mesh(new THREE.BoxGeometry(15,7,11),material(shop.c,.28,.42));base.position.set(shop.x,3.5,shop.z);base.castShadow=true;scene.add(base);const glow=new THREE.PointLight(shop.c,14,24,2);glow.position.set(shop.x,5,shop.z+7);scene.add(glow);const canopy=new THREE.Mesh(new THREE.BoxGeometry(11,.55,3),new THREE.MeshBasicMaterial({color:shop.c}));canopy.position.set(shop.x,4.8,shop.z+7);scene.add(canopy);collisionBoxes.push(new THREE.Box3().setFromObject(base).expandByScalar(.8))})
    ...
    const streetLights:THREE.Group[]=[];for(let i=0;i<18;i++){const g=new THREE.Group();const pole=new THREE.Mesh(new THREE.CylinderGeometry(.12,.16,6,8),material(0x353b45,.7,.3));pole.position.y=3;g.add(pole);const lamp=new THREE.PointLight(0xffd7a0,5,18,2);lamp.position.y=6.2;g.add(lamp);const bulb=new THREE.Mesh(new THREE.SphereGeometry(.28,10,8),new THREE.MeshBasicMaterial({color:0xffd7a0}));bulb.position.y=6.2;g.add(bulb);g.position.set(i%2?10:-10,0,-76+i*9);scene.add(g);streetLights.push(g)}

    const avatar=new THREE.Group();const body=new THREE.Mesh(new THREE.CapsuleGeometry(1.05,2.2,5,10),material(0x58e8ff,.38,.32));body.position.y=2.2;body.castShadow=true;avatar.add(body);const head=new THREE.Mesh(new THREE.SphereGeometry(.78,18,14),material(0xc98e67,0,.58));head.position.y=4.25;head.castShadow=true;avatar.add(head);avatar.position.set(saved.x??START.x,0,saved.z??START.z);scene.add(avatar)
    replacePrimitiveWithStreetVerseAsset({id:'player-default',fallback:avatar,scene,position:avatar.position.clone()}).then(ok=>ok&&setAssetStatus('PLAYER GLB • 24 RESIDENTS • 20 VEHICLES'))

    const npcs:THREE.Group[]=[]
    for(let i=0;i<24;i++){const g=makeResident(i);const horizontal=i<16;const dir=(i%2===0?1:-1) as 1|-1;const route:ResidentRoute=horizontal?{axis:'x',fixed:i<8?-18:18,dir,speed:1.7+(i%5)*.18,phase:i*9}:{axis:'z',fixed:i<20?-18:18,dir,speed:1.55+(i%4)*.2,phase:i*11};g.userData.route=route;if(route.axis==='x')g.position.set(wrap(-78+route.phase),0,route.fixed);else g.position.set(route.fixed,0,wrap(-78+route.phase));scene.add(g);npcs.push(g)}
    ...
    const dogs:THREE.Group[]=[];for(let i=0;i<3;i++){const dog=new THREE.Group();const dogBody=new THREE.Mesh(new THREE.BoxGeometry(1.6,.75,.7),material(i?0x8a5b35:0x303033));dogBody.position.y=.75;dog.add(dogBody);const dogHead=new THREE.Mesh(new THREE.BoxGeometry(.65,.65,.62),material(i?0x8a5b35:0x303033));dogHead.position.set(.95,1.05,0);dog.add(dogHead);dog.position.set(-20+i*22,0,62-i*15);scene.add(dog);dogs.push(dog)}
    ...
    const clock=new THREE.Clock(),tmp=new THREE.Vector3(),desiredCam=new THREE.Vector3();let raf=0,lastSave=0,elapsed=0,lastWorldPulse=0
    const collides=(x:number,z:number)=>collisionBoxes.some(box=>box.containsPoint(tmp.set(x,2,z)))
    window.dispatchEvent(new CustomEvent('tryamm:streetverse-enter',{detail:{district:'01',residents:npcs.length,vehicles:cars.length}}))
    ...
    const animate=()=>{
      ...
      if(!paused&&(dx||dz)){const len=Math.hypot(dx,dz)||1;dx/=len;dz/=len;const speed=(keys.has('shift')?28:18)*dt;const nx=THREE.MathUtils.clamp(avatar.position.x+dx*speed,-88,88),nz=THREE.MathUtils.clamp(avatar.position.z+dz*speed,-88,88);if(!collides(nx,avatar.position.z))avatar.position.x=nx;if(!collides(avatar.position.x,nz))avatar.position.z=nz;avatar.rotation.y=Math.atan2(dx,dz)}
      npcs.forEach((npc,i)=>{const r=npc.userData.route as ResidentRoute;const travel=wrap(-88+elapsed*r.speed*r.dir+r.phase);if(r.axis==='x'){npc.position.x=travel;npc.position.z=r.fixed;npc.rotation.y=r.dir>0?Math.PI/2:-Math.PI/2}else{npc.position.x=r.fixed;npc.position.z=travel;npc.rotation.y=r.dir>0?0:Math.PI}const swing=Math.sin(elapsed*5+i)*.08;npc.children.slice(3,7).forEach((limb,j)=>{limb.rotation.x=(j%2?swing:-swing)})})
      cars.forEach(car=>{const t=car.userData.traffic as Traffic;const travel=wrap(-88+elapsed*t.speed*t.dir+t.phase);if(t.axis==='x'){car.position.x=travel;car.position.z=t.roadCenter+t.laneOffset;car.rotation.y=t.dir>0?0:Math.PI}else{car.position.x=t.roadCenter+t.laneOffset;car.position.z=travel;car.rotation.y=t.dir>0?-Math.PI/2:Math.PI/2}})
      ...
      const now=performance.now();window.dispatchEvent(new CustomEvent('tryamm:streetverse-player-position',{detail:{x:avatar.position.x,z:avatar.position.z}}));if(now-lastWorldPulse>650){const car=cars[Math.floor(elapsed)%cars.length],npc=npcs[Math.floor(elapsed*1.7)%npcs.length];window.dispatchEvent(new CustomEvent('tryamm:world-sound',{detail:{kind:'vehicle',x:car.position.x,z:car.position.z,speed:(car.userData.traffic as Traffic).speed,level:.08}}));window.dispatchEvent(new CustomEvent('tryamm:world-sound',{detail:{kind:'footstep',x:npc.position.x,z:npc.position.z,level:.035}}));lastWorldPulse=now}if(now-lastSave>900){localStorage.setItem(SAVE_KEY,JSON.stringify({x:avatar.position.x,z:avatar.position.z,visited:visitedRef.current,updatedAt:new Date().toISOString()}));lastSave=now}
      renderer.render(scene,camera);raf=requestAnimationFrame(animate)
    }
    animate()
    return()=>{window.dispatchEvent(new CustomEvent('tryamm:streetverse-exit',{detail:{district:'01'}}));cancelAnimationFrame(raf);ro.disconnect();removeEventListener('keydown',down);removeEventListener('keyup',up);renderer.dispose();renderer.domElement.remove()}
  },[paused])
