...
export default function SchoolNetworkPortal({ onClose }: Props) {
  const [tab,setTab]=useState<'network'|'jarvis'>('network')
  ...
  return <section style={{position:'fixed',inset:0,zIndex:10080,overflowY:'auto',background:'radial-gradient(circle at 20% 0,#102a43,#040712 52%)',color:'#fff',fontFamily:'Inter,system-ui,sans-serif'}}>
    <div style={{maxWidth:1180,margin:'0 auto',padding:'22px 18px 110px'}}>
      ...
      <article style={{...card,marginTop:18,borderColor:'#e8b94466',background:'linear-gradient(145deg,#171207,#07101c)'}}>
        ...
        <p style={{color:'#b7bdc8',lineHeight:1.6,marginBottom:8}}>Students can use TRYAMM as a companion portal for study planning, labs, portfolio building and approved remote work. Greenville University is shown here as a student-selected campus context; this does not claim an official institutional partnership unless a formal agreement is separately verified.</p>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}><span style={{fontSize:10,border:'1px solid #4fe3ff55',borderRadius:999,padding:'5px 8px'}}>School-first schedule</span><span style={{fontSize:10,border:'1px solid #4fe3ff55',borderRadius:999,padding:'5px 8px'}}>Remote work labs</span><span style={{fontSize:10,border:'1px solid #4fe3ff55',borderRadius:999,padding:'5px 8px'}}>Portfolio evidence</span><span style={{fontSize:10,border:'1px solid #4fe3ff55',borderRadius:999,padding:'5px 8px'}}>Stubbs AI coaching</span></div>
      </article>
      ...
      <article style={{...card,marginTop:16}}><h2 style={{marginTop:0}}>AI Café education-to-income loop</h2><div style={{fontWeight:900,lineHeight:1.8,color:'#d8e2ec'}}>CAFÉ SALES + CREATOR TABLES + HOLO ADS + PRODUCT PLACEMENT + MARKETPLACE SALES → OPERATING REVENUE → STAFF/TRAINING WORK → BETTER CAFÉ/CONTENT → MORE CUSTOMERS/AD INVENTORY.</div><p style={{fontSize:11,color:'#8998aa',lineHeight:1.6}}>Paid tasks remain separate from classroom grades. Students must complete real approved work with evidence and supervision; restricted player rewards, creator liabilities and customer balances cannot be used as payroll.</p></article>
    </div>
  </section>
}
