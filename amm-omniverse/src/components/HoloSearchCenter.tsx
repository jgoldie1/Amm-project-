...
export default function HoloSearchCenter({ onClose }:{ onClose:()=>void }) {
  ...
  return <div role="dialog" aria-modal="true" aria-label="Holo Search" style={{position:'fixed',inset:0,zIndex:12500,background:'linear-gradient(180deg,#020713,#07101c)',color:'#fff',overflowY:'auto',fontFamily:'system-ui,sans-serif'}}>
    ...
    <main style={{maxWidth:980,margin:'0 auto',padding:18,display:'grid',gap:14}}>
      <section style={panel}>
        <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:12}}>{modes.map(([id,label])=><button key={id} onClick={()=>setMode(id)} style={{...chip,borderColor:mode===id?'#55eaff':'#39475d',color:mode===id?'#91f7ff':'#d9e0ea'}}>{label}</button>)}</div>
