...
const zeroEngines = (Object.keys(engineLabels) as RevenueEngine[]).map(engine => calculateEngine({
  engine, grossVolume: 0, grossRevenue: 0, providerFees: 0, refunds: 0, reserves: 0, taxesCollected: 0,
  creatorPartnerPayables: 0, infrastructureCost: 0, transactions: 0, activeCustomers: 0, currency: 'USD',
}))

const money = (n:number) => new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(n)

export default function FounderRevenueCockpit({ onClose }: Props) {
  // Until the Money Engine reporting API is connected, zero/unavailable is safer than demo revenue presented as real.
  ...
  return <div role="dialog" aria-label="Founder Revenue Cockpit" style={{position:'fixed',inset:0,zIndex:10040,overflowY:'auto',background:'radial-gradient(circle at 15% 0%,#10283a,#03050c 48%,#020208)',color:'#fff',padding:18,fontFamily:'system-ui,sans-serif'}}>
    <div style={{maxWidth:1180,margin:'0 auto'}}>
      <div style={{display:'flex',justifyContent:'space-between',gap:16,alignItems:'flex-start',marginBottom:18}}>
        <div><div style={{color:'#4fe3ff',fontWeight:900,letterSpacing:3,fontSize:11}}>TRYAMM FOUNDER OS</div><h1 style={{margin:'5px 0',fontSize:'clamp(26px,5vw,48px)'}}>Revenue Cockpit</h1><div style={{color:'#9fb0c6'}}>Know what makes money, what is owed, what costs money, and what can safely fund the next stage.</div></div>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:14}}><span style={{padding:'7px 10px',borderRadius:999,background:'#3c310b',color:'#ffe47a',fontWeight:900}}>RELEASE {snapshot.releaseGate}</span><span style={{padding:'7px 10px',borderRadius:999,background:'#111a26',color:'#9fb0c6'}}>DATA: {snapshot.source.replaceAll('_',' ').toUpperCase()}</span></div>
      {snapshot.source==='unavailable' && <div role="status" style={{border:'1px solid #e8b94466',background:'#2b210b99',padding:14,borderRadius:14,marginBottom:16,color:'#ffe8a3'}}>Live Money Engine reporting is not connected yet. Values remain zero instead of showing invented revenue. Connect the authoritative reporting API before using this cockpit for financial decisions.</div>}
      ...
      <section style={{background:'#070d16dd',border:'1px solid #17283a',borderRadius:18,padding:16,marginBottom:18}}><h2 style={{marginTop:0}}>Revenue engines</h2><div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',minWidth:760}}><thead><tr>{['Engine','GMV','Revenue','Payables','Infra','Contribution','Customers'].map(x=><th key={x} style={{textAlign:'left',padding:'9px 7px',borderBottom:'1px solid #243244',fontSize:10,color:'#8297ae'}}>{x}</th>)}</tr></thead><tbody>{ranked.map(e=><tr key={e.engine}><td style={{padding:9,fontWeight:850}}>{engineLabels[e.engine]}</td><td>{money(e.grossVolume)}</td><td>{money(e.grossRevenue)}</td><td>{money(e.creatorPartnerPayables)}</td><td>{money(e.infrastructureCost)}</td><td>{money(e.contributionMargin)}</td><td>{e.activeCustomers}</td></tr>)}</tbody></table></div></section>

      <section style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:12}}>
        <div style={{background:'#07131a',border:'1px solid #174151',borderRadius:18,padding:16}}><h3>Founder decision loop</h3><div style={{lineHeight:1.8,color:'#b8c6d6'}}>ACQUIRE → ENGAGE → CREATE → TRANSACT → RETAIN → MEASURE → REINVEST → CREATE AGAIN</div></div>
        <div style={{background:'#111008',border:'1px solid #544617',borderRadius:18,padding:16}}><h3>Money safety</h3><div style={{lineHeight:1.8,color:'#d6ccb0'}}>Money Engine → double-entry ledger → Internal Blockchain evidence → Quantum Money Sandbox → audit. Customer principal and regulated assets are never counted as TRYAMM operating revenue.</div></div>
      </section>
    </div>
  </div>
}
