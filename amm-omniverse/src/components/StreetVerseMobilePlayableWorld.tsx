...
export default function StreetVerseMobilePlayableWorld({onClose}:{onClose:()=>void}){
 ...
 useEffect(()=>{let raf=0,last=performance.now(),lastSave=0;const tick=(now:number)=>{const dt=Math.min(.05,(now-last)/1000);last=now;const h=held.current;let dx=(h.right?1:0)-(h.left?1:0),dy=(h.down?1:0)-(h.up?1:0);if(dx||dy){const len=Math.hypot(dx,dy)||1;dx/=len;dy/=len;const moveSpeed=vehicleRef.current?40:24;setPos(p=>({x:Math.max(4,Math.min(96,p.x+dx*moveSpeed*dt)),y:Math.max(8,Math.min(92,p.y+dy*moveSpeed*dt))}))}if(now-lastSave>800){lastSave=now;setPos(p=>{localStorage.setItem(SAVE_KEY,JSON.stringify({...p,vehicle:vehicleRef.current,updatedAt:new Date().toISOString()}));window.dispatchEvent(new CustomEvent('tryamm:streetverse-player-position',{detail:{x:(p.x-50)*1.8,z:(p.y-50)*1.8,speed:(dx||dy)?(vehicleRef.current?12:6):0,mobileSafeMode:true,htmlCity:true,vehicle:vehicleRef.current,vehicleType:vehicleRef.current?'car':undefined}}));return p})}raf=requestAnimationFrame(tick)};raf=requestAnimationFrame(tick);return()=>cancelAnimationFrame(raf)},[])
 useEffect(()=>{for(const m of MISSIONS){const d=Math.hypot(pos.x-m.x,pos.y-m.y);if(d<6){setMessage(`Reached ${m.label} • mission zone active${vehicle?' • arrived by vehicle':''}.`);window.dispatchEvent(new CustomEvent('tryamm:streetverse-mobile-mission-zone',{detail:{id:m.id,label:m.label,vehicle}}));return}}setMessage(vehicle?'DRIVE MODE • use the arrows to cruise Chicago • EXIT VEHICLE returns to walking.':'StreetVerse City is active • walk the avenue and reach a mission beacon.')},[pos,vehicle])
 useEffect(()=>{
  const onVehicle=(event:Event)=>{
   const detail=(event as CustomEvent<{entered?:boolean}>).detail||{}
   const next=typeof detail.entered==='boolean'?detail.entered:!vehicleRef.current
   vehicleRef.current=next;setVehicle(next)
   setMessage(next?'Vehicle entered • DRIVE MODE active • arrows/WASD steer through Chicago.':'Vehicle exited • WALK MODE restored.')
   window.dispatchEvent(new CustomEvent('tryamm:streetverse-vehicle-controlled',{detail:{entered:next,mobileSafeMode:true,htmlCity:true,vehicleType:next?'car':undefined,x:(pos.x-50)*1.8,z:(pos.y-50)*1.8}}))
  }
 ...
 useEffect(()=>{
  ...
  const set=(e:KeyboardEvent,v:boolean)=>{const raw=e.key.toLowerCase();if(raw==='e'&&v){e.preventDefault();window.dispatchEvent(new CustomEvent('tryamm:streetverse-vehicle-interact',{detail:{entered:!vehicleRef.current,source:'keyboard'}}));return}const k=map[raw];if(k){e.preventDefault();held.current[k]=v}}
 ...
 return <div data-streetverse-html-city="true" style={{position:'fixed',inset:0,zIndex:18000,background:'#07101b',color:'#fff',fontFamily:'system-ui',overflow:'hidden'}}>
  ...
  <main style={{position:'absolute',left:0,right:0,top:64,bottom:0,overflow:'hidden',background:'linear-gradient(#65a6c9 0%,#98cbe3 35%,#d9a979 36%,#19232b 67%,#070b10 100%)'}}>
   ...
   <div style={{position:'absolute',right:12,bottom:18,zIndex:35,maxWidth:170,padding:9,borderRadius:12,background:'#030914df',border:'1px solid #34566d',fontSize:10,lineHeight:1.35}}>HTML CITY MODE<br/><b style={{color:'#8effb7'}}>No WebGL required.</b><br/>People • traffic • dog • missions • Reel • {vehicle?'driving active':'tap ENTER VEHICLE to drive'}.</div>
  </main>
 </div>
}
