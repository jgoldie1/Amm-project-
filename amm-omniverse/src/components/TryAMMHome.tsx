...
const statusStyle=(kind:'LIVE'|'BETA'|'COMING SOON'|'VERIFIED'|'NEW')=>({
  display:'inline-flex',alignItems:'center',borderRadius:999,padding:'3px 7px',fontSize:8,fontWeight:900,letterSpacing:1,
  border:`1px solid ${kind==='LIVE'?'#ff5b72':kind==='VERIFIED'?'#62e89b':kind==='NEW'?cyan:kind==='BETA'?gold:'#596174'}`,
  color:kind==='LIVE'?'#ff8da0':kind==='VERIFIED'?'#8fffc1':kind==='NEW'?cyan:kind==='BETA'?'#ffe08a':'#9ca5b5',
  background:kind==='LIVE'?'#2a0b13':kind==='VERIFIED'?'#092319':kind==='NEW'?'#071d27':kind==='BETA'?'#2a210b':'#111522'
} as const)
...
export default function TryAMMHome(){
  ...
  const cards=[
    ...
    {icon:'◈',title:'Middleverse',copy:'The orchestration layer connecting Holoverse, Stubbs AI, Workforce, Commerce, LIVE, Safety and Living Worlds.',status:'NEW' as const,action:()=>openGlobal('__showMiddleverseWorkstation')},
    {icon:'🎧',title:'AI Workforce + WFH',copy:'Work-from-home agent desktop with approved scripts, rebuttals, DNC checks, interaction logging, QA and escalation.',status:'NEW' as const,action:()=>openGlobal('__showMiddleverseWorkstation')},
    ...
    <div style={{maxWidth:1180,margin:'0 auto',padding:'22px 18px 110px'}}>
      <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,marginBottom:34}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap',justifyContent:'flex-end'}}>
          <span style={statusStyle('NEW')}>NEW HOME • AUG 16</span>
          <span style={statusStyle('VERIFIED')}>SAFETY ON</span>
          <button onClick={openInstall} style={{border:`1px solid ${gold}88`,background:'#201807',color:'#ffe49b',borderRadius:999,padding:'9px 13px',fontWeight:900,cursor:'pointer'}}>⬇ INSTALL APP</button>
      ...
      <section style={{display:'grid',gridTemplateColumns:'minmax(0,1.25fr) minmax(260px,.75fr)',gap:22,alignItems:'stretch',marginBottom:24}} className="tryamm-home-hero">
        <div style={{border:'1px solid #20364d',borderRadius:28,padding:'clamp(24px,5vw,54px)',background:'linear-gradient(145deg,rgba(9,20,34,.95),rgba(5,7,15,.9))',boxShadow:'0 24px 80px #0008',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{display:'flex',gap:7,flexWrap:'wrap',marginBottom:18}}><span style={statusStyle('LIVE')}>LIVE</span><span style={statusStyle('NEW')}>MIDDLEVERSE</span><span style={statusStyle('NEW')}>AI WORKFORCE</span><span style={statusStyle('BETA')}>COMMERCE OS</span></div>
          <div style={{color:gold,fontSize:12,fontWeight:900,letterSpacing:3,marginBottom:10}}>THE ALL-IN-ONE DIGITAL WORLD</div>
          <h1 style={{fontSize:'clamp(38px,7vw,76px)',lineHeight:.96,letterSpacing:-3,margin:'0 0 18px',maxWidth:760}}>Watch. Play. Shop. Work. Build. Earn. <span style={{color:cyan}}>Live.</span></h1>
          <p style={{color:'#b8c6d8',fontSize:'clamp(15px,2vw,19px)',lineHeight:1.55,maxWidth:720,margin:'0 0 24px'}}>TRYAMM connects LIVE creators, open worlds, Stubbs AI, Middleverse orchestration, work-from-home operations, commerce, music, business, accessibility and local services through one Living Worlds Passport.</p>
          <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
            ...
            <button onClick={()=>openGlobal('__showMiddleverseWorkstation')} style={{border:`1px solid ${cyan}88`,borderRadius:14,padding:'14px 20px',background:'#071d27',color:cyan,fontWeight:950,cursor:'pointer'}}>◈ OPEN MIDDLEVERSE</button>
            <button onClick={openInstall} style={{border:`1px solid ${gold}88`,borderRadius:14,padding:'14px 20px',background:'#201807',color:'#ffe49b',fontWeight:950,cursor:'pointer'}}>⬇ INSTALL TRYAMM</button>
        ...
        <aside style={{border:`1px solid ${gold}55`,borderRadius:28,padding:24,background:'linear-gradient(180deg,#16120b,#080a12)',display:'flex',flexDirection:'column',justifyContent:'space-between'}}>
          <div>
            ...
            <h2 style={{fontSize:27,margin:'8px 0 10px'}}>One identity. One connected world.</h2>
            <p style={{fontSize:13,color:'#a7b0bf',lineHeight:1.55}}>Your Passport connects creator identity, purchases, worlds, LIVE participation, safety settings, workforce sessions and future services without rebuilding your account for every experience.</p>
          </div>
          <div style={{borderTop:'1px solid #2e2a20',paddingTop:16,marginTop:18,display:'grid',gap:9,fontSize:12,color:'#d5dae3'}}>
            <div>◈ Middleverse handoffs</div><div>🎧 AI Workforce + escalation</div><div>🛡 Report • Block • Mute</div><div>⏸ Protected LIVE pause</div><div>🛒 Unified Commerce OS</div><div>♿ Accessibility-first</div>
          </div>
        </aside>
      </section>

      <section aria-labelledby="explore-tryamm">
        <div style={{display:'flex',alignItems:'end',justifyContent:'space-between',gap:12,margin:'34px 0 14px'}}><div><div style={{color:cyan,fontSize:10,fontWeight:900,letterSpacing:3}}>EXPLORE THE NEW TRYAMM</div><h2 id="explore-tryamm" style={{margin:'5px 0 0',fontSize:28}}>One ecosystem, many ways in</h2></div><button onClick={()=>enter('marketplace')} style={{background:'transparent',border:0,color:'#9acfff',fontWeight:800,cursor:'pointer'}}>Explore Marketplace →</button></div>
