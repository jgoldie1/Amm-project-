...
import GlobalMonthlyLaunchPanel from './GlobalMonthlyLaunchPanel'
...
export default function TryAMMHome(){
  ...
  const ecosystem=[
    ...
    {icon:'✦',title:'HoloGPT / Stubbs AI',copy:'Context-aware AI navigation, creation, translation, mission help, support routing and accessibility assistance across TRYAMM.',action:()=>openGlobal('__showBennie'),label:'BETA'},
    ...
    <div style={{maxWidth:1240,margin:'0 auto',padding:'18px 18px 120px'}}>
      <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap',padding:'5px 0 24px'}}>
        <div style={{display:'flex',alignItems:'center',gap:11}}>
          ...
          <div><div style={{fontWeight:1000,letterSpacing:2.5,fontSize:20}}>TRYAMM</div><div style={{fontSize:9,color:'#8fa1b8',letterSpacing:2}}>GLOBAL LIVING WORLDS • GAMEVERSE • HOLOVERSE • AI</div></div>
        </div>
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          <span style={badge('GLOBAL','gold')}>GLOBAL</span>
          <span style={badge('MONTHLY GAME RELEASE','cyan')}>MONTHLY GAME RELEASE</span>
      ...
      <section className="tryamm-world-hero" style={{display:'grid',gridTemplateColumns:'minmax(0,1.45fr) minmax(270px,.55fr)',gap:18,alignItems:'stretch'}}>
        <div style={{position:'relative',overflow:'hidden',border:'1px solid #21415a',borderRadius:30,padding:'clamp(28px,5vw,58px)',background:'linear-gradient(145deg,rgba(8,22,38,.97),rgba(5,7,15,.92))',boxShadow:'0 28px 90px #0009'}}>
          ...
          <div style={{display:'flex',gap:7,flexWrap:'wrap',marginBottom:18}}><span style={badge('STREETVERSE MONTH 1','gold')}>STREETVERSE MONTH 1</span><span style={badge('AR • VR • MR','cyan')}>AR • VR • MR</span><span style={badge('GLOBAL + DIASPORA','cyan')}>GLOBAL + DIASPORA</span><span style={badge('LIVE','red')}>LIVE</span></div>
          <div style={{fontSize:11,color:gold,fontWeight:950,letterSpacing:4,marginBottom:10}}>ONE PASSPORT. MANY WORLDS. GLOBAL RELEASES.</div>
          ...
          <p style={{margin:'0 0 25px',maxWidth:790,color:'#bdcada',fontSize:'clamp(15px,2vw,20px)',lineHeight:1.58}}>Start with StreetVerse, then discover a new featured TRYAMM game every release month. HoloGPT stays available as a context-aware guide across games, music, movies, HoloArena, University, Marketplace and global experiences.</p>
          <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
            <button onClick={()=>openGlobal('__showGameVerse')} style={{border:0,borderRadius:14,padding:'15px 21px',background:`linear-gradient(135deg,${cyan},#66a6ff)`,color:'#031018',fontWeight:1000,cursor:'pointer',boxShadow:`0 0 30px ${cyan}33`}}>🎮 PLAY STREETVERSE / GAMEVERSE</button>
            ...
            <button onClick={()=>openGlobal('__showBennie')} style={{border:'1px solid #9b77ff88',borderRadius:14,padding:'15px 21px',background:'#171029',color:'#cdbaff',fontWeight:1000,cursor:'pointer'}}>✦ ASK HOLOGPT</button>
          </div>
        </div>
        ...
        <aside style={{border:`1px solid ${gold}55`,borderRadius:30,padding:24,background:'linear-gradient(180deg,#17120a,#080a12)',display:'flex',flexDirection:'column',justifyContent:'space-between'}}>
          <div><div style={{fontSize:52}}>🦁</div><div style={{fontSize:10,color:gold,fontWeight:950,letterSpacing:3,marginTop:10}}>TRYAMM GLOBAL PASSPORT</div><h2 style={{fontSize:28,margin:'8px 0 10px'}}>Your identity travels with you.</h2><p style={{fontSize:13,color:'#a9b4c3',lineHeight:1.6}}>One identity for progression, World Memory, safety, accessibility, creator relationships and regional experiences across the TRYAMM universe.</p></div>
          <div style={{display:'grid',gap:9,borderTop:'1px solid #31291e',paddingTop:16,marginTop:18,fontSize:12,color:'#d5dbe4'}}><div>🌎 US • Canada • Mexico</div><div>🌍 Africa + Diaspora</div><div>🇮🇳 India</div><div>🌏 Asia-Pacific</div><div>🇬🇧 UK + Europe</div><div>🎮 Monthly Featured Games</div><div>✦ HoloGPT Guidance</div></div>
        </aside>
      </section>

      <GlobalMonthlyLaunchPanel />
      ...
      <section style={{marginTop:30,border:'1px solid #193143',borderRadius:22,padding:20,background:'#07101a'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:16}}>
          {[['ONE PASSPORT','Games, worlds, LIVE, commerce, creators and workforce are designed around one connected identity.'],['GLOBAL ROUTING','Regional selection prepares TRYAMM for localized language, eligibility, pricing, events and availability rather than assuming every provider works everywhere.'],['MONTHLY RELEASE ENGINE','StreetVerse launches first, then one featured game is promoted every release month while earlier worlds stay playable.'],['HOLOGPT OVERLAY','AI guidance follows the user across supported surfaces and can minimize so it never blocks critical controls.'],['TRUST + SAFETY','Report, block, mute, appeals and protected LIVE controls remain part of the shared platform.']].map(([title,copy])=><div key={title}><div style={{color:gold,fontSize:10,fontWeight:950,letterSpacing:1}}>{title}</div><div style={{color:'#9eacbd',fontSize:12,lineHeight:1.55,marginTop:6}}>{copy}</div></div>)}
        </div>
      </section>

      <footer style={{marginTop:32,textAlign:'center',fontSize:10,color:'#68778b',lineHeight:1.7}}>TRYAMM GLOBAL LIVING WORLDS • StreetVerse-first monthly release cycle • Regional/provider availability remains truth-gated until production proof.</footer>
    </div>
