...
const RND_SYSTEMS=[
  {name:'Global Routing Fabric',status:'FOUNDATION',detail:'Communications · network · AI · payments · world routing'},
  {name:'TRYAMM Robotics',status:'SIMULATION FIRST',detail:'Physical actions remain safety-gated and locally stoppable'},
  {name:'Blue Energy™',status:'R&D',detail:'Battery · power · thermal · runtime · energy-cost telemetry'},
  {name:'OmniGrid / OmniCompute',status:'R&D',detail:'Compute + energy telemetry and workload optimization'},
  {name:'Quantum Generator',status:'UNVERIFIED R&D',detail:'Theory → simulation → bench test → independent replication → certification'},
] as const
  ...
  return <div role="dialog" aria-modal="true" aria-label="TRYAMM Command Nexus Control Plane" style={{position:'fixed',inset:0,zIndex:12200,background:'radial-gradient(circle at 80% 10%,#14304a,#050914 42%,#02030a)',color:'#fff',overflowY:'auto',fontFamily:'system-ui,sans-serif'}}>
    <header style={{position:'sticky',top:0,zIndex:3,padding:'14px 18px',background:'#040814ed',borderBottom:'1px solid #4fe3ff44',backdropFilter:'blur(14px)'}}>
      <div style={{maxWidth:1200,margin:'0 auto',display:'grid',gap:12}}>
        <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center'}}><div><div style={{fontSize:10,color:'#4fe3ff',letterSpacing:3}}>TRYAMM HOLO CONTROL CENTER</div><h1 style={{margin:'3px 0'}}>Command Nexus</h1><div style={{fontSize:12,opacity:.65}}>One organized launcher for AI, Holoverse, games, creator tools, services, infrastructure and accessibility.</div></div><div style={{display:'flex',gap:8,flexWrap:'wrap',justifyContent:'flex-end'}}><button style={button} onClick={refresh} disabled={loading} aria-busy={loading}>{loading?'CHECKING…':'REFRESH'}</button><button style={button} onClick={onClose}>CLOSE</button></div></div>
        <input aria-label="Search TRYAMM systems" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search HoloGPT, Holoverse, StreetVerse, movies, reels, ride, accessibility…" style={{width:'100%',boxSizing:'border-box',minHeight:44,borderRadius:12,border:'1px solid #31536b',background:'#07111d',color:'#fff',padding:'0 13px',fontSize:14,outline:'none'}}/>
      </div>
    </header>
    <main style={{maxWidth:1200,margin:'0 auto',padding:18,display:'grid',gap:14}}>
      <section style={{...panel,borderColor:statusColor(data?.overall)}}><div style={{display:'flex',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}><div><div role="status" aria-live="polite" aria-atomic="true" style={{fontSize:11,color:statusColor(data?.overall)}}>SYSTEM STATUS · {(data?.overall||'UNVERIFIED').toUpperCase()}</div><h2 style={{margin:'5px 0'}}>Release Truth</h2><div style={{fontSize:11,opacity:.65}}>A BETA/GATED label means the route exists but still needs provider, device or production proof.</div></div><div style={{fontFamily:'monospace',fontSize:11,opacity:.75}}>branch: {data?.deployment?.branch||'unknown'}<br/>sha: {data?.deployment?.commitSha?.slice(0,12)||'unverified'}<br/>project: {data?.deployment?.project||'unverified'}</div></div>{error&&<div role="alert" style={{color:'#ff8795',marginTop:8}}>{error}</div>}</section>
      <section style={panel} aria-label="Release 1 infrastructure and research status"><div style={{fontSize:10,color:'#4fe3ff',fontWeight:950,letterSpacing:2}}>ROBOTICS + ENERGY + GLOBAL INFRASTRUCTURE</div><div style={{fontSize:11,opacity:.58,margin:'3px 0 10px'}}>These labels are capability truth, not marketing claims. Quantum Generator and physical energy hardware remain evidence-gated R&D.</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:8}}>{RND_SYSTEMS.map(item=><article key={item.name} style={{...panel,padding:12}}><div style={{fontSize:9,color:'#ffb86b',fontWeight:900}}>{item.status}</div><strong style={{display:'block',marginTop:5}}>{item.name}</strong><div style={{fontSize:11,opacity:.62,marginTop:5,lineHeight:1.4}}>{item.detail}</div></article>)}</div><div style={{fontSize:11,color:'#e8b944',marginTop:10}}>PHYSICAL RELEASE GATE: measurable evidence + safety validation + required certification. Financial truth stays server-authoritative.</div></section>
