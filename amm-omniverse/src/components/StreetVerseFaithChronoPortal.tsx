...
export default function StreetVerseFaithChronoPortal(){
  ...
  useEffect(()=>{const refresh=()=>setState(read());window.dispatchEvent(new CustomEvent('tryamm:streetverse-enter',{detail:{source:'StreetVerseFaithChronoPortal'}}));window.addEventListener('tryamm:chrono-state',refresh);window.addEventListener('storage',refresh);return()=>{window.removeEventListener('tryamm:chrono-state',refresh);window.removeEventListener('storage',refresh)}},[])
  const resumable=Boolean(state.scenarioId||state.runId)
  return <div aria-label="StreetVerse Faith Chrono portal" style={{position:'fixed',right:12,bottom:14,zIndex:17020,display:'grid',gap:7,justifyItems:'end'}}>{resumable&&<button type="button" onClick={()=>window.dispatchEvent(new CustomEvent('tryamm:chrono-resume'))} style={portal}><span style={{fontSize:18}}>⏳</span><span><b>TIME MACHINE</b><br/><small>{state.name||'Resume Faith Chrono'} · checkpoint {state.checkpoint||0}</small></span></button>}<button type="button" onClick={()=>{window.location.href='/ethiopian-bible'}} style={faith}>📖 ETHIOPIAN BIBLE · FAITH WORLD</button></div>
}
