...
export default function HoloLabGateway({onClose}:Props){
  ...
  return <div role="dialog" aria-modal="true" aria-label="Holo Lab" style={{position:'fixed',inset:0,zIndex:12300,background:'radial-gradient(circle at 30% 10%,#14293d,#060914 48%,#020309)',color:'#fff',overflowY:'auto',fontFamily:'system-ui,sans-serif'}}>
    ...
    <main style={{maxWidth:1100,margin:'0 auto',padding:18,display:'grid',gap:14}}>
      <section style={panel}><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><span style={pill}>HTTPS {support.secure?'READY':'REQUIRED'}</span><span style={pill}>WEBXR {support.webxr?'DETECTED':'FALLBACK'}</span><span style={pill}>CHRONO RECONSTRUCTION SOURCE-GATED</span><span style={pill}>HIGH-RISK AUTO-PROMOTION BLOCKED</span></div><p style={{lineHeight:1.55,opacity:.72}}>{note}</p></section>
      ...
      <section style={{...panel,borderColor:'#8b7435'}}><strong>Faith Chrono integrity boundary</strong><p style={{opacity:.72,lineHeight:1.55}}>Historical Bible-world scenes may combine documented evidence with clearly labeled reconstruction. Scripture text must remain source-identified. Generated historical-character dialogue is educational simulation. Prayer/reflection tools may organize scripture and journaling but do not represent generated text as direct speech from Yahavah or the Ruach.</p><button onClick={()=>nav('/ethiopian-bible')} style={btn}>OPEN FAITH CHRONO LAB</button></section>
