...
export default function AniyahPayCenter({onClose}:Props){
  ...
  return <div role="dialog" aria-label="Aniyah Pay" style={{position:'fixed',inset:0,zIndex:10140,background:'radial-gradient(circle at 50% 0,#132b42 0,#02060f 48%)',color:'#eefcff',fontFamily:'monospace',overflowY:'auto'}}>
    <div style={{maxWidth:920,margin:'0 auto',padding:'max(16px,env(safe-area-inset-top)) 16px 42px'}}>
      ...
      <section style={{...box,marginTop:14,borderColor:'#4fe3ff55'}}><div style={{fontSize:10,color:'#4fe3ff',fontWeight:900,letterSpacing:2}}>PAYMENT ROUTER</div><h1 style={{margin:'6px 0 8px',fontSize:'clamp(24px,5vw,42px)',lineHeight:1}}>Stripe + Flutterwave + Paystack</h1><div style={{fontSize:11,color:'#b7cad5',lineHeight:1.7}}>Aniyah selects an approved provider by corridor. The game economy and real-money economy stay separate. Live provider submission stays server-side and compliance-gated.</div></section>
      ...
      <section style={{...box,marginTop:12,fontSize:10,color:'#8fa6b4',lineHeight:1.7}}>Flow: STREETVERSE MISSION → HOLO FON → ANIYAH PAY → SEND / REQUEST / SPLIT → IDENTITY + RISK GATES → PROVIDER ROUTER → STRIPE / FLUTTERWAVE / PAYSTACK → LEDGER → RECEIPT → COMMAND NEXUS.</section>
    </div>
  </div>
}
