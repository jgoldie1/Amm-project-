...
type MissionComplete={id?:string;missionId?:string;label?:string;xp?:number;timeMs?:number;financialReward?:boolean;progressState?:Record<string,unknown>;[key:string]:unknown}
...
export default function StreetVerseProgressSync(){
  ...
  useEffect(()=>{
    ...
    const onMissionComplete=async(event:Event)=>{
      ...
      const id=String(d.id||d.missionId||'').trim(),xp=Math.round(Number(d.xp||0))
      ...
      const extraState=d.progressState&&typeof d.progressState==='object'?d.progressState:{}
      const progressState={...extraState,lastMissionLabel:String(d.label||id),lastMissionTimeMs:Number.isFinite(Number(d.timeMs))?Number(d.timeMs):null,lastMissionId:id,lastSyncedAt:new Date().toISOString()}
      ...
      window.dispatchEvent(new CustomEvent('tryamm:streetverse-progress-synced',{detail:{missionId:id,earnedXp:xp,xp:totalXp,level:Number(row.level||1),missionsCompleted:Number(row.missions_completed||0),progressState:row.progress_state||progressState,financialReward:false}}))
    }
