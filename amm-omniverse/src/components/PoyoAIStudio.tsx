...
export default function PoyoAIStudio({onClose}:Props){
  ...
  useEffect(()=>{if(!task?.task_id||['finished','failed'].includes(String(task.status)))return;const id=window.setInterval(refresh,5000);return()=>window.clearInterval(id)},[task?.task_id,task?.status])
  const files=Array.isArray(task?.files)?task!.files:[]
  const panel:React.CSSProperties={background:'#09111ce8',border:'1px solid #223a4c',borderRadius:18,padding:14}
  const btn=(active=false):React.CSSProperties=>({background:active?'linear-gradient(135deg,#0f3546,#252036)':'#0a1119',border:active?'1px solid #4fe3ff':'1px solid #293846',color:active?'#c8f8ff':'#d8e1e8',borderRadius:11,padding:'9px 11px',cursor:'pointer',fontWeight:850})
  return <div role="dialog" aria-label="TRYAMM Poyo AI Studio" style={{position:'fixed',inset:0,zIndex:10070,overflowY:'auto',background:'radial-gradient(circle at 12% 8%,#0b3040,#03050b 42%,#010207)',color:'#fff',fontFamily:'Inter,system-ui,sans-serif'}}>
    <div style={{maxWidth:1180,margin:'0 auto',padding:'18px 14px 80px'}}>
      <header style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',marginBottom:14}}><div><div style={{color:'#4fe3ff',fontSize:10,fontWeight:950,letterSpacing:3}}>TRYAMM CREATOR INTELLIGENCE</div><h1 style={{margin:'4px 0',fontSize:'clamp(26px,5vw,44px)'}}>Poyo AI Studio MAX</h1><div style={{color:'#98aabc'}}>Multi-engine image • video • music • voice • chat • 3D • media tools.</div></div><button onClick={onClose} aria-label="Close" style={{...btn(),fontSize:22,width:44,height:44,padding:0}}>×</button></header>
