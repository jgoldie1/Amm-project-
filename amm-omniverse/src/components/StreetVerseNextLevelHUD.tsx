...
export default function StreetVerseNextLevelHUD({district='CHICAGO • DISTRICT 01',assetStatus='WORLD ACTIVE',visited=0,totalMissions=4}:Props){
 ...
 const launch=(names:string[],label:string)=>{
   ...
   setStatus(`${label} is installed in the HUD but its runtime launcher is not connected on this build.`)
 }
 ...
 return <aside aria-label="StreetVerse control deck" style={{position:'absolute',right:10,top:'max(10px,env(safe-area-inset-top))',zIndex:30,width:'min(378px,calc(100vw - 20px))',maxHeight:'calc(100dvh - 20px)',overflowY:'auto',background:'linear-gradient(165deg,#04131cf5,#0b0718f2 62%,#051019f5)',border:'1px solid #4fe3ff66',borderRadius:20,padding:14,backdropFilter:'blur(16px)',boxShadow:'0 20px 70px #000c',fontFamily:'monospace',color:'#e8fbff'}}>
  <header style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'flex-start'}}>
   <div><div style={{fontSize:9,letterSpacing:2.4,color:'#69ebff',fontWeight:900}}>STREETVERSE • CONTROL DECK</div><div style={{fontSize:14,fontWeight:950,marginTop:4}}>{district}</div><div style={{display:'flex',alignItems:'center',gap:6,marginTop:6,fontSize:9,color:'#8fb6c3'}}><span aria-hidden style={{width:7,height:7,borderRadius:99,background:'#78ffb4',boxShadow:'0 0 10px #78ffb4'}}/> {assetStatus}</div></div>
   <button aria-label="Minimize StreetVerse control deck" onClick={()=>setOpen(false)} style={{minWidth:44,minHeight:44,border:'1px solid #294553',borderRadius:12,background:'#07141ddd',color:'#a9c3cc',fontSize:19,cursor:'pointer'}}>×</button>
  </header>

  <div style={{marginTop:12,padding:10,background:'#06141fcc',border:'1px solid #183849',borderRadius:14}}>
   <div style={{display:'flex',justifyContent:'space-between',gap:8,fontSize:9}}><span style={{color:'#8ba9b5'}}>MISSION PROGRESS</span><b style={{color:'#aef5ff'}}>{visited}/{totalMissions} • {progress}%</b></div>
   <div aria-label={`Mission progress ${progress}%`} role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress} style={{height:7,marginTop:7,borderRadius:999,background:'#0e2732',overflow:'hidden'}}><div style={{height:'100%',width:`${progress}%`,background:'linear-gradient(90deg,#4fe3ff,#78ffb4)',boxShadow:'0 0 14px #4fe3ff99',transition:'width .25s ease'}}/></div>
  </div>

  <nav aria-label="StreetVerse controls" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:7,marginTop:10}}>
   <Tab label="PLAY" active={tab==='play'} onClick={()=>setTab('play')}/><Tab label="CREATE" active={tab==='create'} onClick={()=>setTab('create')}/><Tab label="SYSTEM" active={tab==='system'} onClick={()=>setTab('system')}/>
  </nav>

  <section style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginTop:9}}>
   {actions[tab].map(action=><Action key={action.label} {...action} onClick={()=>action.run?action.run():launch(action.names??[],action.label)}/>) }
  </section>
