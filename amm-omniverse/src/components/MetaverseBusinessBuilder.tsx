...
const BusinessPassportCenter=lazy(()=>import('./BusinessPassportCenter'))
...
export default function MetaverseBusinessBuilder(){
  ...
  return <div style={{position:'fixed',inset:0,zIndex:9999,background:'rgba(0,0,0,.72)',display:'flex',alignItems:'center',justifyContent:'center',padding:16}}><div style={{width:'min(900px,96vw)',maxHeight:'90vh',overflow:'auto',background:'#04050e',border:'1px solid rgba(232,185,68,.5)',borderRadius:16,padding:16,color:'white'}}>
    <div style={{display:'flex',justifyContent:'space-between',gap:10}}><div><div style={{fontSize:22,fontWeight:900}}>Living World Business Builder</div><div style={{fontSize:12,color:'#ffd86a'}}>{s.city||'Chicago'}, {s.country||'United States'} · Stage: {s.stage||'idea'}</div></div><button onClick={()=>{setPassportOpen(false);setOpen(false)}}>Close</button></div>
    ...
    <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:14}}><button onClick={()=>setPassportOpen(true)} style={{fontWeight:900}}>📸 BUSINESS PASSPORT / SCAN-TO-TWIN</button><button onClick={()=>window.dispatchEvent(new CustomEvent('tryamm:metaverse-business-find-suppliers'))}>Find Low-MOQ Suppliers</button><button onClick={()=>window.dispatchEvent(new CustomEvent('tryamm:metaverse-business-open-insights'))}>Open OmniBI</button><button onClick={()=>window.dispatchEvent(new CustomEvent('tryamm:metaverse-business-open-marketplace'))}>Open Marketplace</button></div>
    {passportOpen&&<Suspense fallback={<div style={{marginTop:12,color:'#9fb2c4'}}>Loading Business Passport…</div>}><BusinessPassportCenter onClose={()=>setPassportOpen(false)}/></Suspense>}
  </div></div>
}
