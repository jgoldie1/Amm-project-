...
export default function StreetVerseCommunityMobileWorld({slice,onClose}:{slice:StreetVerseCommunitySlice;onClose:()=>void}){
 const missions=slice.missions
 const saveKey=`tryamm.streetverse.community-${slice.communityAreaNumber}.mobile.v1`
 const saved=useMemo<Saved>(()=>{try{return JSON.parse(localStorage.getItem(saveKey)||'{}')}catch{return{}}},[saveKey])
 ...
 const [message,setMessage]=useState(`${slice.name} StreetVerse active • reach all neighborhood checkpoints.`)
 ...
 useEffect(()=>{
  ...
  const detail={id:near.id,label:near.label,reference:near.reference,kind:near.kind,communityAreaNumber:slice.communityAreaNumber,communityAreaName:slice.name,vehicle:vehicleRef.current,mobileSafeMode:true,htmlCity:true,visited:next.length,total:missions.length}
  ...
  if(next.length===missions.length){const complete={id:`community-${slice.communityAreaNumber}-mobile-safe`,label:`StreetVerse ${slice.name}`,communityAreaNumber:slice.communityAreaNumber,communityAreaName:slice.name,mobileSafeMode:true,htmlCity:true,visited:next,total:missions.length,source:'streetverse-community-mobile'};setMessage(`${slice.name.toUpperCase()} COMPLETE ✓ • authoritative reward check is ready • open REEL after claim.`);window.dispatchEvent(new CustomEvent('tryamm:streetverse-mission-complete',{detail:complete}));window.dispatchEvent(new CustomEvent('tryamm:toast',{detail:{message:`${slice.name} complete • ${missions.length}/${missions.length} neighborhood checkpoints ✓`}}))}
 },[near?.id,missions,slice.communityAreaNumber,slice.name])

 const interactVehicle=()=>{if(!vehicleRef.current){if(carDistance>10){setMessage(`StreetVerse car is ${Math.round(carDistance)}m away • move closer.`);return}vehicleRef.current=true;setVehicle(true);posRef.current={...carRef.current};setPos({...carRef.current});setMessage(`DRIVE MODE • ${slice.name.toUpperCase()}`)}else{vehicleRef.current=false;setVehicle(false);setMessage(`WALK MODE • ${slice.name.toUpperCase()}`)}}
 ...
 const openReel=()=>window.dispatchEvent(new CustomEvent('tryamm:open-reel-creator',{detail:{source:'streetverse-community-mobile',missionProgress:`${visited.length}/${missions.length}`,communityAreaNumber:slice.communityAreaNumber,communityAreaName:slice.name,vehicle,mobileSafeMode:true,htmlCity:true}}))

 return <div data-streetverse-html-city="true" data-community-area={slice.communityAreaNumber} style={{position:'fixed',inset:0,zIndex:18000,background:'linear-gradient(#5998bd 0 33%,#d6bd91 34% 39%,#1b2830 40% 100%)',color:'#fff',fontFamily:'system-ui',overflow:'hidden'}}>
  <header style={{height:68,display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,padding:'0 10px',background:'#020712f3',borderBottom:'1px solid #274963'}}><div><b>STREETVERSE • {slice.name.toUpperCase()}</b><div style={{fontSize:11,color:'#8effb7'}}>COMMUNITY AREA {slice.communityAreaNumber} • {vehicle?'DRIVE':'WALK'} • {visited.length}/{missions.length}</div></div><div style={{display:'flex',gap:6,overflowX:'auto'}}><button onClick={interactVehicle} style={{minHeight:44,borderRadius:11,border:'1px solid #59e7ff',background:'#071b25',color:'#9af0ff',fontWeight:900}}>{vehicle?'EXIT CAR':'ENTER CAR'}</button><button onClick={openReel} style={{minHeight:44,borderRadius:11,border:'1px solid #ff7ce8',background:'#251027',color:'#fff',fontWeight:900}}>● REEL</button><button onClick={onClose} aria-label={`Close ${slice.name} StreetVerse`} style={{width:44,height:44,borderRadius:11,border:'1px solid #567',background:'#101923',color:'#fff'}}>×</button></div></header>
  <main style={{position:'absolute',inset:'68px 0 0',overflow:'hidden'}}>
   <div style={{position:'absolute',left:10,right:10,top:10,zIndex:30,padding:'10px 12px',borderRadius:12,background:'#030914e8',border:'1px solid #4e7891'}}>{message}<div style={{fontSize:10,color:'#b9c9d6',marginTop:4}}>CHICAGO 77 SLICE • {slice.status} • AREA {slice.communityAreaNumber}</div></div>
   ...
   {missions.map(m=>{const done=visited.includes(m.id);return <div key={m.id} title={m.reference} style={{position:'absolute',left:`${m.x}%`,top:`${m.y}%`,zIndex:12,textAlign:'center',transform:'translate(-50%,-50%)'}}><div style={{width:18,height:18,margin:'auto',borderRadius:'50%',background:done?'#55e88a':'#ffd65a',border:'2px solid white',boxShadow:done?'0 0 18px #55e88a':'0 0 20px #ffd65a'}}>{done?'✓':''}</div><div style={{marginTop:6,padding:'5px 7px',borderRadius:7,background:'#030914e8',fontSize:9,fontWeight:800,maxWidth:150}}>{m.label}<div style={{fontSize:7,color:'#9fc7dd',marginTop:2}}>{m.reference}</div></div></div>})}
   <div aria-label={vehicle?'Player driving StreetVerse car':'Player'} style={{position:'absolute',left:'50%',bottom:'17%',width:vehicle?46:28,height:vehicle?64:48,transform:`translateX(-50%) rotate(${vehicle?heading:0}deg)`,zIndex:14,borderRadius:vehicle?11:14,background:vehicle?'#36a9e8':'#23d9f4',border:'3px solid #fff',boxShadow:'0 0 22px #23d9f488'}}/>
   {!vehicle&&<div aria-label={`Parked StreetVerse car ${Math.round(carDistance)} meters away`} style={{position:'absolute',left:`${clamp(50+(car.x-pos.x)*1.1,10,90)}%`,top:`${clamp(60+(car.y-pos.y)*.7,28,80)}%`,width:30,height:44,transform:'translate(-50%,-50%)',borderRadius:8,background:'#36a9e8',border:'2px solid #dff8ff'}}/>}
   ...
   <div style={{position:'absolute',right:12,bottom:18,zIndex:35,maxWidth:190,padding:9,borderRadius:12,background:'#030914df',border:'1px solid #34566d',fontSize:10,lineHeight:1.35}}>MOBILE SAFE WORLD<br/><b style={{color:'#8effb7'}}>{slice.name.toUpperCase()}</b><br/>Chicago 77 mission metadata active.</div>
  </main>
 </div>
}
