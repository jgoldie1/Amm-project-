...
export default function StreetVerseCommunityMobileWorld({slice,onClose}:Props){
 ...
 const saveShareReel=async()=>{if(!reelFile){reelFileInput.current?.click();return}try{const file=reelFile;if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({files:[file],title:`${slice.name} StreetVerse Reel`,text:reelDetail.suggestedCaption});setReelStatus('Save / share sheet opened ✓');window.dispatchEvent(new CustomEvent('tryamm:streetverse-reel-save-share-complete',{detail:{...reelDetail,source:'streetverse-community-mobile',action:'share',fileName:file.name,size:file.size,type:file.type||'video/*'}}));return}const url=URL.createObjectURL(file);const a=document.createElement('a');a.href=url;a.download=file.name||`streetverse-reel-${Date.now()}.mp4`;a.rel='noopener';document.body.appendChild(a);a.click();a.remove();window.setTimeout(()=>URL.revokeObjectURL(url),1000);setReelStatus('Reel download started ✓ • check Photos or Files.');window.dispatchEvent(new CustomEvent('tryamm:streetverse-reel-save-share-complete',{detail:{...reelDetail,source:'streetverse-community-mobile',action:'download',fileName:file.name,size:file.size,type:file.type||'video/*'}}))}catch(error){setReelStatus(error instanceof Error?error.message:'Save/share failed. Use CAPTURE / CHOOSE and try again.')}}
 const apiJson=async(url:string,body?:unknown,method:'GET'|'POST'='POST')=>{const token=await getAccessToken();if(!token)throw new Error('Sign in again before using creator media.');const headers:Record<string,string>={authorization:`Bearer ${token}`};if(method!=='GET')headers['content-type']='application/json';const response=await fetch(url,{method,headers,body:method==='GET'?undefined:JSON.stringify(body)});const data=await response.json().catch(()=>({}));if(response.status===401)throw new Error('Your session expired. Sign in again and retry.');if(!response.ok)throw new Error(data?.error||`Request failed (${response.status})`);return data}
 ...
 const publishReel=async()=>{if(!reelMediaId){setReelStatus('Upload and verify the Reel before publishing.');return}setReelBusy(true);try{setReelStatus('Submitting publication…');const result=await apiJson('/api/media/publish',{mediaId:reelMediaId,destinations:reelDetail.destinations});const status=String(result?.job?.status||'');setReelStatus(status==='queued'?'Publication queued ✓ • checking delivery status…':result?.next||'Publication submitted.');window.dispatchEvent(new CustomEvent('tryamm:reel-publish-job',{detail:{...reelDetail,mediaId:reelMediaId,job:result?.job,destinations:result?.destinations}}));if(status==='queued'){const jobs=await apiJson(`/api/media/publish-jobs?mediaId=${encodeURIComponent(reelMediaId)}`,undefined,'GET');const latest=jobs?.jobs?.[0];if(latest?.status&&latest.status!=='queued')setReelStatus(`Publication ${String(latest.status).toLowerCase()}${latest.error_code?` • ${latest.error_code}`:''}`)}}catch(error){setReelStatus(error instanceof Error?error.message:'Reel publication failed')}finally{setReelBusy(false)}}
