...
export default function StreetVerseChronoDojoArena(){
  ...
  const start=(nextMode:Mode)=>{
    setMode(nextMode);setScore(0);setDiscipline(100);setRound(1);setCue('OPENING');setCompleted(false)
    setMessage(nextMode==='history'?'Historical-simulation sparring prototype. Nonlethal points only.':'Present-day StreetVerse tournament. Timing, defense and sportsmanship score the match.')
    setOpen(true)
  }
  ...
  useEffect(()=>{
    const history=(event:Event)=>{
      ...
      if(d.campaignId==='chicago-dojo-wars'&&d.choice==='A')start('history')
    }
    const tournament=()=>start('present')
    window.addEventListener('tryamm:history-route-selected',history)
    window.addEventListener('tryamm:dojo-tournament-start',tournament)
    return()=>{
      window.removeEventListener('tryamm:history-route-selected',history)
      window.removeEventListener('tryamm:dojo-tournament-start',tournament)
    }
  },[])
  ...
  const act=(action:Action)=>{
    ...
    setMessage(delta>=3?'Style advantage • +3':delta===2?'Perfect read • +2':delta===1?'Safe response • +1':delta===0?'Neutral reset':'Mistimed • discipline -5')
    window.dispatchEvent(new CustomEvent('tryamm:dojo-sparring-action',{detail:{cue,action,delta,score:nextScore,discipline:nextDiscipline,round,styleId,style:style.label,bias,mode}}))
    if(nextScore>=12){
      window.clearInterval(timer.current)
      setCompleted(true)
      window.dispatchEvent(new CustomEvent('tryamm:dojo-training-completed',{detail:{score:nextScore,discipline:nextDiscipline,rounds:round,mode:mode==='history'?'nonlethal-historical-simulation':'present-day-tournament'}}))
      if(mode==='history'){
        ...
        window.dispatchEvent(new CustomEvent('tryamm:dojo-tournament-completed',{detail:{won:true,score:nextScore,discipline:nextDiscipline,rounds:round,styleId}}))
        setMessage('TOURNAMENT WIN • Dojo reputation and rival-respect bond increased.')
      }
    }
  }
  ...
  return <div role="dialog" aria-label={mode==='history'?'Chicago Dojo Wars training simulation':'StreetVerse Chicago dojo tournament'} style={{position:'fixed',inset:0,zIndex:17400,background:'#020306f5',color:'#fff'}}>
    <div style={{maxWidth:760,margin:'0 auto',padding:'18px 16px 120px'}}>
      <header style={{display:'flex',justifyContent:'space-between',gap:12}}>
        <div><div style={{fontSize:10,letterSpacing:2.5,color:'#e8b944',fontWeight:950}}>{mode==='history'?'TIME MACHINE • 1970 CHICAGO':'STREETVERSE • PRESENT-DAY CHICAGO'}</div><h1 style={{margin:'6px 0'}}>{mode==='history'?'Dojo Wars • Sparring Reconstruction':'Chicago Dojo Tournament'}</h1><div style={{fontSize:12,opacity:.68}}>{mode==='history'?'Nonlethal gameplay simulation. Historical facts, source claims and legend remain separately labeled.':'Nonlethal sport/game scoring. Style choice changes timing bonuses and strategy.'}</div></div>
        <button onClick={()=>setOpen(false)} aria-label="Close dojo arena" style={{width:48,height:48,borderRadius:14}}>×</button>
      </header>
