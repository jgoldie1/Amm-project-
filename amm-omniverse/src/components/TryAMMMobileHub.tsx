...
export default function TryAMMMobileHub({onClose}:{onClose:()=>void}){
  ...
  return <div role="dialog" aria-label="TRYAMM Mobile" style={{position:'fixed',inset:0,zIndex:12200,overflowY:'auto',background:'radial-gradient(circle at top,#10253a,#04050e 55%,#010205)',color:'#fff',padding:18}}>
    <div style={{maxWidth:1180,margin:'0 auto'}}>
      <header style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center'}}><div><div style={{fontSize:10,color:cyan,letterSpacing:3,fontWeight:900}}>TRYAMM • ALL AMERICAN MOBILE • FRANCHISE • HOLOFON ROADMAP</div><h1 style={{margin:'7px 0'}}>TRYAMM Mobile + All American Mobile</h1><p style={{color:'#a9b7c8',maxWidth:820}}>{TRYAMM_MOBILE.truth}</p></div><button aria-label="Close TRYAMM Mobile" onClick={onClose} style={close}>×</button></header>
