...
import FounderFastTrackAgencyPanel from './FounderFastTrackAgencyPanel'
...
export default function TryAMMMobileHub({onClose}:{onClose:()=>void}){
 ...
 return <div role="dialog" aria-label="TRYAMM Mobile" style={{position:'fixed',inset:0,zIndex:12200,overflowY:'auto',background:'radial-gradient(circle at top,#10253a,#04050e 55%,#010205)',color:'#fff',padding:18}}><div style={{maxWidth:1180,margin:'0 auto'}}>
  ...
  <section style={panel}><h2>Agency Command Center</h2><div style={grid}>{Object.entries(AGENCY_TOOLS).map(([group,items])=><article key={group} style={card}><b>{group}</b><p style={muted}>{items.join(' • ')}</p></article>)}</div><p style={{...muted,color:'#ffcf66'}}>Rules: {AGENCY_GROWTH_RULES.join(' • ')}</p></section>
  ...
  <section style={panel}><h2>Creator Quota Coach</h2><p style={muted}>{QUOTA_COACH.purpose}</p><div style={grid}>{QUOTA_COACH.outputs.map(x=><article key={x} style={card}><b>{x}</b></article>)}</div><p style={{...muted,color:'#ffcf66'}}>Guardrails: {QUOTA_COACH.guardrails.join(' • ')}</p></section>
  <section style={panel}><h2>Bean Packages</h2><div style={grid}>{BEAN_PACKAGE_CATALOG.map(p=><article key={p.id} style={card}><b>{p.name}</b><small>{p.audience}</small><p style={muted}>{p.benefits.join(' • ')}</p><small style={{color:'#ffcf66'}}>{p.moneyRule}</small></article>)}</div><p style={muted}>{BEAN_TRUTH.virtualUnit} {BEAN_TRUTH.cash}</p></section>
  <section style={panel}><h2>Creator Growth Rebate</h2><p style={muted}>{CREATOR_REBATE_PROGRAM.principle}</p><div style={grid}>{CREATOR_REBATE_PROGRAM.rewardTypes.map(x=><article key={x} style={card}><b>{x}</b></article>)}</div><p style={{...muted,color:'#7dffb0'}}>{CREATOR_REBATE_PROGRAM.verification}</p><small>{QUOTA_TO_LEGACY_PATH}</small></section>
