...
export default function StreetVerseRealtimePresence(){
  ...
  useEffect(()=>{
    ...
    const publishPlayers=(presence:PresenceState,userId:string)=>{
      const players=Object.values(presence).flat().filter(p=>p&&p.userId&&p.userId!==userId&&Number.isFinite(Number(p.x))&&Number.isFinite(Number(p.z)))
    ...
    const start=async()=>{
      ...
      channel=sb.channel(CHANNEL,{config:{presence:{key:userId}}})
      channelRef.current=channel
      channel
        .on('presence',{event:'sync'},()=>publishPlayers(channel!.presenceState() as unknown as PresenceState,userId))
        .on('presence',{event:'join'},()=>publishPlayers(channel!.presenceState() as unknown as PresenceState,userId))
        .on('presence',{event:'leave'},()=>publishPlayers(channel!.presenceState() as unknown as PresenceState,userId))
        .subscribe(async status=>{
          ...
          if(status==='SUBSCRIBED'){
            ...
            window.dispatchEvent(new CustomEvent('tryamm:streetverse-multiplayer-status',{detail:{state:'LIVE',online:1}}))
    ...
    const onPosition=(event:Event)=>{
      ...
      if(now-lastTrackRef.current<180)return
      lastTrackRef.current=now
      void active.track(next)
    }
  ...
  return <div aria-live="polite" style={{position:'fixed',left:12,top:72,zIndex:16994,padding:'7px 10px',borderRadius:999,background:'#04131ddd',border:`1px solid ${state==='LIVE'?'#70ffb077':'#5f718077'}`,color:state==='LIVE'?'#bfffd8':'#bfd0d8',fontFamily:'monospace',fontSize:9,fontWeight:950,letterSpacing:.5,pointerEvents:'none'}}>
    MULTIPLAYER • {state==='LIVE'?`${online} ONLINE`:state==='SIGNED_OUT'?'SIGN IN TO JOIN':state}
  </div>
}
