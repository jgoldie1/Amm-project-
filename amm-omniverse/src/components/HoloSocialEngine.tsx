...
export default function HoloSocialEngine({onClose}:Props){
  ...
  useEffect(()=>{
    const onGift=(event:Event)=>{const detail=(event as CustomEvent<any>).detail;setStatus(`${String(detail?.giftType||'GIFT').toUpperCase()} EFFECT • ${detail?.settlementStatus||'VISUAL'}`)}
    window.addEventListener('tryamm:holo-gift',onGift)
    return()=>window.removeEventListener('tryamm:holo-gift',onGift)
  },[])
  ...
  const cards={
    ...
    pk:[['PK ARENA','BIGO-style battles become a holographic arena with teams, meters, gifts and world effects.'],['GIFT FX','Gifts trigger 3D/Lottie-ready effects, sound cues and safe room reactions.'],['CREATOR SCORE','XP, reputation and visual rewards remain distinct from withdrawable cash until settlement is verified.']],
