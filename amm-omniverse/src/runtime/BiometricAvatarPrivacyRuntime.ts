...
export const BIOMETRIC_AVATAR_PRIVACY_POLICY={
  ...
  deletion:'Live camera/video frames are ephemeral and must be discarded immediately after avatar fitting. Uploaded still photos are session-only by default and user-deletable; saving a still photo requires a separate explicit choice.',
...
export function createBiometricAvatarConsent(input:{purpose:AvatarCapturePurpose;views:AvatarCaptureView[];adultOrGuardianAuthorized:boolean;saveRawPhotos?:boolean}):BiometricAvatarConsent{
  const hasLiveVideo=input.views.includes('camera-live')
  return {
    version:'2026-09-01',
    granted:true,
    ...
    advertisingUse:false,
    modelTrainingUse:false,
    rawPhotoRetention:hasLiveVideo?'session-only':input.saveRawPhotos?'user-save-explicit':'session-only',
...
export function canProcessAvatarBiometrics(consent:BiometricAvatarConsent|undefined){
  ...
  if(consent.views.includes('camera-live')&&consent.rawPhotoRetention==='user-save-explicit')return {ok:false,reason:'Live camera/video frames cannot be retained as raw photos. They must be deleted after avatar fitting.'} as const
...
export function requestPrivacySafeAvatarCapture(detail:{purpose:AvatarCapturePurpose;views:AvatarCaptureView[];consent:BiometricAvatarConsent}){
  ...
  const liveVideo=detail.views.includes('camera-live')
  emit('tryamm:avatar-capture-authorized',{
    purpose:detail.purpose,
    views:detail.views,
    localOnly:true,
    noRecognition:true,
    noFingerprint:true,
    noRecording:liveVideo,
    noServerUpload:liveVideo,
    deleteRawImmediatelyAfterFit:liveVideo,
    createdAt:new Date().toISOString(),
  })
  return gate
}
...
export function completeEphemeralAvatarVideoProcessing(){
  requestAvatarBiometricDeletion('raw-captures')
  emit('tryamm:avatar-video-ephemeral-complete',{
...
export function installBiometricAvatarPrivacyRuntime(){
  ...
  emit('tryamm:avatar-biometric-privacy-ready',{
    ...
    liveCamera:['no-recording','no-server-upload','ephemeral-frame','delete-after-fit'],
    noRecognition:true,
    noFingerprint:true,
  })
}
