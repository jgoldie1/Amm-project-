...
function make(branchId:string,city:string,level:number,weather?:string,timeOfDay?:string):DispatchCall{return{id:`dispatch-${Date.now()}`,city,title:`${city} ${branchId.replace('guardian-','').replaceAll('-',' ')} response`,priority:priority(level,weather),branchId,weather,timeOfDay,roles:roleMap[branchId]||['leader','driver','communications'],vehicle:vehicleMap[branchId]||'Guardian Response Vehicle',equipment:equipmentMap[branchId]||['medical kit','radio','navigation tablet'],missionType:'city-response',clipOnMilestones:true,createdAt:Date.now()}}
