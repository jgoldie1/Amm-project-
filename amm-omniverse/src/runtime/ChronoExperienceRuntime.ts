...
function openImmersive(state:ChronoExperienceState){try{const fn=(window as any).__showImmersiveWorlds;if(typeof fn==='function')fn()}catch{};setTimeout(()=>window.dispatchEvent(new CustomEvent('tryamm:chrono-immersive-open',{detail:state})),60)}
export function launchChronoExperience(detail:Record<string,any>){const state:ChronoExperienceState={active:true,runId:String(detail.runId||detail.id||''),scenarioId:String(detail.scenarioId||detail.scenario_id||''),slug:String(detail.slug||detail.scenario?.slug||''),name:String(detail.name||detail.scenario?.name||'Chrono Scenario'),era:String(detail.era||detail.scenario?.era||''),scenarioType:String(detail.scenarioType||detail.scenario_type||detail.scenario?.scenario_type||'simulation'),evidenceLevel:String(detail.evidenceLevel||detail.evidence_level||detail.scenario?.evidence_level||'simulation'),description:String(detail.description||detail.scenario?.description||''),checkpoint:Number(detail.checkpoint||0),missionStep:Number(detail.missionStep||0),returnPoint:(detail.returnPoint||'advanced-worlds') as ChronoExperienceState['returnPoint'],startedAt:String(detail.startedAt||now()),updatedAt:now()};write(state);openImmersive(state);window.dispatchEvent(new CustomEvent('tryamm:chrono-evidence',{detail:{level:state.evidenceLevel,message:evidenceMessage(state.evidenceLevel),scenario:state.name}}));window.dispatchEvent(new CustomEvent('tryamm:chrono-mission',{detail:{step:state.missionStep,title:'Chrono Mission',objective:missionFor(state)}}));narrate('TIME MACHINE ONLINE',`${state.name}. ${evidenceMessage(state.evidenceLevel)} ${missionFor(state)}`);return state}
