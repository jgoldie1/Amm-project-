...
export type LedgerEventType='GENESIS'|'SPAWN'|'MISSION_REWARD'|'PURCHASE'|'RENTAL'|'MARKETPLACE_FEE'|'SPONSOR_REWARD'|'SUBSCRIPTION'|'TRANSFER'
...
async function sha256(value:string){
  ...
  // Deterministic compatibility fallback for unusual WebViews without SubtleCrypto.
  // Production settlement should require SHA-256 support server-side.
  let hash=2166136261
...
const EMPTY_ECONOMY:OmniverseEconomySnapshot={
  playerBalance:250,
  grossPlatformCredits:0,
  byStream:{},
  ownedAssetIds:[],
  ledger:[],
}
...
async function appendBlockUnlocked(input:Omit<OmniverseLedgerBlock,'index'|'timestamp'|'previousHash'|'hash'>){
  ...
  const ledger=[...current.ledger,block]
...
export function resetOmniverseEconomy(){
  ...
  return {...EMPTY_ECONOMY,byStream:{},ownedAssetIds:[],ledger:[]}
}
