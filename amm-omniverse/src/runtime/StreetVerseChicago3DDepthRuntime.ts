...
import { addStreetVerseChicagoSkylineEnhancer } from './StreetVerseChicagoSkylineEnhancer'
...
import { addStreetVerseOriginalCityArt } from './StreetVerseOriginalCityArt'
let installed=false
...
export function addChicagoTransitAirportDepth(scene:THREE.Scene){
 const unregisterNativeScene=registerStreetVerseScene({scene,collisionBoxes:[]})
 window.addEventListener('tryamm:streetverse-scene-close',unregisterNativeScene,{once:true})
 ...
 const originalArt=addStreetVerseOriginalCityArt(scene)
 const physical=addStreetVerseChicagoPhysicalWorld3D(scene);const districts=addStreetVerseChicagoDistrictWorld3D(scene);const skyline=addStreetVerseChicagoSkylineEnhancer(scene);const obamaCenter=addStreetVerseObamaCenterWorld3D(scene)
 ...
 window.dispatchEvent(new CustomEvent('tryamm:streetverse-3d-depth-ready',{detail:{railCars:true,platforms:true,doors:true,crowds:true,visibleFallbackResidents:46,ohare:true,midway:true,physicalChicago:true,animatedFlood:true,rinkSkaters:true,lakefrontPedestrians:true,courthouseCrowds:true,battleTriggers:true,loopTowers:true,downtownSkylineHighRises:21,denseDowntownStreetWalls:true,rooftopDetail:true,riverBridges:true,elevatedL:true,districtArchitecture:['south','west','north'],originalCityArt:originalArt,obamaCenterInspiredCampus:true,obamaCenterMissions:15,obamaLegacyDialogue:true,homeCourtBasketball:true,fictionalPublicEventSecurityMission:true,nativeMultiplayerScene:true}}))
 return {platforms:1,trains:2,riders:46,airports:['O’Hare','Midway'],patrols:4,animatedDoors:doors.length,physicalChicago:true,districtWorld:true,downtownSkyline:skyline,originalCityArt:originalArt,obamaCenterWorld:true,obamaLegacyMissions:true,nativeMultiplayerScene:true}
}