...
function mount(){if(!active()){document.getElementById(ID)?.remove();return}if(document.getElementById(ID))return;const root=document.createElement('div');root.id=ID;root.style.cssText='position:fixed;right:12px;top:82px;z-index:2147482100;pointer-events:auto;font-family:Inter,Arial,sans-serif';const btn=document.createElement('button');btn.textContent='🏙️ CHICAGO 3D';btn.style.cssText='padding:8px 11px;border-radius:999px;border:1px solid #6caed8;background:#07131ded;color:#fff;font-weight:900;font-size:10px';const panel=document.createElement('div');panel.style.cssText='display:none;margin-top:6px;width:min(88vw,360px);padding:10px;border:1px solid #34556b;border-radius:12px;background:#061019f2;color:white;font-size:10px';panel.innerHTML='<b>CHICAGO PHYSICAL WORLD</b><br>Loop tower canyon • Chicago River + bridges • elevated L • lakefront • South/West/North Side architecture • transit • underground • flood • rink • courthouse • automatic battle zones';btn.onclick=()=>panel.style.display=panel.style.display==='none'?'block':'none';root.append(btn,panel);document.body.appendChild(root)}
export function installStreetVerseChicago3DDepthRuntime(){if(installed||typeof window==='undefined'||typeof document==='undefined')return;installed=true;const sync=()=>requestAnimationFrame(mount);window.addEventListener('hashchange',sync);window.addEventListener('popstate',sync);window.addEventListener('tryamm:streetverse-enter',sync);if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',sync,{once:true});else sync()}
...
export function addChicagoTransitAirportDepth(scene:THREE.Scene){
 ...
 const physical=addStreetVerseChicagoPhysicalWorld3D(scene)
 const districts=addStreetVerseChicagoDistrictWorld3D(scene)
 const clock=new THREE.Clock();let frame=0
 const animate=()=>{if(!active())return;const elapsed=clock.getElapsedTime();const openAmount=(Math.sin(elapsed*1.2)+1)/2;doors.forEach(d=>{d.left.position.x=d.baseX-.38-openAmount*.42;d.right.position.x=d.baseX+.38+openAmount*.42});physical.tick(elapsed*1000);if((frame++%6)===0){try{const saved=JSON.parse(localStorage.getItem(SAVE_KEY)||'{}');if(Number.isFinite(saved.x)&&Number.isFinite(saved.z)){const pos=new THREE.Vector3(saved.x,Number(saved.y)||0,saved.z);physical.update(pos);districts.update(pos)}}catch{}}requestAnimationFrame(animate)};animate()
 window.dispatchEvent(new CustomEvent('tryamm:streetverse-3d-depth-ready',{detail:{railCars:true,platforms:true,doors:true,crowds:true,ohare:true,midway:true,physicalChicago:true,animatedFlood:true,rinkSkaters:true,lakefrontPedestrians:true,courthouseCrowds:true,battleTriggers:true,loopTowers:true,riverBridges:true,elevatedL:true,districtArchitecture:['south','west','north']}}))
 return {platforms:1,trains:2,riders:42,airports:['O’Hare','Midway'],patrols:4,animatedDoors:doors.length,physicalChicago:true,districtWorld:true}
}
