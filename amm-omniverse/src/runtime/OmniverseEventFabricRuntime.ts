...
import { installStreetVerseChicagoWorldRuntime } from './StreetVerseChicagoWorldRuntime'
import { installStreetVerseChicagoTransitAirportMissionsRuntime } from './StreetVerseChicagoTransitAirportMissionsRuntime'
import { installStreetVerseAirportAfterDarkRuntime } from './StreetVerseAirportAfterDarkRuntime'
import { installStreetVerseChicagoUndergroundRuntime } from './StreetVerseChicagoUndergroundRuntime'
import { installStreetVerseChicagoTimeMachineMissionsRuntime } from './StreetVerseChicagoTimeMachineMissionsRuntime'
...
function constrainedStreetVerse(){if(typeof window==='undefined'||typeof navigator==='undefined')return false;const path=window.location.pathname; if(!path.startsWith('/streetverse'))return false;const ua=navigator.userAgent||'';const apple=/iPhone|iPad|iPod/i.test(ua);const iosMatch=ua.match(/OS (\d+)_/i);const iosMajor=iosMatch?Number(iosMatch[1]):0;const olderIOS=apple&&iosMajor>0&&iosMajor<=16;const narrow=window.innerWidth<=480;const lowCores=typeof navigator.hardwareConcurrency==='number'&&navigator.hardwareConcurrency<=4;const memory=(navigator as Navigator&{deviceMemory?:number}).deviceMemory;const lowMemory=typeof memory==='number'&&memory<=4;return olderIOS||narrow||lowCores||lowMemory}
function installDeferredThreeRuntimes(){if(constrainedStreetVerse())return;void Promise.all([
  import('./StreetVerseFreeSpaceConstruct3D'),
  import('./StreetVerseBuildingLife3D'),
  import('./StreetVerseQuestImmersiveRuntime'),
  import('./StreetVerseChicago3DDepthRuntime'),
  import('./StreetVerseChicagoEmergentWorldRuntime'),
]).then(([freeSpace,buildingLife,questImmersive,chicagoDepth,emergentWorld])=>{
  freeSpace.installStreetVerseFreeSpaceConstruct3D();
  buildingLife.installStreetVerseBuildingLife3D();
  questImmersive.installStreetVerseQuestImmersiveRuntime();
  chicagoDepth.installStreetVerseChicago3DDepthRuntime();
  emergentWorld.installStreetVerseChicagoEmergentWorldRuntime();
}).catch(error=>console.warn('[OmniverseEventFabric] deferred 3D runtimes unavailable:',error))}
