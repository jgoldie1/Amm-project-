...
export type OmniverseEventReceipt={
  eventId:string
  channel:OmniverseEventChannel
  status:'ready'|'gated'
  reason?:string
}

export type OmniverseFabricState={
  lastEvent?:OmniverseEventEnvelope
  receipts:OmniverseEventReceipt[]
  completedChannels:OmniverseEventChannel[]
}

const STATE_KEY='tryamm_omniverse_event_fabric_v1'
...
function makeId(){return `omni_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,9)}`}
function readState():OmniverseFabricState{
  if(typeof localStorage==='undefined')return {receipts:[],completedChannels:[]}
  try{
    const raw=localStorage.getItem(STATE_KEY)
    if(!raw)return {receipts:[],completedChannels:[]}
    const parsed=JSON.parse(raw) as OmniverseFabricState
    return {
      lastEvent:parsed.lastEvent,
      receipts:Array.isArray(parsed.receipts)?parsed.receipts.slice(-MAX_EVENT_HISTORY):[],
      completedChannels:Array.isArray(parsed.completedChannels)?parsed.completedChannels:[],
    }
