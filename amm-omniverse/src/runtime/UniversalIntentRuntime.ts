...
export class NeuralNoiseCancelingBuffer{
  ...
  push(input:IntentSignal):IntentSignal|null{
    ...
    // Adaptive baseline follows recurring low-level channel noise slowly.
    ...
    // Reject strong artifacts and very weak cleaned samples before fusion.
...
export class QuantumSpeedEngine{
  ...
  enqueue(intent:FusedIntent){
    // STOP always preempts pending work. Otherwise retain only the freshest instance per intent.
