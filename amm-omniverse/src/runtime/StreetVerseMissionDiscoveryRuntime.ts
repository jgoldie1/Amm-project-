...
export function completeStreetVerseMission(missionId:string,outcome:Record<string,unknown>={}){const mission=missionState.get(missionId);if(!mission)return{completed:false,missionId,reason:'mission-not-discovered'};const record={...mission,status:'completed' as const,completedAt:new Date().toISOString(),outcome};missionState.set(missionId,record);emit('tryamm:mission:completed',record);emit('tryamm:mission:consequence',{missionId,rarity:mission.rarity,category:mission.category,outcome,destinations:['world-memory','relationships','crew-reputation','business-network','creator-content','command-nexus']});emit('tryamm:world-memory:record',{...record,type:'mission-consequence'});emit('tryamm:mission:next-candidate',{sourceMissionId:missionId,prefer:mission.rarity==='mythic'?['cross-verse','reality-quest']:['story','exploration','business']});return record}
...
export function getStreetVerseMissionDiscoveryState(){return{missions:[...missionState.values()],secrets:[...secretState.entries()].map(([secretId,value])=>({secretId,...value}))}}
