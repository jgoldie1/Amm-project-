...
export function installStreetVerseChicagoEmergentWorldRuntime(){if(installed||typeof window==='undefined')return;installed=true;let state=read();
window.addEventListener('tryamm:mission-completed',(e:any)=>{const d=e.detail||{};state.reputation=clamp(state.reputation+(d.source==='chicago-underground'?4:2));if(d.easterEgg)state.secrets+=1;save(state);emit('tryamm:chicago-world-state',{...state,reason:'mission-completed'})});
...
emit('tryamm:chicago-emergent-ready',{systems:['persistent choices','district reputation','secret chains','weather reactions','dynamic encounters','hidden fast travel'],state})}
