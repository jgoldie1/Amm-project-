...
const makeCar=(color:number)=>{const g=new THREE.Group();addBox(g,0,.72,0,3.8,.7,1.8,material(color,.55,.35));addBox(g,.15,1.25,0,2.1,.72,1.55,material(0x6d8798,.2,.55,true,.82));for(const x of [-1.15,1.15])for(const z of [-.92,.92]){const w=new THREE.Mesh(new THREE.CylinderGeometry(.34,.34,.22,10),material(0x111111,.95,0));w.rotation.x=Math.PI/2;w.position.set(x,.38,z);g.add(w)}return g}
const makePerson=(shirt:number)=>{const g=new THREE.Group();const body=new THREE.Mesh(new THREE.CapsuleGeometry(.24,.68,3,6),material(shirt,.8,.02));body.position.y=1;g.add(body);const head=new THREE.Mesh(new THREE.SphereGeometry(.22,8,8),material(0x8a5a44,.82,.02));head.position.y=1.68;g.add(head);return g}
...
export function addStreetVerseChicagoDistrictWorld3D(scene:THREE.Scene):ChicagoDistrictWorld3D{
  ...
  const pedestrians:THREE.Group[]=[];const traffic:THREE.Group[]=[]
  ...
  const loop=new THREE.Group();root.add(loop)
  ...
  addLabel(loop,'STATE ST',-10,5.2,7);addLabel(loop,'WABASH AVE',12,5.2,7);addBusStop(loop,'CTA • LOOP',-5,27)
  ...
  for(let i=0;i<16;i++){const p=makePerson([0x315a7d,0x9b4c43,0x4f7b55,0x7d568a][i%4]);p.position.set(-34+i*4.4,0,12+(i%3)*6);p.userData={baseX:p.position.x,baseZ:p.position.z,phase:i*.53,speed:.7+(i%4)*.13,axis:i%2};loop.add(p);pedestrians.push(p)}
  ...
  const riverwalk=addBox(root,0,.3,10,210,.5,7,concrete);riverwalk.receiveShadow=true;for(let x=-72;x<=72;x+=18)addStreetFurniture(root,x,13)
  for(let i=0;i<10;i++){const p=makePerson([0x3f668a,0x9a633e,0x5e7c50][i%3]);p.position.set(-78+i*16,0,8.2);p.userData={baseX:p.position.x,baseZ:8.2,phase:i*.7,speed:.9,axis:0};root.add(p);pedestrians.push(p)}

  const lTrack=new THREE.Group();lTrack.position.set(0,7,34);root.add(lTrack);for(let x=-78;x<=78;x+=10){addBox(lTrack,x,-3.3,0,.7,6.6,.7,steel);addBox(lTrack,x,0,0,10,.35,.5,steel)}
  ...
  const lTrain=new THREE.Group();lTrain.position.set(-72,1.8,0);lTrack.add(lTrain);for(let i=0;i<3;i++){addBox(lTrain,i*6,0,0,5.4,3.2,2.6,material(0xaeb6bd,.55,.35));addBox(lTrain,i*6,.1,-1.34,5,.35,.08,material(0xc60c30,.45,.2))}
  const loopBus=makeBus(0x1468b3);loopBus.position.set(-70,.05,22);root.add(loopBus)
  for(let i=0;i<8;i++){const c=makeCar([0x37495f,0x8f3d35,0x535b42,0xb4b6b8][i%4]);c.position.set(-72+i*18,.02,i%2?4:30);c.userData={phase:i*18,speed:5.5+(i%3)};root.add(c);traffic.push(c)}

  const lake=new THREE.Mesh(new THREE.PlaneGeometry(300,170),water);lake.rotation.x=-Math.PI/2;lake.position.set(0,-.03,180);root.add(lake);addBox(root,0,.1,104,210,.22,14,green);addBox(root,0,.16,112,210,.22,7,concrete)
  ...
  addLabel(root,'LAKE SHORE',-18,4.6,108);addBusStop(root,'CTA • LAKEFRONT',22,109);for(let x=-60;x<=60;x+=20)addStreetFurniture(root,x,115)
  const pavilion=new THREE.Group();pavilion.position.set(48,0,100);root.add(pavilion);for(let i=-4;i<=4;i++){const rib=new THREE.Mesh(new THREE.TorusGeometry(5+i*.08,.12,6,16,Math.PI),material(0xb0b8c0,.35,.6));rib.rotation.z=Math.PI/2;rib.rotation.y=Math.PI/2;rib.position.set(i*1.1,4,0);pavilion.add(rib)}addLabel(root,'MILLENNIUM PARK DISTRICT',48,7.4,92)
  for(let i=0;i<14;i++){const p=makePerson([0x394867,0x7a5548,0x4d6847,0x825d90][i%4]);p.position.set(-78+i*12,0,111+(i%2)*4);p.userData={baseX:p.position.x,baseZ:p.position.z,phase:i*.41,speed:1.05,axis:0};root.add(p);pedestrians.push(p)}

  const south=new THREE.Group();south.position.set(-30,0,72);root.add(south)
  for(let i=0;i<14;i++){const row=Math.floor(i/7),col=i%7,x=-54+col*17,z=row*24,m=i%3===0?stone:brick;addBox(south,x,6,z,13,12,16,m);addBox(south,x,7.2,z-8.05,8,1.2,.25,material(0x2d2a27,.8,.1));for(const wx of [-4,0,4])for(const wy of [4,8])addBox(south,x+wx,wy,z-8.2,1.5,2.1,.15,glass)}
  ...
  addBox(south,38,.14,15,26,.28,16,court);addLabel(south,'79TH ST',-20,5,39.5);addLabel(south,'SOUTH SIDE',34,5,39.5)
  ...
  addBox(north,0,7,38,78,.6,8,steel);for(let x=-34;x<=34;x+=12)addBox(north,x,3.5,38,.7,7,.7,steel);addLabel(north,'NORTH SIDE',0,5.2,31);addLabel(north,'BROADWAY',-20,5.2,-9);addBusStop(north,'CTA • NORTH',28,28);addStation(north,'CTA • NORTH SIDE',0,38,52);const northBus=makeBus(0x8246af);northBus.position.set(-50,.05,31);north.add(northBus)

  const districts:DistrictDef[]=[{id:'loop',label:'THE LOOP',center:new THREE.Vector3(0,0,0),radius:72},{id:'river',label:'CHICAGO RIVER',center:new THREE.Vector3(0,0,-8),radius:34},{id:'lakefront',label:'LAKEFRONT',center:new THREE.Vector3(0,0,112),radius:48},{id:'south',label:'SOUTH SIDE',center:new THREE.Vector3(-30,0,78),radius:62},{id:'west',label:'WEST SIDE',center:new THREE.Vector3(-78,0,12),radius:58},{id:'north',label:'NORTH SIDE',center:new THREE.Vector3(18,0,-86),radius:62}]
  let current=''
  const update=(player:THREE.Vector3)=>{const t=performance.now()/1000;lTrain.position.x=-74+((t*7)%148);loopBus.position.x=-78+((t*5)%156);southBus.position.x=-58+((t*3.8)%108);westBus.position.x=72-((t*3.6)%112);northBus.position.x=-58+((t*4.2)%116)
