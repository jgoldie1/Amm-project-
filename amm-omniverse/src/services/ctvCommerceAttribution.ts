import { createPaymentIntent, type PaymentIntent } from '../runtime/QuantumPaymentNetwork'
...
export function attributeVerifiedOrder(input:{campaignId:string;provider:string;sessionId:string;orderId:string;grossMinor:number;platformBps?:number;merchantDid:string;buyerDid?:string}){
 if(input.grossMinor<=0)throw new Error('Verified order total must be positive');const existing=settlements.get(input.orderId);if(existing)return existing
 const platformBps=Math.max(0,Math.min(10000,input.platformBps??1000)),platformMinor=Math.floor(input.grossMinor*platformBps/10000),merchantMinor=input.grossMinor-platformMinor
 recordCTVAttribution({campaignId:input.campaignId,provider:input.provider,type:'purchase',uraDid:input.buyerDid,sessionId:input.sessionId,orderId:input.orderId,valueMinor:input.grossMinor,currency:'USD'})
 const paymentIntent=createPaymentIntent({payerDid:input.buyerDid??'did:ura:tryamm:buyer',payeeDid:input.merchantDid,amountMinor:input.grossMinor,currency:'USD',purpose:`ctv-commerce:${input.orderId}`,idempotencyKey:`ctv:${input.orderId}`})
 const result:CommerceAttribution={campaignId:input.campaignId,provider:input.provider,sessionId:input.sessionId,orderId:input.orderId,grossMinor:input.grossMinor,platformMinor,merchantMinor,currency:'USD',touches:getSessionAttribution(input.sessionId),paymentIntent};settlements.set(input.orderId,result);return result
}
export function getCTVRevenueSummary(campaignId?:string){const rows=[...settlements.values()].filter(x=>!campaignId||x.campaignId===campaignId);return {orders:rows.length,grossMinor:rows.reduce((n,x)=>n+x.grossMinor,0),platformMinor:rows.reduce((n,x)=>n+x.platformMinor,0),merchantMinor:rows.reduce((n,x)=>n+x.merchantMinor,0),currency:'USD' as const}}
