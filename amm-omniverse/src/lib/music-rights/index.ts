...
// Release decisions are conservative: unknown rights block release until verified.
...
export type AssetOrigin =
  ...
  | 'cover'
  | 'interpolation'
  | 'third-party-master-sample'
  | 'remix'
  | 'unknown'

export interface RightsParty {
  id: string
  legalName: string
  role: 'writer' | 'publisher' | 'master-owner' | 'featured-artist' | 'producer' | 'performer' | 'administrator'
  sharePercent?: number
  pro?: 'ASCAP' | 'BMI' | 'SESAC' | 'GMR' | 'OTHER' | 'NONE' | 'UNKNOWN'
  ipiCae?: string
}

export interface EvidenceRef {
  id: string
  kind: 'license' | 'split-sheet' | 'work-for-hire' | 'sample-clearance' | 'cover-license' | 'public-domain-evidence' | 'consent' | 'other'
  storageRef: string
  checksum?: string
  effectiveAt?: string
  expiresAt?: string
...
export interface SampleAsset {
  ...
  origin: AssetOrigin
  source?: string
  masterPermission: RightsStatus
  compositionPermission: RightsStatus
  evidenceIds: string[]
  contentIdEligible?: boolean
}

export interface DistributionMetadata {
  isrc?: string
  upc?: string
  distributor?: 'DistroKid' | 'OTHER' | 'NONE'
  releaseDate?: string
  territories?: string[]
  explicit?: boolean
}
...
export interface RightsPassport {
  ...
  proMetadataVerified?: boolean
  mlcMetadataVerified?: boolean
  soundExchangeMetadataVerified?: boolean
  neighboringRightsReviewed?: boolean
  syncRightsReviewed?: boolean
  aiProvenanceRecorded?: boolean
  contentIdEligibilityReviewed?: boolean
  immutableReleaseSnapshot?: string
}
...
const sumShares = (parties: RightsParty[], role: RightsParty['role']) =>
  parties.filter(p => p.role === role).reduce((sum, p) => sum + (p.sharePercent ?? 0), 0)
...
export function evaluateReleaseGate(passport: RightsPassport): { status: ReleaseGateStatus; findings: GateFinding[] } {
  ...
  for (const sample of passport.samples) {
    if (sample.origin === 'third-party-master-sample' && (sample.masterPermission !== 'verified' || sample.compositionPermission !== 'verified')) {
      findings.push({ code: 'SAMPLE_CLEARANCE_REQUIRED', severity: 'BLOCK', message: `${sample.title}: third-party master and composition permissions must be verified.` })
    }
    if (sample.origin === 'interpolation' && sample.compositionPermission !== 'verified') {
      findings.push({ code: 'INTERPOLATION_CLEARANCE_REQUIRED', severity: 'BLOCK', message: `${sample.title}: composition permission must be verified.` })
    }
    ...
    if (sample.masterPermission === 'expired' || sample.compositionPermission === 'expired') findings.push({ code: 'RIGHTS_EXPIRED', severity: 'BLOCK', message: `${sample.title}: rights evidence is expired.` })
...
export const RELEASE_PIPELINE = [
  'CREATE', 'RIGHTS_SCAN', 'CLEARANCE', 'SPLITS', 'METADATA', 'ROYALTY_ROUTING',
  'IMMUTABLE_SNAPSHOT', 'DISTRIBUTION', 'MONITOR', 'COLLECT', 'LEDGER', 'PAYOUT',
] as const
