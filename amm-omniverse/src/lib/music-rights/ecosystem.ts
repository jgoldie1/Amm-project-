...
export type EcosystemNodeId = 'all-american-records' | 'spectra-entertainment' | 'all-american-network' | 'holo-music' | 'holoverse' | 'streetverse' | 'rights-release-engine' | 'creator-ledger'
export interface EcosystemNode { id: EcosystemNodeId; name: string; role: string; status: 'LOCKED' | 'BUILDING' | 'READY' | 'LIVE'; externalAgreementRequired?: boolean }
...
export const MUSIC_ECOSYSTEM_FLOW = ['StreetVerse discovery/create','All American Records A&R/project ownership','Rights & Release clearance','Holo Music immersive production','Spectra Entertainment production/sync/event coordination','All American Network owned-media launch','HoloVerse immersive experiences','Distribution adapters and external destinations','Usage/revenue reconciliation','Creator Ledger payout'] as const
export interface PartnerAuthorization { partnerNode: EcosystemNodeId; agreementId?: string; verified: boolean; scopes: string[]; territories?: string[]; expiresAt?: string }

export function canUseExternalPartner(node: EcosystemNode, authorization?: PartnerAuthorization, useAt: Date = new Date()): boolean {
  if (!node.externalAgreementRequired) return true
  if (!authorization?.verified || authorization.partnerNode !== node.id || !authorization.agreementId) return false
  if (authorization.expiresAt) {
    const expiry = Date.parse(authorization.expiresAt)
    if (!Number.isFinite(expiry) || useAt.getTime() > expiry) return false
  }
  return true
}
