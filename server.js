...
const FILES={
  knowledge:path.join(DATA_DIR,'knowledge.json'),feedback:path.join(DATA_DIR,'feedback.json'),payments:path.join(DATA_DIR,'payment-events.json'),content:path.join(DATA_DIR,'content.json'),games:path.join(DATA_DIR,'games.json'),rides:path.join(DATA_DIR,'rides.json'),deliveries:path.join(DATA_DIR,'deliveries.json'),arena:path.join(DATA_DIR,'arena-sessions.json'),collectibles:path.join(DATA_DIR,'collectibles.json'),gameSaves:path.join(DATA_DIR,'game-saves.json'),workers:path.join(DATA_DIR,'mobility-workers.json'),dispatch:path.join(DATA_DIR,'dispatch-offers.json'),tripEvents:path.join(DATA_DIR,'trip-events.json'),incidents:path.join(DATA_DIR,'safety-incidents.json')
};
...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'AMM Intelligence Phase 2',claude:claudeService.connected(),openAICompatible:Boolean(process.env.AI_API_URL&&process.env.AI_API_KEY&&process.env.AI_MODEL),stripe:stripeService.connected(),livekit:livekitService.connected(),meshy:meshyService.connected(),africaPaymentProvider:getProvider(),holo:true,yogihooPlayable:true,mobility:'mock-safe-advanced'}));
