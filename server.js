...
const gamePlatformApi=require('./services/game-platform-api');
const gameProductionApi=require('./services/game-production-api');
...
app.post('/api/ai/chat',async(req,res)=>{const message=String(req.body.message||'').trim();const mode=Object.hasOwn(modeInstructions,req.body.mode)?req.body.mode:'quick';const history=Array.isArray(req.body.history)?req.body.history:[];if(!message)return res.status(400).json({error:'Message is required.'});const knowledge=searchKnowledge(message,mode);const memories=await memoryApi.retrieveForRequest(req,message,{limit:6}).catch(()=>[]);const memorySources=memories.map(item=>({id:item.id,title:`Memory: ${item.title}`,category:`memory:${item.tier}`,content:item.content}));const sources=[...knowledge,...memorySources].slice(0,10);try{const answer=await callModel({message,mode,sources,history});res.json({answer:answer||localAnswer(message,mode,sources),mode,provider:answer?(claudeService.connected()?'claude':'connected-model'):'local-knowledge',memoryCount:memorySources.length,sources:sources.map(({id,title,category})=>({id,title,category}))});}catch(error){console.error(error);res.json({answer:localAnswer(message,mode,sources),mode,provider:'local-fallback',memoryCount:memorySources.length,sources:sources.map(({id,title,category})=>({id,title,category})),warning:'The connected AI provider was unavailable.'});}});
...
app.use('/api/memory',memoryApi.router);
app.use('/api/game-platform',gamePlatformApi);
app.use('/api/game-production',gameProductionApi);
