export default async function handler(req, res) {
  ...
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || process.env.VITE_APP_URL || 'https://tryamm.online';

  const checks = {
    app_https: /^https:\/\//.test(appUrl),
    supabase_url: Boolean(supabaseUrl),
    supabase_public_key: present('VITE_SUPABASE_ANON_KEY') || present('VITE_SUPABASE_PUBLISHABLE_KEY'),
  ...
  const criticalPassed = criticalKeys.filter((k) => checks[k]).length;
  const ready = criticalPassed === criticalKeys.length;

  let supabaseReachable = false;
  let supabaseStatus = null;
  if (supabaseUrl) {
    try {
      const r = await fetch(`${supabaseUrl.replace(/\/$/, '')}/auth/v1/health`, { method: 'GET' });
  ...
  res.status(ready && supabaseReachable ? 200 : 503).json({
    release: 'live-vite-readiness-1',
    site: 'tryamm.online',
    ready,
    criticalPassed,
    ...
    note: 'This endpoint reports presence/health only and never returns secret values.'
  });
}
