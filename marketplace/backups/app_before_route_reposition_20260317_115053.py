...
@app.route("/profile-db-center")
def profile_db_center_direct_safe():
    try:
        with app.app_context():
            db.create_all()
    except Exception:
    ...
    <h1>Profile DB Center</h1>
    <div class="card">
      <p>Database-backed profile tools are active in safe mode.</p>
    </div>
    <a href="/auth/login">Auth Login</a>
    <a href="/profile-safe">Profile</a>
    <a href="/db-role-preferences">DB Role Preferences</a>
    <a href="/db-favorites">DB Favorites</a>
    """
    ...
    if not is_auth:
        body += """
        <div class="card">
          <p>You are not logged in yet. You can still reach these pages, but saving data requires login.</p>
        </div>
        """
...
@app.route("/db-role-preferences", methods=["GET","POST"])
def db_role_preferences_direct_safe():
    try:
        with app.app_context():
            db.create_all()
    except Exception:
    ...
    if uid is None:
        ...
        <h1>DB Role Preferences</h1>
        <div class="card">
          <p>Please log in to save role preferences.</p>
        </div>
        <a href="/auth/login">Auth Login</a>
        <a href="/profile-db-center">Profile DB Center</a>
        """
