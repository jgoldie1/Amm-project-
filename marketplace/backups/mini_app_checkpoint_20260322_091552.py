...
def home():
    ...
    <div class="grid">
      ...
      <div class="card"><h2>Business Lanes</h2><a href="/business-lanes">Open</a></div>
      <div class="card"><h2>Event Center</h2><a href="/event-center">Open</a></div>
      <div class="card"><h2>Safe Diagnostics</h2><a href="/safe-diagnostics">Open</a></div>
...
def ask_ai():
    ...
    elif "event" in user_message or "events" in user_message:
        response = "Opening the Event Center."
    elif "jacobie" in user_message:
        response = "Opening Jacobie's business lane."
    elif "isaiah" in user_message:
        response = "Opening Isaiah's business lane."
    elif "aniyah" in user_message:
        response = "Opening Aniyah's business lane."
    elif "alton kevon" in user_message or "kevon" in user_message:
        response = "Opening Alton Kevon's business lane."
    elif "big al" in user_message or user_message == "al":
        response = "Opening Al's business lane."
...
def ask_ui():
    ...
    <div class="card">
      ...
      <button onclick="go('/business-lanes')">Business Lanes</button>
      <button onclick="go('/event-center')">Event Center</button>
    </div>
    ...
    function pickRoute(msg){
      ...
      if (v.includes('event')) return '/event-center';
      if (v.includes('jacobie')) return '/business-lane/jacobie';
      if (v.includes('isaiah')) return '/business-lane/isaiah';
      if (v.includes('aniyah')) return '/business-lane/aniyah';
      if (v.includes('alton kevon') || v.includes('kevon')) return '/business-lane/alton-kevon';
      if (v === 'al' || v.includes('big al')) return '/business-lane/al';
...
def zone_dashboard():
    try:
        ...
        return page("Zone Dashboard Error", f"<div class='card'><h1>Error loading zones</h1><pre>{str(e)}</pre></div>")

    cards = ""
    for r in rows:
        zone = r["zone_name"] if "zone_name" in r.keys() else ""
        scope = r["scope_type"] if "scope_type" in r.keys() else ""
        region = r["region_name"] if "region_name" in r.keys() else ""
        module = r["linked_module"] if "linked_module" in r.keys() else ""
        purpose = r["purpose"] if "purpose" in r.keys() else ""
        notes = r["notes"] if "notes" in r.keys() else ""

        cards += f"<div class='card'><h2>{zone}</h2><p><strong>Scope:</strong> {scope}</p><p><strong>Region:</strong> {region}</p><p><strong>Module:</strong> {module}</p><p><strong>Purpose:</strong> {purpose}</p><p>{notes}</p></div>"

    if not cards:
        cards = "<div class='card'><p>No saved zones yet.</p></div>"

    return page("Zone Dashboard", f"""
    <div class="card">
      <h1>Zone Dashboard</h1>
      <p>Saved domestic and global zones.</p>
    </div>
    <div class="grid">
...
def metaverse_webgl():
    ...
    <div class="card">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;">
        <button id="touch-up" onclick="move('w')" style="min-width:90px;min-height:54px;font-size:18px;">Up</button>
        <button id="touch-left" onclick="move('a')" style="min-width:90px;min-height:54px;font-size:18px;">Left</button>
        <button id="touch-down" onclick="move('s')" style="min-width:90px;min-height:54px;font-size:18px;">Down</button>
        <button id="touch-right" onclick="move('d')" style="min-width:90px;min-height:54px;font-size:18px;">Right</button>
      </div>
      <p><small>Phone-safe controls for metaverse movement testing.</small></p>
    </div>
    ...
    <div class="card">
      ...
      <button onclick="go('/aam-holographic-streaming-ecosystem')">Ecosystem</button>
      <button onclick="go('/aam-university')">University</button>
      <button onclick="go('/safe-diagnostics')">Diagnostics</button>
    </div>

    <div class="card">
      <h2>Player Status</h2>
      <pre id="player-status">Player position: X=0 Y=0</pre>
    </div>

    <div class="card">
      <h2>Phone Play Mode</h2>
      <p>Use the touch controls on your phone screen to test movement.</p>
      <p>You can also cast or mirror your phone to a TV while using the phone as the controller surface.</p>
    </div>
    ...
    function renderPlayerStatus(){
      let el = document.getElementById('player-status');
      if(!el) return;
      el.textContent = "Player position: X=" + playerX + " Y=" + playerY;
    }
    ...
    function move(key){
      ...
      if(key === 'a') playerX -= 1;
      if(key === 'd') playerX += 1;
      console.log("Move:", key, playerX, playerY);
      renderPlayerStatus();
    }

    window.addEventListener('load', renderPlayerStatus);
    </script>
    """)


@app.route("/business-lanes")
def business_lanes():
    try:
        c = conn()
        c.row_factory = sqlite3.Row
        rows = c.execute("SELECT slug, person_name, lane_title FROM heir_business_lanes ORDER BY id").fetchall()
        c.close()
    except Exception as e:
        return page("Business Lanes Error", f"<div class='card'><h1>Error loading business lanes</h1><pre>{str(e)}</pre></div>")

    cards = ""
    for r in rows:
        slug = r["slug"] if "slug" in r.keys() else ""
        person_name = r["person_name"] if "person_name" in r.keys() else ""
        lane_title = r["lane_title"] if "lane_title" in r.keys() else ""
        cards += f"<div class='card'><h2>{person_name}</h2><p>{lane_title}</p><a href='/business-lane/{slug}'>Open</a></div>"

    if not cards:
        cards = "<div class='card'><p>No business lanes saved yet.</p></div>"

    return page("Business Lanes", f"""
    <div class="card">
      <h1>Heir Business Lanes</h1>
      <p>Dedicated business categories for each heir.</p>
    </div>
    <div class="grid">{cards}</div>
    """)
@app.route("/business-lane/<slug>")
def business_lane(slug):
    try:
        c = conn()
        c.row_factory = sqlite3.Row
        row = c.execute("SELECT * FROM heir_business_lanes WHERE slug=?", (slug,)).fetchone()
        c.close()
    except Exception as e:
        return page("Business Lane Error", f"<div class='card'><h1>Error loading business lane</h1><pre>{str(e)}</pre></div>")

    if not row:
        return page("Business Lane", "<div class='card'><h1>Business lane not found.</h1></div>")

    person_name = row["person_name"] if "person_name" in row.keys() else ""
    lane_title = row["lane_title"] if "lane_title" in row.keys() else ""
    business_items = row["business_items"] if "business_items" in row.keys() else ""
    items_html = "<br>".join([item.strip() for item in str(business_items).split(",") if item.strip()])

    return page("Business Lane", f"""
    <div class="card">
      <h1>{lane_title}</h1>
...
def event_center():
    try:
        c = conn()
        c.row_factory = sqlite3.Row
        rows = c.execute("SELECT event_title, event_type, event_date, notes FROM event_items ORDER BY id DESC LIMIT 50").fetchall()
        c.close()
    except Exception as e:
        return page("Event Center Error", f"<div class='card'><h1>Error loading events</h1><pre>{str(e)}</pre></div>")

    cards = ""
    for r in rows:
        event_title = r["event_title"] if "event_title" in r.keys() else ""
        event_type = r["event_type"] if "event_type" in r.keys() else ""
        event_date = r["event_date"] if "event_date" in r.keys() else ""
        notes = r["notes"] if "notes" in r.keys() else ""
        cards += f"<div class='card'><h2>{event_title}</h2><p><strong>Type:</strong> {event_type}</p><p><strong>Date:</strong> {event_date}</p><p>{notes}</p></div>"

    if not cards:
        cards = "<div class='card'><p>No events yet.</p></div>"

    return page("Event Center", f"""
    <div class="card">
      <h1>Event Center</h1>
      <p>Birthdays, launches, reminders, and milestone events.</p>
    </div>
    <div class="grid">
      <div class="card"><h2>Add Event</h2><a href="/event-planner">Open</a></div>
      <div class="card"><h2>Birthday Center</h2><a href="/birthday-center">Open</a></div>
      <div class="card"><h2>System Status</h2><a href="/system-status">Open</a></div>
    </div>
    <div class="grid">{cards}</div>
...
def event_planner():
    ...
    if request.method == "POST":
        ...
        cur = c.cursor()
        cur.execute(
            "INSERT INTO event_items (event_title, event_type, event_date, notes) VALUES (?, ?, ?, ?)",
            (
        ...
        c.commit()
        c.close()
        msg = "<p><strong>Event saved.</strong></p>"

    return page("Event Planner", f"""
    <div class="card">
      <h1>Event Planner</h1>
      <p>Create birthdays, launches, reminders, and milestones.</p>
      {msg}
    </div>
    <div class="card">
      <form method="post">
...
def safe_diagnostics():
    return page("Safe Diagnostics", """
    <div class="card">
      <h1>Safe Diagnostics</h1>
      <p>This page is a lightweight diagnostics shell for the current stable prototype.</p>
    </div>
    ...
    <div class="grid">
      ...
      <div class="card"><h2>Zone Dashboard</h2><a href="/zone-dashboard">Open</a></div>
      <div class="card"><h2>Metaverse WebGL</h2><a href="/metaverse-webgl">Open</a></div>
      <div class="card"><h2>Ask UI</h2><a href="/ask-ui">Open</a></div>
    </div>

    <div class="card">
      <h2>What This Confirms</h2>
      <p>The server is running, core routes are reachable, and the stable checkpoint is active.</p>
    </div>
    """)
