...
def page(title, body):
    ...
    <link rel="stylesheet" href="/static/css/holo.css"></head>
    <body>
    <img src="/static/img/holographic_lion_saturn.svg" class="lion-left">
    <img src="/static/img/holographic_lion_saturn.svg" class="lion-right">
...
def _protected_html(title, role_needed):
    ...
    <div class="shell">
      <div class="top-nav">
        ...
        <a href="/media-demo-home">Media</a>
      </div>
...
def activity_feed_center_legacy():
    body = """
    <div class="kid-hero">
...
@app.route("/moderation-center")
def moderation_center_legacy__moderation_center__2():
    body = """
    <div class="kid-hero">
      <h1>Moderation Center</h1>
...
def launch_readiness_center_legacy():
    body = """
    <div class="kid-hero">
...
def _brand_asset_url(prefix):
    import os
    exts = [".png", ".jpg", ".jpeg", ".webp", ".png"]
    for ext in exts:
        rel = f"static/brand/{prefix}{ext}"
        if os.path.exists(rel):
            return "/" + rel
    return None

@app.route("/brand-gallery-real")
def brand_gallery_real():
    crest = _brand_asset_url("stubbs_crest_real") or ""
    flag = _brand_asset_url("american_flag_real") or ""
    saturn = _brand_asset_url("saturn_real") or ""
    lion = _brand_asset_url("lion_real") or ""

    body = f"""
    <div class="brand-hero">
      <h1>Brand Gallery</h1>
      <p>This gallery uses your real local assets first. If one is missing, the app uses a clean fallback so the site stays stable.</p>
    </div>

    <div class="brand-grid">
      <div class="brand-card">
        <h2>Stubbs Crest</h2>
        <img src="{crest}" alt="Stubbs Crest">
        <p>Primary family crest surface.</p>
      </div>
      <div class="brand-card">
        <h2>American Flag</h2>
        <img src="{flag}" alt="American Flag">
        <p>Patriotic flagship identity.</p>
      </div>
      <div class="brand-card">
        <h2>Saturn</h2>
        <img src="{saturn}" alt="Saturn">
        <p>Planetary brand symbol and omniverse identity.</p>
      </div>
      <div class="brand-card">
        <h2>Holographic Lion</h2>
        <img src="{lion}" alt="Lion">
        <p>Strength, royalty, and flagship dominance symbol.</p>
      </div>
    </div>
    """
    if 'holo_commerce_shell' in globals():
        return holo_commerce_shell("Brand Gallery", body)
    if 'launch_finish_shell' in globals():
        return launch_finish_shell("Brand Gallery", body)
    if 'heirs_shell' in globals():
        return heirs_shell("Brand Gallery", body)
    return body


def brand_url(name):
    return f"/static/brand/{name}.png"

@app.route("/brand-system")
def brand_system():
    body = f"""
    <div class="hero">
      <h1>Brand System (Final)</h1>
      <p>All real assets are installed, cropped, optimized, and stabilized.</p>
    </div>
    ...
    <div class="grid" style="margin-top:20px">
      ...
      <div class="card">
        <h2>Stubbs Crest</h2>
        <img src="{brand_url('crest')}" style="width:100%">
      </div>

      <div class="card">
        <h2>El Saturn</h2>
        <img src="{brand_url('saturn')}" style="width:100%">
      </div>

      <div class="card">
        <h2>American Flag</h2>
        <img src="{brand_url('flag')}" style="width:100%">
      </div>

      <div class="card">
        <h2>Holographic Lion</h2>
        <img src="{brand_url('lion')}" style="width:100%">
      </div>
    ...
    if 'holo_commerce_shell' in globals():
        return holo_commerce_shell("Brand System", body)
    return body
...
def beta_nda():
    return f"""
    <div style="max-width:700px;margin:auto;color:white;padding:20px">
    <h1>Private Beta Access Agreement</h1>

    <p><b>NOTICE:</b> This is a private heirs-only beta system.</p>

    <form method="POST" action="/beta-nda-sign">

    <input name="full_name" placeholder="Full Name" required style="width:100%;padding:10px;margin:10px 0">
    <input name="email" placeholder="Email" required style="width:100%;padding:10px;margin:10px 0">
    <input name="birthday" placeholder="Birthday (YYYY-MM-DD)" required style="width:100%;padding:10px;margin:10px 0">

    <h3>NDA + Beta Terms</h3>
    <p>
    You agree NOT to share, record, distribute, or disclose any part of this system.
    This platform is under active development and may contain incomplete features.
    All access is monitored and logged.
    </p>

    <label>
    <input type="checkbox" name="agree" required> I Agree to NDA + Beta Terms
    </label>

    <input name="signature" placeholder="Type Full Name as Signature" required style="width:100%;padding:10px;margin:10px 0">

    <button style="padding:12px 20px;background:#22c55e;border:none;border-radius:8px;color:white">
    Sign & Enter
    </button>
...
@app.route("/beta-nda-sign", methods=["POST"])
def beta_nda_sign():
    import sqlite3
    ...
    cur.execute("""
    INSERT INTO beta_users_v1 (full_name, email, birthday, signed_name, ip_address, agreed)
    VALUES (?, ?, ?, ?, ?, 1)
    """, (
        data.get("full_name"),
        data.get("email"),
    ...
    session["beta_user"] = data.get("email")

    return redirect("/heirs-app")
...
def beta_required():
    if "beta_user" not in session:
        return redirect("/beta-nda")
    return None

@app.route("/heirs-app")
def heirs_app__heirs_app__2():
    guard = beta_required()
    if guard:
        return guard

    return "<h1 style='color:white;padding:40px'>Welcome to Heirs App</h1>"
...
def beta_admin():
    ...
    users = conn.execute("SELECT * FROM beta_users_v1 ORDER BY id DESC").fetchall()
    conn.close()

    rows = ""
    for u in users:
        rows += f"<tr><td>{u['full_name']}</td><td>{u['email']}</td><td>{u['birthday']}</td><td>{u['created_at']}</td></tr>"

    return f"""
    <h1 style="color:white">Beta Users</h1>
    <table border=1 style="color:white">
    <tr><th>Name</th><th>Email</th><th>Birthday</th><th>Joined</th></tr>
    {rows}
    </table>
    """
...
def full_holo_shell(title, body_html):
    nav = """
    <div class="topbar">
      <div class="topbar-inner">
        <div class="brand">AAME Full Holographic App</div>
        <div class="nav">
          <a href="/full-app-showcase">Showcase</a>
          <a href="/heirs-app">Heirs App</a>
          <a href="/holoverse-lobby-v2">Holoverse</a>
          <a href="/holo-commerce-home">Commerce</a>
          <a href="/launch-ops-center-v2">Launch Ops</a>
          <a href="/founder-command-center">Founder Command</a>
        </div>
      </div>
    </div>
    """
    ...
    <head>
      ...
      <title>{title}</title>
      <link rel="stylesheet" href="/static/css/heirs_launch.css">
      <link rel="stylesheet" href="/static/css/child_theme_launch.css">
      <link rel="stylesheet" href="/static/css/launch_finish.css">
      <link rel="stylesheet" href="/static/css/holo_commerce_premium.css">
      <link rel="stylesheet" href="/static/css/holoverse_avatar.css">
      <link rel="stylesheet" href="/static/css/brand_gallery.css">
      <link rel="stylesheet" href="/static/css/full_holo_showcase.css">
    <link rel="stylesheet" href="/static/css/holo.css"></head>
    <body>
    <img src="/static/img/holographic_lion_saturn.svg" class="lion-left">
    <img src="/static/img/holographic_lion_saturn.svg" class="lion-right">
...
def _brand_asset_url_show(prefix):
    import os
...
@app.route("/full-app-showcase")
def full_app_showcase():
    crest = _real_brand_png("stubbs_crest_real") or _brand_asset_url_show("crest")
    flag = _real_brand_png("american_flag_real") or _brand_asset_url_show("flag")
    saturn = _real_brand_png("saturn_real") or _brand_asset_url_show("saturn")
    lion = _real_brand_png("lion_real") or _brand_asset_url_show("lion")

    body = f"""
    <div class="full-hero">
      <h1>All American Marketplace</h1>
      <p>This is the full visual ecosystem showcase with heirs-first access, holographic branding, holoverse navigation, creator tools, launch ops, world hubs, live-commerce layers, reports, moderation, and founder control surfaces.</p>
      <div class="hero-row">
        <span class="hero-chip">Heirs-Only Soft Launch</span>
        <span class="hero-chip">Holographic Streaming Ecosystem</span>
        <span class="hero-chip">Holoverse Worlds</span>
        <span class="hero-chip">Live Commerce</span>
        <span class="hero-chip">Founder Command</span>
      </div>
    </div>

    <div class="brand-row">
      <div class="brand-tile"><h3>Stubbs Crest</h3><img src="{crest}" alt="Stubbs Crest"></div>
      <div class="brand-tile"><h3>American Flag</h3><img src="{flag}" alt="American Flag"></div>
      <div class="brand-tile"><h3>El Saturn</h3><img src="{saturn}" alt="Saturn"></div>
      <div class="brand-tile"><h3>Holographic Lion</h3><img src="{lion}" alt="Lion"></div>
    </div>

    <div class="section-title">Core launch surfaces</div>
    <div class="show-grid">
      <div class="show-card"><h2>Heirs App</h2><p>Private launch home with simplified entry for heirs and trusted beta users.</p><a class="btn" href="/heirs-app">Open</a></div>
      <div class="show-card"><h2>Launchpad</h2><p>Main clean launch entry for uploads, bookings, browsing, and reports.</p><a class="btn" href="/heirs-launchpad">Open</a></div>
      <div class="show-card"><h2>Founder Command</h2><p>Top-level oversight for gaps, risks, AI, world expansion, and readiness.</p><a class="btn" href="/founder-command-center">Open</a></div>
      <div class="show-card"><h2>Launch Ops</h2><p>Invites, joins, activity, moderation, and readiness tracking.</p><a class="btn" href="/launch-ops-center-v2">Open</a></div>
    </div>

    <div class="section-title">Creator + operations layer</div>
    <div class="show-grid">
      <div class="show-card"><h2>Files Center</h2><p>Upload management, tags, and file actions.</p><a class="btn" href="/files-center-v4">Open</a></div>
      <div class="show-card"><h2>Bookings Center</h2><p>Service bookings and actions.</p><a class="btn" href="/bookings-center-v4">Open</a></div>
      <div class="show-card"><h2>Releases Center</h2><p>Artist releases, launch status, and promo prep.</p><a class="btn" href="/releases-center-v4">Open</a></div>
      <div class="show-card"><h2>Workflows Center</h2><p>Template-based automation flows.</p><a class="btn" href="/workflows-center-v4">Open</a></div>
    </div>

    <div class="section-title">Holographic world + identity layer</div>
    <div class="show-grid">
      <div class="show-card"><h2>Avatar Creator</h2><p>Create persistent identity for the ecosystem.</p><a class="btn" href="/avatar-creator">Open</a></div>
      <div class="show-card"><h2>Holoverse Lobby</h2><p>Portal-based entry to worlds and mission zones.</p><a class="btn" href="/holoverse-lobby-v2">Open</a></div>
      <div class="show-card"><h2>Teleport Center</h2><p>Jump between world hubs and future regions.</p><a class="btn" href="/teleport-center">Open</a></div>
      <div class="show-card"><h2>Presence Center</h2><p>Lightweight online room presence shell.</p><a class="btn" href="/presence-center">Open</a></div>
    </div>

    <div class="section-title">Premium commerce + streaming layer</div>
    <div class="show-grid">
      <div class="show-card"><h2>Holo Commerce</h2><p>Premium storefront and ecosystem commerce home.</p><a class="btn" href="/holo-commerce-home">Open</a></div>
      <div class="show-card"><h2>Live Shopping</h2><p>Founder-led and creator-led live-selling shell.</p><a class="btn" href="/live-shopping-center">Open</a></div>
      <div class="show-card"><h2>Drops Center</h2><p>Seasonal, founder, and creator drops.</p><a class="btn" href="/drops-center">Open</a></div>
      <div class="show-card"><h2>Shop Discovery</h2><p>Premium category-based discovery.</p><a class="btn" href="/shop-discovery-v2">Open</a></div>
    </div>

    <div class="section-title">Growth + protection layer</div>
    <div class="show-grid">
      <div class="show-card"><h2>Report Center</h2><p>Bug reporting and issue capture.</p><a class="btn" href="/report-center">Open</a></div>
      <div class="show-card"><h2>Streaming Review</h2><p>Clip queue and media review pipeline.</p><a class="btn" href="/streaming-review-center">Open</a></div>
      <div class="show-card"><h2>Moderation</h2><p>Admin review and control actions.</p><a class="btn" href="/moderation-center-v2">Open</a></div>
      <div class="show-card"><h2>Announcements</h2><p>Notifications and launch messaging.</p><a class="btn" href="/announcements-center-v2">Open</a></div>
    </div>

    <div class="section-title">World expansion + trust layer</div>
    <div class="show-grid">
      <div class="show-card"><h2>World Expansion</h2><p>UK, Atlantic, Africa, Americas, and regional growth grouping.</p><a class="btn" href="/world-expansion-center-v2">Open</a></div>
      <div class="show-card"><h2>Armed Forces Center</h2><p>Honor lane for service members and families.</p><a class="btn" href="/armed-forces-center">Open</a></div>
      <div class="show-card"><h2>Military Verification</h2><p>Structured verification intake.</p><a class="btn" href="/military-verification-center">Open</a></div>
      <div class="show-card"><h2>AI Assistant Center</h2><p>Guidance, support, learning, and smart task help.</p><a class="btn" href="/ai-assistant-center">Open</a></div>
    </div>
    """
    return full_holo_shell("Full App Showcase", body)
...
def _fivepm_brand(name):
    import os
    for rel in [
        f"static/brand/{name}.png",
        f"static/brand/{name}.svg",
        f"static/img/{name}.png",
        f"static/img/{name}.svg",
    ]:
        if os.path.exists(rel):
            return "/" + rel
    return ""

def fivepm_shell(title, body_html):
    nav = """
    <div class="topbar">
      <div class="topbar-inner">
        <div class="brand">AAME Family Launch</div>
        <div class="nav">
          <a href="/app-home">Home</a>
          <a href="/family-launch-home">Family Launch</a>
          <a href="/media-demo-home">Media</a>
          <a href="/holoverse-lobby-v2">Holoverse</a>
          <a href="/holo-commerce-home">Commerce</a>
          <a href="/founder-command-center">Founder</a>
        </div>
      </div>
    </div>
    """
    ...
    <head>
      ...
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>{title}</title>
      <link rel="stylesheet" href="/static/css/fivepm_lock.css">
...
def family_launch_home():
    ...
    <head>
      ...
      <link rel="stylesheet" href="/static/css/fivepm_lock.css">
      <link rel="stylesheet" href="/static/css/final_home.css">
    </head>
    <body>
      <div class="shell">
        ...
        <div class="home-hero">
          <h1>Family App</h1>
          <p>Welcome Aniyah — your birthday experience is live 🎉</p>
        </div>
        ...
        <div class="home-grid">
          ...
          <div class="home-card">
            <h2>🎂 Birthday</h2>
            <p>View birthday page and celebration</p>
            <a href="/birthday-center">Open</a>
          </div>

          <div class="home-card">
            <h2>🎁 NFT Gifts</h2>
            <p>Create or claim birthday gift</p>
            <a href="/birthday-nft-gift-center">Open</a>
          </div>

          <div class="home-card">
            <h2>📺 Shows</h2>
            <p>Watch shows demo</p>
            <a href="/shows-demo">Open</a>
          </div>

          <div class="home-card">
            <h2>🎵 Music</h2>
            <p>Play music demo</p>
            <a href="/music-demo">Open</a>
          </div>

          <div class="home-card">
            <h2>🎤 Coaching</h2>
            <p>Music coaching & recording</p>
            <a href="/music-coaching-home">Open</a>
          </div>

          <div class="home-card">
            <h2>⭐ Artist Signing</h2>
            <p>Artist signup & intake</p>
            <a href="/artist-signing-home">Open</a>
          </div>
          <div class="home-card">
            <h2>🎉 Aniyah Welcome</h2>
            <p>Birthday welcome page</p>
            <a href="/welcome-aniyah">Open</a>
          </div>
          <div class="home-card">
            <h2>🛡 Alton Kevon</h2>
            <p>Special family welcome page</p>
            <a href="/welcome-alton-kevon">Open</a>
          </div>
          <div class="home-card">
            <h2>🔗 Share</h2>
            <p>Copy same-Wi-Fi links</p>
            <a href="/share-family-launch">Open</a>
          </div>
          <div class="home-card">
            <h2>👑 Jacobie</h2>
            <p>Personal welcome and business lane</p>
            <a href="/welcome-jacobie">Open</a>
          </div>
          <div class="home-card">
            <h2>⭐ Isaiah</h2>
            <p>Personal welcome and business lane</p>
            <a href="/welcome-isaiah">Open</a>
          </div>
          <div class="home-card">
            <h2>🛡 Brother Alton</h2>
            <p>Business and leadership welcome page</p>
            <a href="/welcome-brother-alton">Open</a>
          </div>
          <div class="home-card">
            <h2>👑 Jacobie</h2>
            <p>Personal welcome and business lane</p>
...
@app.route("/media-demo-home")
def media_demo_home():
    body = """
    <div class="hero">
      <h1>Media Demo Home</h1>
      <p>This is the simple family media entry for streaming TV, music, creator clips, and music coaching/signing demos.</p>
      <div class="badges">
        <span class="badge">Shows</span>
        <span class="badge">Music</span>
        <span class="badge">Clips</span>
        <span class="badge">Coaching</span>
      </div>
    </div>

    <div class="grid" style="margin-top:18px">
      <div class="card"><h2>Streaming App</h2><p>Main streaming shell for shows and channels.</p><a class="btn" href="/streaming-app">Open</a></div>
      <div class="card"><h2>Music App</h2><p>Artists, playlists, and release surfaces.</p><a class="btn" href="/music-app">Open</a></div>
      <div class="card"><h2>Streaming Review</h2><p>Clip queue and media review pipeline.</p><a class="btn" href="/streaming-review-center">Open</a></div>
      <div class="card"><h2>Vocal Coach Studio</h2><p>Voice training and recording workflow shell.</p><a class="btn" href="/vocal-coach-studio-recorder">Open</a></div>
    </div>
    """
    return fivepm_shell("Media Demo Home", body)

@app.route("/creator-tools-home")
def creator_tools_home():
    body = """
    <div class="hero">
      <h1>Creator Tools Home</h1>
      <p>Simple access to uploads, bookings, releases, and workflows for family-first beta use.</p>
    </div>

    <div class="grid" style="margin-top:18px">
      <div class="card"><h2>Files Center</h2><p>Uploads and file actions.</p><a class="btn" href="/files-center-v4">Open</a></div>
      <div class="card"><h2>Bookings Center</h2><p>Services and booking flow.</p><a class="btn" href="/bookings-center-v4">Open</a></div>
      <div class="card"><h2>Releases Center</h2><p>Music and artist release flow.</p><a class="btn" href="/releases-center-v4">Open</a></div>
      <div class="card"><h2>Workflows Center</h2><p>Template-based actions.</p><a class="btn" href="/workflows-center-v4">Open</a></div>
    </div>
    """
    return fivepm_shell("Creator Tools Home", body)


def media_demo_shell(title, body_html):
    nav = """
    <div class="topbar">
      <div class="topbar-inner">
        <div class="brand">AAME Media Demo</div>
        <div class="nav">
          <a href="/family-launch-home">Family Launch</a>
          <a href="/media-demo-home">Media Home</a>
          <a href="/shows-demo">Shows</a>
          <a href="/music-demo">Music</a>
          <a href="/music-coaching-home">Coaching</a>
          <a href="/artist-signing-home">Signing</a>
        </div>
      </div>
    </div>
    """
    ...
    <head>
      ...
      <link rel="stylesheet" href="/static/css/birthday.css">
      <link rel="stylesheet" href="/static/css/birthday.css">
      <link rel="stylesheet" href="/static/css/media_demo.css">
    </head>
    <body>
      {nav}
...
@app.route("/api/media-shows-v1")
def api_media_shows_v1():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM media_shows_v1 ORDER BY id").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/media-tracks-v1")
def api_media_tracks_v1():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM media_tracks_v1 ORDER BY id").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/music-coaching-requests-v1")
def api_music_coaching_requests_v1():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM music_coaching_requests_v1 ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/create-music-coaching-request-v1", methods=["POST"])
def api_create_music_coaching_request_v1():
    from flask import request
    import sqlite3
    payload = request.get_json(silent=True) or request.form
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO music_coaching_requests_v1 (student_name, email, goal, session_type, status, notes) VALUES (?, ?, ?, ?, 'submitted', ?)",
        (student_name, email, goal, session_type, notes)
    )
    conn.commit()
    conn.close()
    return jsonify({"ok": True})

@app.route("/api/artist-signing-intake-v1")
def api_artist_signing_intake_v1():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM artist_signing_intake_v1 ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/create-artist-signing-intake-v1", methods=["POST"])
def api_create_artist_signing_intake_v1():
    from flask import request
    import sqlite3
    payload = request.get_json(silent=True) or request.form
    artist_name = (payload.get("artist_name") or "").strip()
    email = (payload.get("email") or "").strip()
    style_name = (payload.get("style_name") or "").strip()
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO artist_signing_intake_v1 (artist_name, email, style_name, current_stage, status, notes) VALUES (?, ?, ?, ?, 'submitted', ?)",
        (artist_name, email, style_name, current_stage, notes)
    )
    conn.commit()
    conn.close()
    return jsonify({"ok": True})
...
@app.route("/media-demo-home")
def media_demo_home_v2():
    body = """
    <div class="media-hero">
      <h1>Media Demo Home</h1>
      <p>This is the clean family media demo for shows, music, coaching, and artist signing. It is the usable demo version for today.</p>
      <div class="badges">
        <span class="badge">Shows</span>
        <span class="badge">Music</span>
        <span class="badge">Coaching</span>
        <span class="badge">Artist Signing</span>
      </div>
    </div>

    <div class="media-grid" style="margin-top:18px">
      <div class="media-card"><h2>Shows Demo</h2><p>Simple family-first show discovery lane.</p><a class="btn" href="/shows-demo">Open</a></div>
      <div class="media-card"><h2>Music Demo</h2><p>Simple music discovery lane.</p><a class="btn" href="/music-demo">Open</a></div>
      <div class="media-card"><h2>Music Coaching</h2><p>Request a vocal/music coaching session.</p><a class="btn" href="/music-coaching-home">Open</a></div>
      <div class="media-card"><h2>Artist Signing</h2><p>Artist intake lane for future label growth.</p><a class="btn" href="/artist-signing-home">Open</a></div>
    </div>
    """
    return media_demo_shell("Media Demo Home", body)
...
def shows_demo():
    ...
    <div class="media-hero">
      <h1>Shows Demo</h1>
      <p>Featured show lanes for the family beta.</p>
    </div>
    <div class="media-grid" style="margin-top:18px">
      <div class="media-card"><h2>Heirs Launch Show</h2><p>Family-first welcome show lane.</p></div>
      <div class="media-card"><h2>Holo Stream Spotlight</h2><p>Creator and clip spotlight lane.</p></div>
      <div class="media-card"><h2>Bloom Wellness Sessions</h2><p>Wellness and encouragement lane.</p></div>
      <div class="media-card"><h2>Big Al Records Sessions</h2><p>Music and artist development lane.</p></div>
    </div>
    <div class="card" style="margin-top:18px"><a class="btn" href="/api/media-shows-v1">Open Shows Data</a></div>
    """
    return media_demo_shell("Shows Demo", body)
...
def music_demo():
    ...
    <div class="media-hero">
      <h1>Music Demo</h1>
      <p>Featured music lanes for family-first beta listening and artist identity.</p>
    </div>
    <div class="media-grid" style="margin-top:18px">
      <div class="media-card"><h2>Rise In Sound</h2><p>Aniyah featured demo lane.</p></div>
      <div class="media-card"><h2>Platform Anthem</h2><p>Main anthem lane for the platform.</p></div>
      <div class="media-card"><h2>Lifted Voices</h2><p>Faith-centered music lane.</p></div>
      <div class="media-card"><h2>Night Drive Holo Mix</h2><p>Cinematic promo music lane.</p></div>
    </div>
    <div class="card" style="margin-top:18px"><a class="btn" href="/api/media-tracks-v1">Open Music Data</a></div>
    """
    return media_demo_shell("Music Demo", body)

@app.route("/music-coaching-home")
def music_coaching_home():
    body = """
    <div class="media-hero">
      <h1>Music Coaching</h1>
      <p>Simple intake page for vocal coaching, practice help, and music growth support.</p>
    </div>

    <div class="grid" style="margin-top:18px">
      <div class="card">
        <h2>Request Coaching</h2>
        <form id="coachForm">
          <input name="student_name" placeholder="Student Name">
          <input name="email" placeholder="Email">
          <input name="goal" placeholder="Goal (singing, confidence, practice, writing)">
          <select name="session_type">
            <option value="vocal">vocal</option>
            <option value="recording">recording</option>
            <option value="artist_development">artist_development</option>
          </select>
          <textarea name="notes">Music coaching request</textarea>
          <button type="button" onclick="submitCoach()">Submit Coaching Request</button>
        </form>
      </div>
      <div class="card">
        <h2>What This Does</h2>
        <p>This gives your family beta a real coaching intake flow for music growth and signing preparation.</p>
        <a class="btn-secondary" href="/api/music-coaching-requests-v1">Open Coaching Requests</a>
      </div>
    </div>

    <script>
    async function submitCoach() {
      const fd = new FormData(document.getElementById('coachForm'));
      const res = await fetch('/api/create-music-coaching-request-v1', {method:'POST', body:fd});
      const data = await res.json();
      alert(data.ok ? 'Coaching request submitted' : 'Failed');
    }
    </script>
    """
    return media_demo_shell("Music Coaching", body)

@app.route("/artist-signing-home")
def artist_signing_home():
    body = """
    <div class="media-hero">
      <h1>Artist Signing</h1>
      <p>Simple artist intake page for future development, review, and signing flow.</p>
    </div>
    ...
    <div class="grid" style="margin-top:18px">
      <div class="card">
        ...
        <form id="signForm">
          ...
          <textarea name="notes">Artist signing intake request</textarea>
          <button type="button" onclick="submitSign()">Submit Artist Intake</button>
        </form>
      </div>
      <div class="card">
        <h2>What This Does</h2>
        <p>This creates a real artist intake path instead of only talking about signing and development.</p>
        <a class="btn-secondary" href="/api/artist-signing-intake-v1">Open Artist Intake Data</a>
      </div>
    </div>

    <script>
    async function submitSign() {
      const fd = new FormData(document.getElementById('signForm'));
      const res = await fetch('/api/create-artist-signing-intake-v1', {method:'POST', body:fd});
      const data = await res.json();
      alert(data.ok ? 'Artist intake submitted' : 'Failed');
    }
    </script>
    """
    return media_demo_shell("Artist Signing", body)


def media_player_shell(title, body):
    return f"""
    <!doctype html>
    <html>
    <head>
      ...
      <link rel="stylesheet" href="/static/css/birthday.css">
      <link rel="stylesheet" href="/static/css/media_demo.css">
      <link rel="stylesheet" href="/static/css/media_polish.css">
    </head>
    <body>
...
def watch_show(show_id):
    return media_player_shell("Watch Show", f"""
    <div class="media-hero">
      <a class="back-btn" href="/shows-demo">&larr; Back</a><h1>Now Watching</h1>
      <p>Show ID: {show_id}</p>
    </div>
...
def play_track(track_id):
    return media_player_shell("Play Track", f"""
    <div class="media-hero">
      <a class="back-btn" href="/music-demo">&larr; Back</a><h1>Now Playing</h1>
...
@app.route("/shows-demo")
def shows_demo_v2():
    return media_player_shell("Shows Demo", """
    <div class="media-hero">
      <h1>Shows</h1>
      <p>Tap a show to start watching</p>
    </div>

    <div class="media-grid">
      <div class="media-card" onclick="location.href='/watch-show/1'">
        <h2>Heirs Launch Show</h2>
        <p>Family welcome show</p>
      </div>

      <div class="media-card" onclick="location.href='/watch-show/2'">
        <h2>Holo Stream Spotlight</h2>
        <p>Creator spotlight</p>
      </div>

      <div class="media-card" onclick="location.href='/watch-show/3'">
        <h2>Bloom Wellness</h2>
        <p>Wellness sessions</p>
      </div>

      <div class="media-card" onclick="location.href='/watch-show/4'">
        <h2>Big Al Records</h2>
        <p>Music sessions</p>
      </div>
    </div>
    """)
...
@app.route("/music-demo")
def music_demo_v2():
    return media_player_shell("Music Demo", """
    <div class="media-hero">
      <h1>Music</h1>
      <p>Tap a track to play</p>
    </div>

    <div class="media-grid">
      <div class="media-card" onclick="location.href='/play-track/1'">
        <h2>Rise In Sound</h2>
        <p>Aniyah</p>
      </div>

      <div class="media-card" onclick="location.href='/play-track/2'">
        <h2>Platform Anthem</h2>
        <p>AAME</p>
      </div>

      <div class="media-card" onclick="location.href='/play-track/3'">
        <h2>Lifted Voices</h2>
        <p>Faith</p>
      </div>

      <div class="media-card" onclick="location.href='/play-track/4'">
        <h2>Night Drive Mix</h2>
        <p>Kevon</p>
      </div>
    </div>
    """)
    ...
    <body>
      <div class="shell">
        <div class="hero">
          ...
          <p>Birthday celebration pages for the family app.</p>
        </div>

        <div class="bday-banner">
          <h2>🎉 Happy Birthday Aniyah 🎉</h2>
          <p>Your birthday spotlight is live in the family app.</p>
          <div class="cta-row">
            <a class="btn" href="/happy-birthday-aniyah">Open Aniyah Birthday Page</a>
            <a class="btn-secondary" href="/birthday-nft-gallery">Open Birthday NFT Gallery</a>
            <a class="btn-secondary" href="/birthday-nft-gift-center">Open NFT Gift Center</a>
          </div>
        </div>

        <div class="bday-grid">
          <div class="bday-card">
            <h2>Alton Kevon Countdown</h2>
            <p>Only 3 days left until Alton Kevon’s birthday spotlight.</p>
            <a class="btn" href="/countdown-alton-kevon">Open Countdown</a>
          </div>
        </div>
      </div>
    </body>
...
def happy_birthday_aniyah():
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Happy Birthday Aniyah</title>
      <link rel="stylesheet" href="/static/css/fivepm_lock.css">
      <link rel="stylesheet" href="/static/css/birthday.css">
    </head>
    <body>
      <div class="shell">
        <div class="bday-banner">
          <h1>🎂 Happy Birthday Aniyah 🎂</h1>
          <p>Welcome to your special birthday page inside the family app.</p>
          <div class="cta-row">
            <a class="btn" href="/birthday-nft-gallery">See Birthday NFT Concept</a>
            <a class="btn-secondary" href="/family-launch-home">Back to Family Home</a>
          </div>
        </div>
      </div>
    </body>
...
def countdown_alton_kevon():
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Alton Kevon Countdown</title>
      <link rel="stylesheet" href="/static/css/fivepm_lock.css">
      <link rel="stylesheet" href="/static/css/birthday.css">
    </head>
    <body>
      <div class="shell">
        <div class="bday-banner">
          <h1>🎉 3 Days Until Alton Kevon’s Birthday 🎉</h1>
          <p>The family app is counting down to his birthday spotlight.</p>
          <a class="btn-secondary" href="/family-launch-home">Back to Family Home</a>
        </div>
      </div>
    </body>
...
def birthday_nft_gallery():
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Birthday NFT Gallery</title>
      <link rel="stylesheet" href="/static/css/fivepm_lock.css">
      <link rel="stylesheet" href="/static/css/birthday.css">
    </head>
    <body>
      <div class="shell">
        <div class="hero">
          <h1>Birthday NFT Gallery</h1>
          <p>Concept gallery for family birthday keepsake NFTs.</p>
        </div>
        <div class="bday-grid">
          <div class="bday-card">
            <h2>Happy Birthday Aniyah NFT</h2>
            <p>Edition 1/1 family keepsake concept.</p>
          </div>
          <div class="bday-card">
            <h2>Happy Birthday Alton Kevon NFT</h2>
            <p>Edition 1/1 family keepsake concept.</p>
          </div>
        </div>
      </div>
...
@app.route("/api/birthday-nft-catalog-v1")
def api_birthday_nft_catalog_v1():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM birthday_nft_catalog_v1 ORDER BY id").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/birthday-nft-gifts-v1")
def api_birthday_nft_gifts_v1():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM birthday_nft_gifts_v1 ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/birthday-nft-claims-v1")
def api_birthday_nft_claims_v1():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM birthday_nft_claims_v1 ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/create-birthday-nft-gift-v1", methods=["POST"])
def api_create_birthday_nft_gift_v1():
    from flask import request
    import sqlite3, random, string
    payload = request.get_json(silent=True) or request.form
    nft_title = (payload.get("nft_title") or "").strip()
    honoree_name = (payload.get("honoree_name") or "").strip()
    gifted_by = (payload.get("gifted_by") or "").strip()
    gifted_to_name = (payload.get("gifted_to_name") or "").strip()
    gifted_to_email = (payload.get("gifted_to_email") or "").strip()
    notes = (payload.get("notes") or "").strip()
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO birthday_nft_gifts_v1 (nft_title, honoree_name, gifted_by, gifted_to_name, gifted_to_email, claim_code, claim_status, notes) VALUES (?, ?, ?, ?, ?, ?, 'gifted', ?)",
        (nft_title, honoree_name, gifted_by, gifted_to_name, gifted_to_email, claim_code, notes)
    )
    conn.commit()
    conn.close()
    return jsonify({"ok": True, "claim_code": claim_code})

@app.route("/api/claim-birthday-nft-v1", methods=["POST"])
def api_claim_birthday_nft_v1():
    from flask import request
    import sqlite3
    payload = request.get_json(silent=True) or request.form
    claim_code = (payload.get("claim_code") or "").strip()
    claimer_name = (payload.get("claimer_name") or "").strip()
    claimer_email = (payload.get("claimer_email") or "").strip()
    notes = (payload.get("notes") or "").strip()

    conn = sqlite3.connect("instance/app.db")
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO birthday_nft_claims_v1 (claim_code, claimer_name, claimer_email, status, notes) VALUES (?, ?, ?, 'claimed', ?)",
        (claim_code, claimer_name, claimer_email, notes)
    )
    cur.execute(
        "UPDATE birthday_nft_gifts_v1 SET claim_status='claimed' WHERE claim_code=?",
        (claim_code,)
    )
    conn.commit()
    conn.close()
...
def birthday_nft_gift_center():
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Birthday NFT Gift Center</title>
      <link rel="stylesheet" href="/static/css/fivepm_lock.css">
      <link rel="stylesheet" href="/static/css/nft_gift.css">
    </head>
    <body>
      <div class="shell">
        <div class="nft-hero">
          <h1>Birthday NFT Gift Center</h1>
          <p>Create a giftable birthday NFT claim pass that Aniyah can give to a friend when they sign up.</p>
        </div>
        ...
        <div class="nft-grid">
          <div class="nft-card">
            ...
            <form id="giftForm">
              ...
              <input name="honoree_name" value="Aniyah">
              <input name="gifted_by" value="Aniyah">
              <input name="gifted_to_name" placeholder="Friend Name">
              <input name="gifted_to_email" placeholder="Friend Email">
              <textarea name="notes">Birthday NFT gift pass</textarea>
              <button type="button" onclick="createGift()">Create Gift Pass</button>
            </form>
            <div id="giftResult" class="code-box" style="display:none"></div>
          </div>
          ...
          <div class="nft-card">
            ...
            <form id="claimForm">
              ...
              <input name="claimer_name" placeholder="Your Name">
              <input name="claimer_email" placeholder="Your Email">
              <textarea name="notes">Claiming birthday NFT pass</textarea>
              <button type="button" onclick="claimGift()">Claim NFT Pass</button>
            </form>
          </div>

          <div class="nft-card">
            <h2>What This Does</h2>
            <p>This creates a giftable birthday collectible that can help friends join the app and feel part of something special.</p>
            <a class="btn-secondary" href="/api/birthday-nft-gifts-v1">Gift Records</a>
            <a class="btn-secondary" href="/api/birthday-nft-claims-v1">Claim Records</a>
          </div>
        </div>
      </div>
      ...
      async function createGift() {
        const fd = new FormData(document.getElementById('giftForm'));
        const res = await fetch('/api/create-birthday-nft-gift-v1', {method:'POST', body:fd});
        const data = await res.json();
        const box = document.getElementById('giftResult');
        if (data.ok) {
          box.style.display = 'block';
          box.innerHTML = 'Gift Pass Code: ' + data.claim_code;
        } else {
          alert('Failed');
        }
      }
      ...
      async function claimGift() {
        ...
        const res = await fetch('/api/claim-birthday-nft-v1', {method:'POST', body:fd});
        const data = await res.json();
        alert(data.ok ? ('Claimed: ' + data.claim_code) : 'Failed');
      }
      </script>
    </body>
    </html>
...
def family_finish_shell(title, body_html):
    return f"""
    <!doctype html>
    <html>
    <head>
      ...
      <title>{title}</title>
      <link rel="stylesheet" href="/static/css/fivepm_lock.css">
      <link rel="stylesheet" href="/static/css/launch_finish_family.css">
    </head>
    <body>
...
@app.route("/welcome-aniyah")
def welcome_aniyah():
    body = """
    <div class="launch-hero">
      <h1>Happy Birthday Aniyah 🎉</h1>
      <p>Welcome to your private family app. Your birthday center, music, shows, and NFT gift pages are ready.</p>
      <p><a href="/family-launch-home">Back to Family Home</a></p>
    </div>
    <div class="launch-grid">
      <div class="launch-card"><h2>Birthday Center</h2><a href="/birthday-center">Open</a></div>
      <div class="launch-card"><h2>NFT Gift Center</h2><a href="/birthday-nft-gift-center">Open</a></div>
      <div class="launch-card"><h2>Shows</h2><a href="/shows-demo">Open</a></div>
      <div class="launch-card"><h2>Music</h2><a href="/music-demo">Open</a></div>
    </div>
    """
    return family_finish_shell("Welcome Aniyah", body)

@app.route("/welcome-alton-kevon")
def welcome_alton_kevon():
    body = """
    <div class="launch-hero">
      <h1>Welcome Alton Kevon ⭐</h1>
      <p>Your family-first access is ready. Your countdown page, music, shows, and artist tools are available.</p>
      <p><a href="/family-launch-home">Back to Family Home</a></p>
    </div>
    <div class="launch-grid">
      <div class="launch-card"><h2>Birthday Countdown</h2><a href="/countdown-alton-kevon">Open</a></div>
      <div class="launch-card"><h2>Artist Signing</h2><a href="/artist-signing-home">Open</a></div>
      <div class="launch-card"><h2>Shows</h2><a href="/shows-demo">Open</a></div>
      <div class="launch-card"><h2>Music</h2><a href="/music-demo">Open</a></div>
    </div>
    """
    return family_finish_shell("Welcome Alton Kevon", body)

@app.route("/share-family-launch")
def share_family_launch():
    host = "http://192.168.1.115:8080"
    body = f"""
    <div class="launch-hero">
      <h1>Share Family Launch</h1>
      <p>Use these same-Wi-Fi links to share the app with your kids first.</p>
    </div>
    <div class="launch-grid">
      <div class="launch-card">
        <h2>Main Home</h2>
        <div class="link-box">{host}/family-launch-home</div>
      </div>
      <div class="launch-card">
        <h2>Aniyah</h2>
        <div class="link-box">{host}/welcome-aniyah</div>
      </div>
      <div class="launch-card">
        <h2>Alton Kevon</h2>
        <div class="link-box">{host}/welcome-alton-kevon</div>
      </div>
      <div class="launch-card">
        <h2>Birthday Center</h2>
        <div class="link-box">{host}/birthday-center</div>
      </div>
    </div>
    """
    return family_finish_shell("Share Family Launch", body)
...
def launch_checklist():
    ...
    <div class="launch-hero">
      ...
      <p>Final pages for today’s family beta are ready.</p>
    </div>
    <div class="launch-grid">
      <div class="launch-card"><h2>Family Home</h2><a href="/family-launch-home">Open</a></div>
      <div class="launch-card"><h2>Aniyah Welcome</h2><a href="/welcome-aniyah">Open</a></div>
      <div class="launch-card"><h2>Alton Kevon Welcome</h2><a href="/welcome-alton-kevon">Open</a></div>
      <div class="launch-card"><h2>Birthday Center</h2><a href="/birthday-center">Open</a></div>
      <div class="launch-card"><h2>Shows</h2><a href="/shows-demo">Open</a></div>
      <div class="launch-card"><h2>Music</h2><a href="/music-demo">Open</a></div>
      <div class="launch-card"><h2>Coaching</h2><a href="/music-coaching-home">Open</a></div>
      <div class="launch-card"><h2>Share Links</h2><a href="/share-family-launch">Open</a></div>
    </div>
    """
    return family_finish_shell("Launch Checklist", body)


@app.route("/welcome-jacobie")
def welcome_jacobie():
    body = """
    <div class="launch-hero">
      <h1>Welcome Jacobie ⭐</h1>
      <p>Your family app page is ready. This page can grow into your personal business and legacy dashboard.</p>
      <p><a href="/family-launch-home">Back to Family Home</a></p>
    </div>
    <div class="launch-grid">
      <div class="launch-card"><h2>Business Lane</h2><p>Jacobie Vision / future business lane.</p><a href="/creator-tools-home">Open</a></div>
      <div class="launch-card"><h2>Shows</h2><a href="/shows-demo">Open</a></div>
      <div class="launch-card"><h2>Music</h2><a href="/music-demo">Open</a></div>
    </div>
    """
    return family_finish_shell("Welcome Jacobie", body)

@app.route("/welcome-isaiah")
def welcome_isaiah():
    body = """
    <div class="launch-hero">
      <h1>Welcome Isaiah ⭐</h1>
      <p>Your family app page is ready. This can become your personal growth, business, and creator lane.</p>
      <p><a href="/family-launch-home">Back to Family Home</a></p>
    </div>
    <div class="launch-grid">
      <div class="launch-card"><h2>Business Lane</h2><p>Isaiah business and future creator lane.</p><a href="/creator-tools-home">Open</a></div>
      <div class="launch-card"><h2>Shows</h2><a href="/shows-demo">Open</a></div>
      <div class="launch-card"><h2>Music</h2><a href="/music-demo">Open</a></div>
    </div>
    """
    return family_finish_shell("Welcome Isaiah", body)

@app.route("/welcome-brother-alton")
def welcome_brother_alton():
    body = """
    <div class="launch-hero">
      <h1>Welcome Alton 🛡</h1>
      <p>Your page is ready with your business lanes connected into the family platform.</p>
      <p><a href="/family-launch-home">Back to Family Home</a></p>
    </div>
    <div class="launch-grid">
      <div class="launch-card"><h2>Security + Compliance</h2><p>Business lane for security and compliance growth.</p><a href="/armed-forces-center">Open</a></div>
      <div class="launch-card"><h2>Media / Records</h2><p>Big Al Records and media lane.</p><a href="/artist-signing-home">Open</a></div>
      <div class="launch-card"><h2>Commerce</h2><p>Family commerce and launch lane.</p><a href="/holo-commerce-home">Open</a></div>
    </div>
    """
    return family_finish_shell("Welcome Brother Alton", body)

@app.route("/share-family-links")
def share_family_links():
    host = "http://192.168.1.115:8080"
    body = f"""
    <div class="launch-hero">
      <h1>Family Share Links</h1>
      <p>Send these same-Wi-Fi links to each person directly.</p>
    </div>
    <div class="launch-grid">
      <div class="launch-card"><h2>Aniyah</h2><div class="link-box">{host}/welcome-aniyah</div></div>
      <div class="launch-card"><h2>Alton Kevon</h2><div class="link-box">{host}/welcome-alton-kevon</div></div>
      <div class="launch-card"><h2>Jacobie</h2><div class="link-box">{host}/welcome-jacobie</div></div>
      <div class="launch-card"><h2>Isaiah</h2><div class="link-box">{host}/welcome-isaiah</div></div>
      <div class="launch-card"><h2>Brother Alton</h2><div class="link-box">{host}/welcome-brother-alton</div></div>
      <div class="launch-card"><h2>Main Home</h2><div class="link-box">{host}/family-launch-home</div></div>
    </div>
    """
    return family_finish_shell("Family Share Links", body)
