...
echo "=== MASTER SMOKE + STABILIZE + FREEZE ==="
...
curl -s -X POST http://127.0.0.1:4900/join-api \
  ...
  -d '{"username":"master_freeze_user","referrer":"all_american_creator"}'
echo

echo
echo "[4] PUBLIC BETA CHECK"
...
cat > snapshots/final/master_status_${STAMP}.txt <<EOF
checkpoint=$STAMP
