...
echo "=== SETTLEMENT RECONCILE + ENTITLEMENTS BUILD START ==="
...
conn.commit()
conn.close()
print("[OK] entitlement DB additions ready")
PYEOF

########################################
# 2) RECONCILE SEEDED/MISSING SETTLEMENTS
########################################
python << 'PYEOF'
import sqlite3
from pathlib import Path
...
WHERE (receipt_id IS NULL OR wallet_tx_id IS NULL)
ORDER BY id ASC
""").fetchall()
...
p.write_text(text)
print("[OK] world_socket.js entitlement patch applied")
PYEOF
...
p.write_text(text)
print("[OK] dashboard.js access patch applied")
PYEOF
...
cp apps/world_socket.js "backups/world_socket_entitlements_${STAMP}.js"
cp apps/dashboard.js "backups/dashboard_entitlements_${STAMP}.js"
cp db/aam.db "backups/aam_entitlements_${STAMP}.db"
...
echo "SETTLEMENT RECONCILE + ENTITLEMENTS CHECKPOINT COMPLETE: $STAMP"
echo "Test:"
