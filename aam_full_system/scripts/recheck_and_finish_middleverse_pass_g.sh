...
cd ~/aam_full_system
echo "=== RECHECK + FINISH MIDDLEVERSE PASS G START ==="

STAMP="$(date +%Y%m%d_%H%M%S)"
mkdir -p backups snapshots reports test_results

cp apps/dashboard.js "backups/dashboard_recheck_finish_pass_g_${STAMP}.js"
cp db/aam.db "backups/aam_recheck_finish_pass_g_${STAMP}.db"

########################################
# 1) VERIFY PASS G TABLES EXIST
########################################
python3 << 'PYEOF'
import sqlite3, sys
...
print(f"[OK] middleverse pass G scan complete: {len(issues)} issues")
PYEOF
...
# 8) REPORT
########################################
cat > "reports/recheck_and_finish_middleverse_pass_g_${STAMP}.txt" <<REPORT
RECHECK + FINISH MIDDLEVERSE PASS G REPORT
Timestamp: ${STAMP}
...
- middleverse_dependency_safety_registry
- middleverse_recovery_checkpoint_registry
- safe flag/policy/config/incident/channel/dependency/checkpoint actions
- dashboard health
- jarvis health
...
Purpose:
- finish middleverse pass G cleanly
- write the missing scan and report
- preserve stable runtime
REPORT

echo "RECHECK + FINISH MIDDLEVERSE PASS G COMPLETE: $STAMP"
echo "Check:"
...
echo "  cat reports/recheck_and_finish_middleverse_pass_g_${STAMP}.txt"
echo "Open:"
