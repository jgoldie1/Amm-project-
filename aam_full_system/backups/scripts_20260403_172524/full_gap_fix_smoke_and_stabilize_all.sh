...
cd ~/aam_full_system
echo "=== FULL GAP FIX + SMOKE + STABILIZE ALL START ==="
...
# 1) BACKUPS
########################################
cp apps/dashboard.js "backups/dashboard_full_gap_fix_${STAMP}.js"
cp apps/jarvis.js "backups/jarvis_full_gap_fix_${STAMP}.js"
cp db/aam.db "backups/aam_full_gap_fix_${STAMP}.db"

########################################
# 2) ENSURE CORE AUDIT TABLES
########################################
python3 << 'PYEOF'
import sqlite3
...
cur.execute("""
CREATE TABLE IF NOT EXISTS zero_issue_runtime_registry (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
# 6) SAFE ACTION SMOKE
########################################
curl -s -i -X POST http://127.0.0.1:4900/dispatch-actions/request-create-smoke > "test_results/gap_request_${STAMP}.txt" || true
curl -s -i -X POST http://127.0.0.1:4900/dispatch-actions/handoff-smoke > "test_results/gap_handoff_${STAMP}.txt" || true
curl -s -i -X POST http://127.0.0.1:4900/dispatch-actions/assign-safe > "test_results/gap_assign_${STAMP}.txt" || true
curl -s -i -X POST http://127.0.0.1:4900/dispatch-actions/status-safe > "test_results/gap_status_${STAMP}.txt" || true
curl -s -i -X POST http://127.0.0.1:4900/dispatch-actions/escalate-safe > "test_results/gap_escalate_${STAMP}.txt" || true
curl -s -i -X POST http://127.0.0.1:4900/dispatch-actions/resolve-safe > "test_results/gap_resolve_${STAMP}.txt" || true
curl -s -i -X POST http://127.0.0.1:4900/studio/create-scene-safe > "test_results/gap_scene_${STAMP}.txt" || true
curl -s -i -X POST http://127.0.0.1:4900/studio/create-content-safe > "test_results/gap_content_${STAMP}.txt" || true
curl -s -i -X POST http://127.0.0.1:4900/studio/create-voice-safe > "test_results/gap_voice_${STAMP}.txt" || true

########################################
# 7) GAP AUDIT + AUTO-REFRESH REGISTRIES
########################################
python3 << PYEOF
import sqlite3, json
from pathlib import Path
...
print("[OK] full gap audit written")
...
print(f"[OK] full gap fix scan complete: {len(issues)} issues")
PYEOF
...
# 10) REPORT
########################################
cat > "reports/full_gap_fix_smoke_and_stabilize_all_${STAMP}.txt" <<REPORT
FULL GAP FIX + SMOKE + STABILIZE ALL REPORT
Timestamp: ${STAMP}
...
- route_registry
- zero_issue_runtime_registry
- full gap summary snapshot

Purpose:
- check for current stack gaps
- confirm runtime stability
- preserve a clean all-stack checkpoint
- prepare for the next build section
REPORT

echo "FULL GAP FIX + SMOKE + STABILIZE ALL COMPLETE: $STAMP"
echo "Check:"
...
echo "  cat reports/full_gap_fix_smoke_and_stabilize_all_${STAMP}.txt"
echo "Open:"
