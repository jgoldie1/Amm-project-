...
@app.route("/api/university-courses")
def api_university_courses():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM university_courses ORDER BY id").fetchall()
    conn.close()
...
def api_wallet_summary():
    ...
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM wallet_accounts ORDER BY id").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
def api_music_tracks():
    ...
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM music_tracks ORDER BY id").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/stream-channels")
def api_stream_channels():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM stream_channels ORDER BY id").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
