...
def app_home():
    ...
    <div class="hero">
      <h1>All American Marketplace</h1>
      <p>A flagship platform for media, AI, finance, commerce, Holoverse experiences, creator tools, and advanced ecosystem control.</p>
      <div class="badges">
        <span class="badge">Flagship App</span>
        <span class="badge">Search + Media + Finance</span>
        <span class="badge">Holoverse Ready</span>
      </div>
      <div class="brandline">
        <img src="/static/img/stubbs_crest.svg" alt="Stubbs Crest">
        <img src="/static/img/holographic_lion_saturn.svg" alt="Holographic Lion Saturn">
        <img src="/static/img/american_flag_holo.svg" alt="American Flag">
      </div>
      <div class="cta-row">
        <a class="btn" href="/holoverse-center">Enter Holoverse</a>
        <a class="btn-secondary" href="/search-ultra">Open Search</a>
      </div>
    </div>

    <div class="section-title">Core surfaces</div>
    <div class="grid">
      <div class="card"><h2>Control Tower</h2><p>Main command surface for the ecosystem.</p></div>
      <div class="card"><h2>Creator Market</h2><p>Uploads, previews, creator assets, and media direction.</p></div>
      <div class="card"><h2>FinBank</h2><p>International banking shell and finance controls.</p></div>
      <div class="card"><h2>AI Suite</h2><p>HoloGPT, AI orchestration, search intelligence, and workflow direction.</p></div>
    </div>

    <p class="footer-note">Minimal flagship surface active.</p>
    """
    return minimal_shell("AAME App Home", body)
...
def control_tower():
    ...
    <div class="grid">
      ...
      <div class="card"><h2>University</h2><p>Training, certification, onboarding, and workforce building.</p><a class="btn" href="/all-american-marketplace-university">Open</a></div>
      <div class="card"><h2>Wallet</h2><p>Balances, rewards, payouts, and financial rails.</p><a class="btn" href="/all-american-marketplace-wallet">Open</a></div>
      <div class="card"><h2>Holoverse</h2><p>Immersive gateway for holographic systems.</p><a class="btn" href="/holoverse-center">Open</a></div>
      <div class="card"><h2>AI Suite</h2><p>Flagship AI orchestration and guidance layer.</p><a class="btn" href="/flagship-ai-suite">Open</a></div>
      <div class="card"><h2>Media</h2><p>Music, streaming, uploads, and creator assets.</p><a class="btn" href="/media-studio">Open</a></div>
      <div class="card"><h2>FinBank</h2><p>International bank shell and finance controls.</p><a class="btn" href="/finbank-international">Open</a></div>
      <div class="card"><h2>Search</h2><p>Cross-module flagship discovery layer.</p><a class="btn" href="/search-ui">Open</a></div>
      <div class="card"><h2>Gap Fix</h2><p>Missing items, risks, and detrimental gaps still left.</p><a class="btn" href="/gap-fix-center">Open</a></div>
    </div>
    """ + flagship_dock()
    return app_shell("Control Tower", body)
...
@app.route("/api/upload-assets")
def api_upload_assets():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM upload_assets ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/finance-controls")
def api_finance_controls():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM finance_controls ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/workflow-actions")
def api_workflow_actions():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM workflow_actions ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/finbank-international")
def finbank_international():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM finance_controls ORDER BY id DESC").fetchall()
    ...
    for r in rows:
        ...
        <div class="card">
          <div class="pill">{r['status'].title()}</div>
          <h2>{r['control_name']}</h2>
          <p><strong>Scope:</strong> {r['control_scope']}</p>
        </div>
        """)
    cards_html = "".join(cards) if cards else "<div class='card'><h2>No Finance Controls Yet</h2></div>"
    return f"""
    <!doctype html>
    <html>
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>El Saturn FinBank International</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>El Saturn FinBank International</h1>
          <p>International banking and finance control surface for wallet rails, cross-border routing, account governance, and future institution expansion.</p>
          <div class="brandbar">
            <img src="/static/img/stubbs_crest.svg" alt="Stubbs Crest">
            <img src="/static/img/holographic_lion_saturn.svg" alt="Holographic Lion Saturn">
            <img src="/static/img/american_flag_holo.svg" alt="American Flag">
        ...
        <div class="grid">{cards_html}</div>
        <p style="margin-top:24px;">API: <a class="link" href="/api/finance-controls">/api/finance-controls</a></p>
        <p style="margin-top:24px;"><a class="link" href="/all-american-marketplace-wallet">← Back to Wallet</a></p>
      </div>
    </body>
    </html>
    """
...
def uploads_admin():
    ...
    for r in rows:
        ...
        <div class="card">
          <div class="pill">{r['status'].title()}</div>
          <h2>{r['file_name']}</h2>
          <p><strong>Type:</strong> {r['asset_kind']}<br><strong>Owner:</strong> {r['owner_name']}</p>
        </div>
        """)
    cards_html = "".join(cards) if cards else "<div class='card'><h2>No Upload Assets Yet</h2></div>"
    return f"""
    <!doctype html>
    <html>
    <head>
    ...
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Uploads Admin</h1>
          <p>Media and brand asset control panel for current and future uploads.</p>
          <div class="brandbar">
            <img src="/static/img/stubbs_crest.svg" alt="Stubbs Crest">
            <img src="/static/img/holographic_lion_saturn.svg" alt="Holographic Lion Saturn">
            <img src="/static/img/american_flag_holo.svg" alt="American Flag">
        ...
        <div class="grid">{cards_html}</div>
        <p style="margin-top:24px;">API: <a class="link" href="/api/upload-assets">/api/upload-assets</a></p>
        <p style="margin-top:24px;"><a class="link" href="/media-studio">← Back to Media Studio</a></p>
      </div>
    </body>
    </html>
    """
...
def search_app():
    ...
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Flagship Search App</h1>
          <p>Next-level search surface across marketplace, university, wallet, music, streaming, vocal, cross-border payment, media assets, workflows, and module registry.</p>
        </div>
        <div class="card">
          <h2>Search Examples</h2>
          <p><a class="link" href="/api/search?q=music">/api/search?q=music</a></p>
          <p><a class="link" href="/api/search?q=wallet">/api/search?q=wallet</a></p>
          <p><a class="link" href="/api/search?q=aniyah">/api/search?q=aniyah</a></p>
          <p><a class="link" href="/api/search?q=kingdom">/api/search?q=kingdom</a></p>
        </div>
        <p style="margin-top:24px;"><a class="link" href="/next-gen-search-lab">← Back to Search Lab</a></p>
      </div>
    </body>
    </html>
    """

@app.route("/auth-security-center")
def auth_security_center():
    return """
    <!doctype html>
    <html>
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Auth Security Center</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Auth Security Center</h1>
          <p>Security control surface for role enforcement, sensitive-route protection, and future permission hardening.</p>
        </div>
        <div class="grid">
          <div class="card"><h2>Role Profiles</h2><p>Current live role map for admin, operator, creator, vendor, student, and customer.</p><a class="btn" href="/auth-role-center">Open Roles</a></div>
          <div class="card"><h2>Audit History</h2><p>Review system events before sensitive actions are enabled.</p><a class="btn" href="/audit-history">Open Audit</a></div>
          <div class="card"><h2>Next Step</h2><p>Enforce actual route-level protection on finance, upload, and admin pages.</p></div>
        </div>
        <p style="margin-top:24px;"><a class="link" href="/gap-fix-center">← Back to Gap Fix Center</a></p>
      </div>
    </body>
    </html>
    """

@app.route("/finance-hardening-center")
def finance_hardening_center():
    return """
    <!doctype html>
    <html>
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Finance Hardening Center</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Finance Hardening Center</h1>
          <p>Sandbox-first finance readiness center for wallet, cross-border, payouts, and FinBank controls.</p>
        </div>
        <div class="grid">
          <div class="card"><h2>Wallet</h2><p>Live wallet dashboard with balances and rewards.</p><a class="btn" href="/all-american-marketplace-wallet">Open Wallet</a></div>
          <div class="card"><h2>Cross-Border</h2><p>Live payment dashboard with seeded transfer records.</p><a class="btn" href="/aniyah-cross-border-payment">Open Cross-Border</a></div>
          <div class="card"><h2>FinBank International</h2><p>International bank control surface and finance controls.</p><a class="btn" href="/finbank-international">Open FinBank</a></div>
        </div>
        <p style="margin-top:24px;"><a class="link" href="/risk-checklist">← Back to Risk Checklist</a></p>
      </div>
    </body>
    </html>
    """

@app.route("/action-workflow-center")
def action_workflow_center():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM workflow_actions ORDER BY id DESC").fetchall()
    ...
    for r in rows:
        cards.append(f"""
        <div class="card">
          <div class="pill">{r['action_status'].title()}</div>
          <h2>{r['action_name']}</h2>
          <p><strong>Module:</strong> {r['action_module']}</p>
        </div>
        """)
    cards_html = "".join(cards) if cards else "<div class='card'><h2>No Workflow Actions Yet</h2></div>"
    return f"""
    <!doctype html>
    <html>
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Action Workflow Center</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Action Workflow Center</h1>
          <p>Workflow action dashboard for guided platform actions and future AI-triggered operations.</p>
        </div>
        <div class="grid">{cards_html}</div>
        <p style="margin-top:24px;">API: <a class="link" href="/api/workflow-actions">/api/workflow-actions</a></p>
        <p style="margin-top:24px;"><a class="link" href="/workflow-lab">← Back to Workflow Lab</a></p>
      </div>
    </body>
    </html>
    """
...
@app.route("/api/security-units")
def api_security_units():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM security_units ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/media-projects")
def api_media_projects():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM media_projects ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/api/trading-watchlist")
def api_trading_watchlist():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM trading_watchlist ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])
...
def alton_security():
    ...
    for r in rows:
        ...
        <div class="card">
          <div class="pill">{r['status'].title()}</div>
          <h2>{r['unit_name']}</h2>
          <p><strong>Type:</strong> {r['unit_type']}</p>
          <p>{r['notes']}</p>
        </div>
        """)
    cards_html = "".join(cards) if cards else "<div class='card'><h2>No Security Units Yet</h2></div>"
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Alton Security</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Alton Security</h1>
          <p>Security operations, compliance, access control planning, incident review, and route protection center.</p>
        </div>
        <div class="grid">{cards_html}</div>
        <p style="margin-top:24px;">API: <a class="link" href="/api/security-units">/api/security-units</a></p>
        <p style="margin-top:24px;"><a class="link" href="/auth-security-center">← Back to Auth Security Center</a></p>
      </div>
    </body>
    </html>
    """

@app.route("/security-compliance-center")
def security_compliance_center():
    return """
    <!doctype html>
    <html>
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Security Compliance Center</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          ...
          <p>Compliance shell for licensing, policy, access control, training status, and audit readiness.</p>
        </div>
        <div class="grid">
          <div class="card"><h2>Licensing & Policy</h2><p>Track compliance records and policy readiness.</p></div>
          <div class="card"><h2>Training Status</h2><p>Store safe training, certification, and review records.</p></div>
          <div class="card"><h2>Audit Readiness</h2><p>Connect security oversight to audit history and incident review.</p></div>
          <div class="card"><h2>Restricted Access</h2><p>Future role-based protection for sensitive pages and controls.</p></div>
        </div>
        <p style="margin-top:24px;"><a class="link" href="/alton-security">← Back to Alton Security</a></p>
      </div>
    </body>
    </html>
    """

@app.route("/alton-kevon-ops")
def alton_kevon_ops():
    return """
    <!doctype html>
    <html>
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Alton Kevon Ops</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Alton Kevon Ops</h1>
          <p>Operations shell for security, media execution, family-brand expansion, and project coordination.</p>
        </div>
        <div class="grid">
          <div class="card"><h2>Security Ops</h2><p>Connect to Alton Security and compliance layers.</p><a class="btn" href="/alton-security">Open</a></div>
          <div class="card"><h2>Media Ops</h2><p>Connect to Kevon Shot It projects and creator assets.</p><a class="btn" href="/kevon-shotit-media">Open</a></div>
          <div class="card"><h2>Workflow Ops</h2><p>Future operational routing and task execution center.</p><a class="btn" href="/action-workflow-center">Open</a></div>
        </div>
        <p style="margin-top:24px;"><a class="link" href="/control-tower">← Back to Control Tower</a></p>
      </div>
    </body>
    </html>
    """

@app.route("/kevon-shotit-media")
def kevon_shotit_media():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM media_projects ORDER BY id DESC").fetchall()
    ...
    for r in rows:
        ...
        <div class="card">
          ...
          <p><strong>Owner:</strong> {r['owner_name']}<br>
          <strong>Type:</strong> {r['project_type']}</p>
        </div>
        """)
    cards_html = "".join(cards) if cards else "<div class='card'><h2>No Media Projects Yet</h2></div>"
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Kevon Shot It Media</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Kevon Shot It Media</h1>
          <p>Live media project dashboard for Alton Kevon visual production, campaigns, and creator visuals.</p>
        </div>
        <div class="grid">{cards_html}</div>
        <p style="margin-top:24px;">API: <a class="link" href="/api/media-projects">/api/media-projects</a></p>
        <p style="margin-top:24px;"><a class="link" href="/alton-kevon-ops">← Back to Alton Kevon Ops</a></p>
      </div>
    </body>
    </html>
    """

@app.route("/forex-stock-trading-lab")
def forex_stock_trading_lab():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM trading_watchlist ORDER BY id DESC").fetchall()
    ...
    for r in rows:
        ...
        <div class="card">
          <div class="pill">{r['status'].title()}</div>
          <h2>{r['symbol']}</h2>
          <p><strong>Market:</strong> {r['market_type']}<br>
          <strong>Strategy:</strong> {r['strategy_label']}</p>
        </div>
        """)
    cards_html = "".join(cards) if cards else "<div class='card'><h2>No Watchlist Items Yet</h2></div>"
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width,initial-scale=1">
      <title>Forex & Stock Trading Lab</title>
      <link rel="stylesheet" href="/static/css/flagship.css">
    </head>
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Forex & Stock Trading Lab</h1>
          <p>Sandbox watchlist and market research shell for forex, stocks, ETFs, and future analytics workflows.</p>
        </div>
        <div class="grid">{cards_html}</div>
        <p style="margin-top:24px;">API: <a class="link" href="/api/trading-watchlist">/api/trading-watchlist</a></p>
        <p style="margin-top:24px;"><a class="link" href="/flagship-ai-suite">← Back to Flagship AI Suite</a></p>
      </div>
    </body>
    </html>
    """
...
@app.route("/api/security-training-records")
def api_security_training_records():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM security_training_records ORDER BY id DESC").fetchall()
    conn.close()
    return jsonify([dict(r) for r in rows])

@app.route("/security-training-records")
def security_training_records():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT * FROM security_training_records ORDER BY id DESC").fetchall()
    ...
    for r in rows:
        cards.append(f"""
        <div class="card">
          <div class="pill">{r['certification_status'].title()}</div>
          <h2>{r['person_name']}</h2>
          <p><strong>Training:</strong> {r['training_name']}<br>
          <strong>Renewal:</strong> {r['renewal_status']}</p>
        </div>
        """)
    cards_html = "".join(cards) if cards else "<div class='card'><h2>No Training Records Yet</h2></div>"
    ...
    <body>
      <div class="wrap">
        <div class="hero">
          <h1>Security Training Records</h1>
          <p>Compliance-safe dashboard for training completion, certification state, renewals, and audit readiness.</p>
        </div>
        <div class="grid">{cards_html}</div>
        <p style="margin-top:24px;">API: <a class="link" href="/api/security-training-records">/api/security-training-records</a></p>
        <p style="margin-top:24px;"><a class="link" href="/security-compliance-center">← Back to Security Compliance Center</a></p>
      </div>
    </body>
    </html>
    """
...
def flagship_nav():
    ...
    <div class="topnav">
      <div class="topnav-inner">
        <div style="font-weight:bold;color:#e2e8f0">AAME Flagship</div>
        <div class="nav-links">
          <a href="/app-home">App Home</a>
          <a href="/control-tower">Control Tower</a>
          <a href="/holoverse-center">Holoverse</a>
          <a href="/flagship-showcase">Showcase</a>
          <a href="/search-ui">Search</a>
          <a href="/media-studio">Media</a>
          <a href="/finbank-international">FinBank</a>
          <a href="/flagship-ai-suite">AI Suite</a>
          <a href="/missing-pieces-center">Missing Pieces</a>
        </div>
      </div>
    </div>
    """

def app_shell(title, body_html):
    return f"""
    <!doctype html>
    <html>
    <head>
...
@app.route("/flagship-showcase")
def flagship_showcase():
    body = """
    <div class="hero hero-center">
      <h1>Flagship Showcase</h1>
      <p>This is the visual identity layer for the All American Marketplace Holographic Streaming Ecosystem — built to feel premium, branded, immersive, and original.</p>
      <div class="hero-badges">
        <span class="pill">Flagship App</span>
        <span class="pill">Holoverse Ready</span>
        <span class="pill">AI + Finance + Media</span>
      </div>
      <div class="brandbar">
        <img src="/static/img/stubbs_crest.svg" alt="Stubbs Crest">
        <img src="/static/img/holographic_lion_saturn.svg" alt="Holographic Lion Saturn">
    ...
    <div class="showcase">
      <div class="panel">
        <h2>What you created</h2>
        <p>You created the shell of a multi-domain flagship platform operating system that combines commerce, media, training, finance, AI, blockchain planning, verse navigation, and holographic internet strategy into one app surface.</p>
        <div class="feature-list">
          <div class="feature-item">Marketplace core</div>
          <div class="feature-item">Holoverse gateway</div>
          <div class="feature-item">University + learning</div>
          <div class="feature-item">Wallet + FinBank shell</div>
          <div class="feature-item">Music + streaming</div>
          <div class="feature-item">AI command + search</div>
          <div class="feature-item">Cross-border payments</div>
          <div class="feature-item">Security + compliance</div>
        </div>
        <div class="button-row">
          <a class="btn" href="/app-home">Open App Home</a>
          <a class="btn-secondary" href="/holoverse-center">Open Holoverse</a>
        </div>
      </div>

      <div class="panel">
        <h2>How you know it is working</h2>
        <p>The app is working when the branded pages load, the images appear, the route tests return <strong>200 OK</strong>, and the modules open from one shared navigation surface without breaking startup.</p>
        <div class="feature-list">
          <div class="feature-item">Brand images visible</div>
          <div class="feature-item">Shared nav visible</div>
          <div class="feature-item">Search UI loads</div>
          <div class="feature-item">Holoverse page opens</div>
          <div class="feature-item">Media + FinBank pages open</div>
          <div class="feature-item">No startup crash in logs</div>
        </div>
      </div>
    </div>

    <p class="footer-note">This is the flagship visual layer — not a clone surface.</p>
    """
    return app_shell("Flagship Showcase", body)


def flagship_dock():
    return """
    <div style="position:fixed;left:50%;transform:translateX(-50%);bottom:16px;z-index:40;
                display:flex;gap:10px;flex-wrap:wrap;justify-content:center;
                background:rgba(2,6,23,.88);backdrop-filter:blur(12px);
                border:1px solid rgba(103,232,249,.16);border-radius:999px;
                padding:10px 12px;box-shadow:0 18px 40px rgba(0,0,0,.35)">
      <a href="/app-home" style="color:#dbeafe;text-decoration:none;padding:8px 12px;border-radius:999px;background:rgba(17,24,39,.9)">Home</a>
      <a href="/holoverse-center" style="color:#dbeafe;text-decoration:none;padding:8px 12px;border-radius:999px;background:rgba(17,24,39,.9)">Holoverse</a>
      <a href="/flagship-ai-suite" style="color:#dbeafe;text-decoration:none;padding:8px 12px;border-radius:999px;background:rgba(17,24,39,.9)">AI</a>
      <a href="/search-ui" style="color:#dbeafe;text-decoration:none;padding:8px 12px;border-radius:999px;background:rgba(17,24,39,.9)">Search</a>
      <a href="/finbank-international" style="color:#dbeafe;text-decoration:none;padding:8px 12px;border-radius:999px;background:rgba(17,24,39,.9)">FinBank</a>
    </div>
    """

@app.route("/live-status-board")
def live_status_board():
    body = """
    <div class="hero hero-center">
      <h1>Live Status Board</h1>
      <p>Fast visual board for core flagship systems that are already online in the local app.</p>
      <div class="hero-badges">
        <span class="pill">Recovery Complete</span>
        <span class="pill">Modules Online</span>
        <span class="pill">Flagship Buildout</span>
      </div>
    </div>

    <div class="grid">
      <div class="card"><h2>App Home</h2><p>Main branded entry point for the platform.</p><a class="btn" href="/app-home">Open</a></div>
      <div class="card"><h2>Holoverse</h2><p>Immersive center for holographic internet and worlds.</p><a class="btn" href="/holoverse-center">Open</a></div>
      <div class="card"><h2>Control Tower</h2><p>Main command surface for the ecosystem.</p><a class="btn" href="/control-tower">Open</a></div>
      <div class="card"><h2>University</h2><p>Training and learning system.</p><a class="btn" href="/all-american-marketplace-university">Open</a></div>
      <div class="card"><h2>Wallet</h2><p>Wallet and finance layer.</p><a class="btn" href="/all-american-marketplace-wallet">Open</a></div>
      <div class="card"><h2>FinBank</h2><p>International finance shell.</p><a class="btn" href="/finbank-international">Open</a></div>
      <div class="card"><h2>Media Studio</h2><p>Streaming, music, uploads, and media records.</p><a class="btn" href="/media-studio">Open</a></div>
      <div class="card"><h2>AI Suite</h2><p>Flagship AI layer and search intelligence.</p><a class="btn" href="/flagship-ai-suite">Open</a></div>
    </div>
    """
    return app_shell("Live Status Board", body)

@app.route("/feature-radar")
def feature_radar():
    body = """
    <div class="hero hero-center">
      <h1>Feature Radar</h1>
      <p>What is already built, what is partially built, and what still needs hardening.</p>
    </div>

    <div class="grid">
      <div class="card"><div class="pill">Built</div><h2>Core Platform Shell</h2><p>Marketplace, AI, finance, training, media, and holoverse structure are in place.</p></div>
      <div class="card"><div class="pill">Built</div><h2>Brand Identity</h2><p>Stubbs Crest, Holographic Lion Saturn, and American Flag visuals are wired in.</p></div>
      <div class="card"><div class="pill">Partial</div><h2>Search System</h2><p>Cross-module search exists, but ranking and polished result cards still need improvement.</p></div>
      <div class="card"><div class="pill">Partial</div><h2>Uploads</h2><p>Upload structures exist, but true upload handling still needs wiring.</p></div>
      <div class="card"><div class="pill">Partial</div><h2>AI Actions</h2><p>Workflow dashboards exist, but real action execution still needs wiring.</p></div>
      <div class="card"><div class="pill">Important</div><h2>Security + Finance Hardening</h2><p>Protected routes, stronger controls, and real money safety still need final hardening.</p></div>
    </div>
    """
    return app_shell("Feature Radar", body)
...
def api_system_readiness():
    ...
    readiness_file = Path("config/system_readiness.json")
    if not readiness_file.exists():
        return jsonify({"status": "missing", "message": "system_readiness.json not found"}), 404
    return jsonify(json.loads(readiness_file.read_text()))

@app.route("/system-readiness")
def system_readiness():
    ...
    <div class="hero hero-center">
      ...
      <p>Main readiness board for what is working, what is missing, and what is detrimental right now.</p>
      <div class="hero-badges">
        <span class="pill">Status: {data.get('status', 'unknown').title()}</span>
        <span class="pill">Live Project: ~/marketplace</span>
      </div>
    </div>

    <div class="grid">
      <div class="card">
        <h2>Important Missing</h2>
        <ul>{li(data.get('important_missing', []))}</ul>
      </div>
      <div class="card">
        <h2>Detrimental Risks</h2>
        <ul>{li(data.get('detrimental_risks', []))}</ul>
      </div>
      <div class="card">
        <h2>Working Surfaces</h2>
        <ul>{li(data.get('working_surfaces', []))}</ul>
      </div>
      <div class="card">
        <h2>Recommended Next Order</h2>
        <ol>{''.join(f"<li>{item}</li>" for item in data.get('recommended_next_order', []))}</ol>
      </div>
    </div>

    <div class="button-row" style="justify-content:center;margin-top:20px">
      <a class="btn" href="/app-home">Open App Home</a>
      <a class="btn-secondary" href="/missing-pieces-center">Open Missing Pieces</a>
      <a class="btn-secondary" href="/gap-fix-center">Open Gap Fix</a>
    </div>
    """
    return app_shell("System Readiness", body)


@app.route("/api/creator-uploads")
def api_creator_uploads():
    import sqlite3
    conn = sqlite3.connect("instance/app.db")
    conn.row_factory = sqlite3.Row
...
@app.route("/upload-creator-asset", methods=["POST"])
def upload_creator_asset():
    import sqlite3, os, re, time
    from pathlib import Path
    from flask import request, redirect

    title = (request.form.get("title") or "Untitled Asset").strip()
    owner_name = (request.form.get("owner_name") or "Unknown Owner").strip()
    category = (request.form.get("category") or "general").strip()
    f = request.files.get("file")

    if not f or not f.filename:
        return redirect("/upload-studio?status=missing_file")
    ...
    cur.execute(
        ...
        (title, final_name, file_type, owner_name, category, "uploaded")
    )
    conn.commit()
    conn.close()

    return redirect("/upload-studio?status=uploaded")


@app.route("/upload-studio")
def upload_studio():
    from flask import request
    status = (request.args.get("status") or "").strip()
    status_html = ""
    if status == "uploaded":
        status_html = '<p style="color:#86efac">Upload saved successfully.</p>'
    elif status == "missing_file":
        status_html = '<p style="color:#fbbf24">Please choose a file first.</p>'

    body = f"""
    <div class="hero">
      <h1>Upload Studio</h1>
      <p>Upload images, audio, video, and files into your flagship creator asset system.</p>
      <div class="badges">
    ...
    <div class="split">
      <div class="panel">
        <h2>New Upload</h2>
        {status_html}
        <form action="/upload-creator-asset" method="post" enctype="multipart/form-data">
      ...
      <div class="panel">
        ...
        <p>This is what starts turning the platform into a real creator and media app instead of only seeded demo records.</p>
        <p class="muted">API: <a href="/api/creator-uploads">/api/creator-uploads</a></p>
      </div>
    </div>
    """
    return minimal_shell("Upload Studio", body)

@app.route("/search-ultra")
def search_ultra():
    body = """
    <div class="hero">
      <h1>Search Ultra</h1>
      <p>A flagship cross-platform search surface for media, uploads, music, streams, wallet records, modules, workflows, and branded assets.</p>
      <div class="badges">
        <span class="badge">Cross-Module Search</span>
        <span class="badge">Media Previews</span>
        <span class="badge">Premium Layout</span>
      </div>
    </div>

    <div class="toolbar">
      <input id="q" placeholder="Search everything: music, wallet, aniyah, lion, stream, brand...">
      ...
      <button onclick="runUltraSearch()">Search</button>
    </div>
    ...
    <div class="split">
      <div class="panel">
        <h2>Search guide</h2>
        <ul>
          <li>Search uploaded images, video, and audio</li>
          <li>Search wallet, media, university, and workflow data</li>
          <li>Filter by source or file type</li>
        </ul>
        <p class="muted">Examples: music, lion, wallet, brand, aniyah, stream</p>
      </div>
      <div class="panel">
        <h2>Results</h2>
    ...
    function renderPreview(r) {
      ...
      if ((source === "creator_uploads" || source === "media_assets") && filename) {
        const src = filename.startsWith("http") ? filename : (filename.includes("/") ? "/" + filename.replace(/^\/+/, "") : "/static/uploads/" + filename);
        if (fileType === "image" || /\.(png|jpg|jpeg|gif|webp|svg)$/i.test(filename)) {
          return `<img class="preview" src="${src}" alt="preview">`;
        }
        if (fileType === "video" || /\.(mp4|mov|webm|mkv)$/i.test(filename)) {
          return `<video class="preview" controls src="${src}"></video>`;
        }
        if (fileType === "audio" || /\.(mp3|wav|ogg|m4a)$/i.test(filename)) {
          return `<audio style="width:100%;margin-bottom:12px" controls src="${src}"></audio>`;
        }
      }
      return `<div class="preview" style="display:flex;align-items:center;justify-content:center;color:#94a3b8">No Preview</div>`;
    }
    ...
    async function runUltraSearch() {
      ...
      if (typeFilter) results = results.filter(r => (r.file_type || "") === typeFilter);

      if (!results.length) {
        box.innerHTML = '<div class="card"><h3>No results found</h3><p>Try another keyword or clear filters.</p></div>';
        return;
      }

      box.innerHTML = results.map(r => {
        const title =
          r.title || r.asset_title || r.module_name || r.account_name || r.channel_name ||
          r.workflow_name || r.lesson_title || r.sender_name || r.symbol || "Result";

        const meta = Object.entries(r)
          .filter(([k]) => !["_fields"].includes(k))
          .map(([k,v]) => `<div><strong>${k}:</strong> ${v}</div>`)
          .join("");

        return `
          <div class="card">
            <div class="pill">${r._source || 'result'}</div>
            ${renderPreview(r)}
            <h3>${title}</h3>
            <div class="muted" style="display:grid;gap:6px">${meta}</div>
          </div>
        `;
      }).join('');
    }
    ...
    """
    return minimal_shell("Search Ultra", body)

@app.route("/creator-market")
def creator_market():
    body = """
    <div class="hero">
      <h1>Creator Market</h1>
      <p>A premium creator asset surface for images, audio, video, brand visuals, uploads, and future creator commerce.</p>
      <div class="badges">
        <span class="badge">Uploads</span>
        <span class="badge">Previews</span>
        <span class="badge">Brand Assets</span>
      </div>
      <div class="cta-row">
        <a class="btn" href="/upload-studio">Upload Studio</a>
        <a class="btn-secondary" href="/media-studio">Media Studio</a>
      </div>
    </div>

    <div class="grid">
      <div class="card">
        <img class="preview" src="/static/img/stubbs_crest.svg" alt="Stubbs Crest">
        <h3>Stubbs Crest</h3>
        <p>Core flagship identity asset.</p>
      </div>
      <div class="card">
        <img class="preview" src="/static/img/holographic_lion_saturn.svg" alt="Holographic Lion Saturn">
        <h3>Holographic Lion Saturn</h3>
        <p>Premium visual flagship symbol.</p>
      </div>
      <div class="card">
        <img class="preview" src="/static/img/american_flag_holo.svg" alt="American Flag">
        <h3>Holographic American Flag</h3>
        <p>National visual woven into the flagship brand layer.</p>
      </div>
    </div>
    """
    return minimal_shell("Creator Market", body)
