...
def business_lanes():
    c = conn()
    c.row_factory = sqlite3.Row
    rows = c.execute("SELECT * FROM heir_business_lanes ORDER BY id").fetchall()
    c.close()

    cards = ""
    for r in rows:
        cards += f"<div class='card'><h2>{esc(r['person_name'])}</h2><p>{esc(r['lane_title'])}</p><a href='/business-lane/{esc(r['slug'])}'>Open</a></div>"
...
def business_lane(slug):
    ...
    items = esc(row["business_items"]).replace(",", "<br>")
    return page(row["lane_title"], f"""
    <div class="card">
      <h1>{esc(row["lane_title"])}</h1>
      <p><strong>Person:</strong> {esc(row["person_name"])}</p>
      <p>{items}</p>
    </div>
    <div class="grid">
      <div class="card"><h2>Family Home</h2><a href="/family-launch-home">Open</a></div>
...
def event_center():
    c = conn()
    c.row_factory = sqlite3.Row
    rows = c.execute("SELECT * FROM event_items ORDER BY id DESC LIMIT 50").fetchall()
    c.close()

    cards = ""
    for r in rows:
        cards += f"<div class='card'><h2>{esc(r['event_title'])}</h2><p><strong>Type:</strong> {esc(r['event_type'])}</p><p><strong>Date:</strong> {esc(r['event_date'])}</p><p>{esc(r['notes'])}</p></div>"
