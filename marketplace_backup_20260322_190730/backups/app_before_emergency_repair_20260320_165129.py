...
def watch_show(show_id):
    body = f"""
    <a class="back-btn" href="/shows-demo">&larr; Back</a>
    <div class="media-hero">
      <h1>Now Watching</h1>
      <p>Show ID: {show_id}</p>
    </div>
    <div class="player-box">
      <div class="player-screen">Video Player Demo</div>
    ...
    """
    return media_player_shell("Watch Show", body)

@app.route("/play-track/<int:track_id>")
def play_track(track_id):
    body = f"""
    <a class="back-btn" href="/music-demo">&larr; Back</a>
    <div class="media-hero">
      <h1>Now Playing</h1>
      <p>Track ID: {track_id}</p>
    </div>
    <div class="player-box">
      <div class="player-screen">Audio Player Demo</div>
      <div class="player-controls">
        <button type="button">Play</button>
        <button type="button">Pause</button>
      </div>
    ...
    """
    return media_player_shell("Play Track", body)
...
def shows_demo():
    ...
    """
    return media_player_shell("Shows Demo", body)
...
def music_demo():
    ...
    """
    return media_player_shell("Music Demo", body)

@app.route("/world/<world_name>")
def holoverse_world_page(world_name):
    action_map = {
        "Marketplace": [("Browse Drops", "/holo-commerce-home"), ("Open Creator Tools", "/creator-tools-home")],
        "Streaming Studio": [("Watch Shows", "/shows-demo"), ("Play Music", "/music-demo"), ("Open Coaching", "/music-coaching-home")],
        "Creator Arena": [("Upload Files", "/files-center-v4"), ("Artist Signing", "/artist-signing-home")],
        "University": [("Open Learning", "/all-american-marketplace-university")],
        "Armed Forces Command": [("Open Service Center", "/armed-forces-center")],
        "Nigeria Hub": [("Open Regional Hub", "/world-expansion-center-v2")],
        "UK Hub": [("Open Regional Hub", "/world-expansion-center-v2")],
    }

    cards = ""
    for title, href in action_map.get(world_name, []):
        cards += f'<div class="world-card"><h2>{title}</h2><a href="{href}">Open</a></div>'

    body = f"""
    <div class="world-hero">
      <a class="btn-secondary" href="/holoverse-lobby-v2">Back to Holoverse</a>
      <h1>{world_name}</h1>
      <p>World actions are now connected.</p>
    </div>
    <div class="world-grid">
      {cards if cards else '<div class="world-card"><h2>No actions yet</h2><p>This world needs actions connected.</p></div>'}
    </div>
    """
    ...
    <head>
      ...
      <link rel="stylesheet" href="/static/css/fivepm_lock.css">
      <link rel="stylesheet" href="/static/css/holoverse_actions.css">
    </head>
    <body>
