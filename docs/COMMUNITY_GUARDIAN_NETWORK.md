# TRYAMM Community Guardian Network

Status: LOCKED PRODUCT / BUSINESS SPECIFICATION

## Mission
Create a visible, non-vigilante community safety and hospitality network that helps people feel safer, connects them to help, reduces avoidable conflict, supports youth and vulnerable people, and gives businesses/campuses/neighborhoods a practical alternative for low-risk safety needs.

Community Guardians are NOT police, do not claim law-enforcement authority, do not pursue suspects, do not conduct searches, do not detain people, and do not use the program as a cover for vigilantism or intimidation.

## Core services
- Safe Walk / Safe Ride coordination
- hospitality and directions
- visible unarmed community presence
- business and storefront check-ins
- de-escalation and conflict interruption within training/scope
- youth engagement and mentorship
- homeless/resource outreach and referrals
- lost-child / vulnerable-person assistance
- event support
- transit-stop / school-route presence where contracted and lawful
- non-emergency hazard reporting
- emergency escalation to 911 / local authorities when needed
- documentation of low-risk incidents and service requests
- accessibility assistance
- Community Gap Finder referrals

## App experience
HOME → `I NEED A SAFE WALK` | `REQUEST A GUARDIAN` | `REPORT A HAZARD` | `YOUTH / COMMUNITY HELP` | `BUSINESS SAFETY CHECK-IN` | `EVENT SUPPORT`.

### Safe Walk
REQUEST → pickup → destination → accessibility needs → guardian/team assignment → live ETA → consented tracking during mission → check-in → completed → feedback.

### Business Guardian
Businesses can subscribe to scheduled storefront check-ins, opening/closing accompaniment, employee safe-walks, hospitality support, incident documentation and referral to emergency/security services where appropriate.

### Community Guardian JARVIS
JARVIS can route service requests, provide approved de-escalation scripts/training reminders, surface nearby safe resources, coordinate check-ins, document non-sensitive outcomes and escalate emergencies. It cannot instruct vigilantism, confrontation, pursuit or illegal detention.

## Workforce pathways
Roles can include:
- Community Ambassador
- Youth Mentor
- Safe-Walk Ambassador
- Outreach Navigator
- Event Ambassador
- Accessibility Ambassador
- Team Lead
- Dispatcher
- Program Supervisor
- Training Coordinator

Where a jurisdiction classifies a role as private security or requires licensing, TRYAMM must route the employer/operator through the applicable licensing requirements instead of relabeling regulated guard work.

## Training baseline
- de-escalation
- conflict avoidance
- customer service/hospitality
- emergency recognition/escalation
- CPR/first aid where appropriate
- trauma-informed interaction
- youth safeguarding
- disability/accessibility awareness
- homelessness/resource navigation
- bias/non-discrimination
- documentation/privacy
- boundaries: no police impersonation, searches, detention or pursuit

## Business model
Potential revenue lanes:
1. Business Improvement District contracts
2. campus/university contracts
3. hospital/health-system campus contracts
4. residential/community association contracts
5. retail/shopping district subscriptions
6. employer safe-walk programs
7. event support contracts
8. transit-area contracts where procured
9. sponsored youth/community safety programs
10. city/county grants/contracts for eligible outreach or violence-prevention services
11. training/certification-prep services where lawful and not misrepresented as government licensing
12. software/SaaS fee for Guardian Dispatch, reporting and analytics

### Suggested pricing architecture
Pricing is local and labor-cost dependent. Use transparent components:
BASE PLATFORM + STAFF HOURS + SUPERVISION + INSURANCE/REQUIRED LICENSING + EQUIPMENT + TRAINING + ADMIN + MARGIN.

Do not underprice labor by relying on unpaid volunteers for commercial contracts. Community volunteers may exist in separate, well-scoped service programs, but commercial clients should fund trained staff.

## Profitability & 3.00x sustainability
Guardian Network contributes eligible TRYAMM platform revenue through SaaS/dispatch subscriptions, employer/business subscriptions and contracted program-management/software fees. Wages owed to ambassadors and restricted grant/program funds are not counted as free platform revenue.

A healthy contract should target positive contribution margin after direct labor, supervision, insurance, training, payroll burden, equipment and incident support. TRYAMM's platform-wide goal remains 3.00x eligible platform revenue to measured infrastructure cost; Guardian labor economics are additionally tracked at the program level.

## Evidence / impact dashboard
Aggregate privacy-preserving measures:
- safe walks completed
- average response time
- business check-ins
- hazards reported/resolved
- outreach referrals
- youth mentor engagements
- emergency escalations
- user-reported safety/hospitality satisfaction
- incidents avoided/de-escalated where evidence supports it
- jobs created

Do not claim crime reduction without appropriate comparative evidence.

## Relationship to existing systems
Accessibility Passport → appropriate participation/request UI
Student JARVIS / Launchpad → youth mentorship/workforce pathway
Emergency Path → urgent resource routing
Community Gap Finder → underserved-area needs
Quantum Discord → community groups and mission coordination
Quantum Zapier → request/dispatch/notification workflows
Jin Pay → approved payments/contracts
Money Engine → payroll/program/mission separation
Company Digital Twin → operator staffing, contracts, service zones, costs
Business Simulator → model staffing/service levels and contract economics
Community Spotlight → consented impact stories

## Safety principle
`BE PRESENT → HELP → DE-ESCALATE → CONNECT → DOCUMENT → ESCALATE WHEN REQUIRED`.
Not: patrol for suspects, chase people, act as police, or physically enforce laws.
