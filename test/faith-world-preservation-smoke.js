...
const streetPortal=fs.readFileSync('amm-omniverse/src/components/StreetVerseFaithChronoPortal.tsx','utf8');
...
assert(main.includes('StreetVerseFaithChronoPortal'), 'StreetVerse must mount Faith Chrono portal bridge');
...
for(const token of ['tryamm:streetverse-enter','tryamm:chrono-resume','ETHIOPIAN BIBLE · FAITH WORLD']) assert(streetPortal.includes(token),`StreetVerse Faith Chrono portal missing ${token}`);
...
console.log('Faith-world, Faith Chrono, Holo Lab, StreetVerse portal, 88-book curriculum and Kingdoms Press preservation checks passed');
