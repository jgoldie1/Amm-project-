...
for (const layer of ['ROADS','BUILDINGS','TRANSIT','BUSINESSES','NPC_POPULATION','TRAFFIC','MISSIONS','PROPERTIES','CREATOR_LOCATIONS','ECONOMY','STREAMING_LOD']) {
  assert(job.playableSlice.compile.layers.includes(layer), `${layer} missing from playable neighborhood slice`);
}
...
console.log('TRYAMM world compiler, Chicago 77 Neighborhood Compiler, Google reference boundary, After Dark, equal-area map and supply-chain smoke checks passed');
