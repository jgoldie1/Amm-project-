...
for (const layer of ['ROADS','BUILDINGS','TRANSIT','BUSINESSES','NPC_POPULATION','TRAFFIC','MISSIONS','PROPERTIES','CREATOR_LOCATIONS','ECONOMY','STREAMING_LOD']) {
  assert(job.playableSlice.compile.layers.includes(layer), `${layer} missing from reusable Neighborhood Compiler`);
}
...
assert.strictEqual(job.geospatial.googleReferenceOnly, true, 'Google imagery must remain a reference layer');
assert.strictEqual(job.geospatial.googleGeometryExtractionAllowed, false, 'Google imagery cannot be a geometry extraction source');
...
assert.strictEqual(chicago77.slices.length, 77, 'Neighborhood Compiler must produce all 77 reusable slice plans');
...
console.log('TRYAMM Chicago 77 Neighborhood Compiler, playable slices, Street View policy, certification, After Dark and supply-chain checks passed');
