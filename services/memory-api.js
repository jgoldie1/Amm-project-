...
router.get('/status',(req,res)=>res.json({configured:configured(),embedding:Boolean(process.env.EMBEDDING_API_URL&&process.env.EMBEDDING_API_KEY&&process.env.EMBEDDING_MODEL),service:'Googolplex Memory',africaCommerce:true,globalCommerce:true,quantumVocalStudio:true,aniyahCrossborder:true,globalGrowth:true}));
router.use(auth);
