...
const globalGrowthApi=require('./global-growth-api');
const generationalIntelligenceApi=require('./generational-intelligence-api');
...
router.use('/growth',globalGrowthApi.router);
router.use('/generations',generationalIntelligenceApi.router);
router.get('/status',(req,res)=>res.json({configured:configured(),embedding:Boolean(process.env.EMBEDDING_API_URL&&process.env.EMBEDDING_API_KEY&&process.env.EMBEDDING_MODEL),service:'Googolplex Memory',africaCommerce:true,globalCommerce:true,quantumVocalStudio:true,aniyahCrossborder:true,globalGrowth:true,generationalIntelligence:true,agiClaim:'adaptive multi-agent intelligence; not human-level AGI'}));
router.use(auth);
